...
export const agentPlatformBriefs = {
  "veadk": {
    ...
    "principles": [
      {
        "zh": "版本化 Agent 定义",
        "en": "Versioned Agent Definition",
        "explanation": "Agent 把名称、模型、指令、工具和记忆配置形成可测试的代码对象。",
        "decision": "把代码、模型、Prompt、工具 Schema 和运行配置作为同一候选版本验收。"
      },
      {
        "zh": "Runner 执行循环",
      ...
      },
      {
        "zh": "工具合同",
        "en": "Tool Contract",
        "explanation": "工具以明确用途、类型、错误和副作用合同连接外部能力，模型只提出调用。",
        "decision": "应用在执行时重新校验身份、权限、参数、超时和幂等。"
      },
      {
        "zh": "会话作用域",
        "en": "Session Scope",
      ...
      },
      {
        "zh": "分层状态",
        "en": "Layered State",
        "explanation": "短期会话、长期记忆和权威业务事实具有不同生命周期、隔离与纠错责任。",
        "decision": "跨会话偏好进入受治理记忆；余额、订单和权限仍实时读取权威系统。"
      },
      {
        "zh": "显式应用适配",
        "en": "Explicit App Integration",
        "explanation": "既有 root_agent 可通过集成入口封装为 AgentKit App，使开发定义进入应用合同。",
        "decision": "分别验收本地应用适配和云端部署，不能把二者写成同一结果。"
      }
    ],
    "decisions": [
      {
        "question": "使用本地调试入口还是正式应用入口？",
        "signal": "只需快速检查 Prompt/Tool，或需要稳定 API、资源绑定和后续部署。",
        "recommendation": "开发期可用本地调试；集成验收使用显式 App 入口并固定依赖。",
        "boundary": "本地页面可用不证明云端 Runtime、身份和网络可用。"
      },
      {
        "question": "复用内置 Tool 还是自定义 Tool？",
        "signal": "通用能力已覆盖，或业务系统需要专用 Schema、授权和错误语义。",
        "recommendation": "通用读取与搜索优先复用官方 Tool；业务动作使用最小职责的自定义合同。",
        "boundary": "可发现、可注册都不等于当前用户获准执行。"
      },
      {
        "question": "短期会话使用什么存储？",
        "signal": "单机实验、进程重启恢复，或多实例共享与故障切换。",
        "recommendation": "本地实验可用 SQLite；多实例采用共享数据库或托管 Session 服务并做隔离回归。",
        "boundary": "文件持久化不等于多实例一致性、备份和灾备。"
      },
      {
        "question": "何时需要长期记忆？",
        "signal": "确有跨会话偏好或任务连续性，且有主体、用途、纠错和删除规则。",
        "recommendation": "先保留 Session 边界，再把少量合格候选写入持久化记忆后端。",
        "boundary": "相似检索只能找相关内容，不能裁决真实性和当前有效性。"
      },
      {
        "question": "何时接入 AgentKit？",
        "signal": "本地 Agent 已稳定，需要应用合同、云端 Runtime、资源绑定和发布治理。",
        "recommendation": "冻结 root_agent、入口、依赖和验收集后再进入 AgentKit 构建与部署。",
        "boundary": "适配函数返回应用对象不等于已经上云。"
      }
    ],
    ...
    "deepDives": [
      {
        "kind": "sequence",
        "eyebrow": "DELIVERY & EXECUTION CONTRACT",
        "title": "一次 VeADK 请求的五个可检查对象",
        "intro": "开发准备、应用适配和请求执行分别留下版本、接口、Session、事件与状态证据。最终回答只是其中一项。",
        "sourceIds": [
        ...
        "items": [
          {
            "name": "冻结 Agent 定义",
            "en": "Freeze Definition",
            "mechanism": "绑定模型、指令、工具与状态配置。",
            "decision": "记录候选版本与测试输入。",
            "boundary": "名称一致不代表行为一致。"
          },
          {
            "name": "封装应用合同",
            "en": "Adapt Application",
            "mechanism": "把 root_agent 交给 AgentKit 集成入口。",
            "decision": "用 API、错误和版本合同验收。",
            "boundary": "本地 API 覆盖应用合同；云部署另有 Runtime 与目标环境记录。"
          },
          {
            "name": "建立会话",
            "en": "Create Session",
            "mechanism": "以应用、用户和会话范围读取事件。",
            "decision": "执行同会话、换会话和换用户的正反例。",
            "boundary": "会话 ID 只用于作用域定位，认证由可信身份链完成。"
          },
          {
            "name": "运行模型与工具循环",
            "en": "Run Loop",
            "mechanism": "模型提出动作，应用执行后把结果送回下一轮判断。",
            "decision": "限制调用、时间、Token 和副作用预算。",
            "boundary": "Prompt 不能替代执行授权。"
          },
          {
            "name": "保存必要状态",
            "en": "Persist State",
            "mechanism": "按后端保存会话事件；长期记忆走独立写入路径。",
            "decision": "为重试、纠错和删除保留来源。",
            "boundary": "事件写入不证明外部业务动作已经成功。"
          }
        ]
      },
      {
        "kind": "diagnostic",
        "eyebrow": "MEMORY TRIAGE",
        "title": "记忆召回失败的五个检查点",
        "intro": "检查表记录实际作用域、后端、写入确认、工具调用和应用入口，每项附观察结果。",
        "sourceIds": [
        ...
        "items": [
          {
            "name": "作用域不一致",
            "en": "Scope Mismatch",
            "mechanism": "应用、用户或会话标识变化。",
            "decision": "记录实际三元组并做正反例。",
            "boundary": "修正 ID 不能解决身份伪造。"
          },
          {
            "name": "后端生命周期不匹配",
            "en": "Backend Lifetime",
            "mechanism": "进程内或文件后端不满足共享与恢复要求。",
            "decision": "按实例数和恢复目标选后端。",
            "boundary": "换后端仍需迁移与一致性验证。"
          },
          {
            "name": "写入路径未闭环",
            "en": "Write Path",
            "mechanism": "应用完成写入请求后，目标后端是否保存并可检索仍需验证。",
            "decision": "把写入确认与后续检索分成两项检查。",
            "boundary": "等待不能掩盖写入失败。"
          },
          {
            "name": "模型未调用记忆工具",
            "en": "Tool Not Invoked",
            "mechanism": "完整 Agent 路径受模型选择影响。",
            "decision": "用直接搜索与 Agent 调用分层检查。",
            "boundary": "强制业务步骤应由工作流保证。"
          },
          {
            "name": "应用入口接线不同",
            "en": "Integration Mismatch",
            "mechanism": "调试入口与正式 App 可能使用不同服务装配。",
            "decision": "显式打印并验收 Session/Memory 后端。",
            "boundary": "临时兼容层不应成为生产架构。"
          }
        ]
      }
    ],
    ...
    "cloudHooks": [
      {
        "stage": "模型与向量化",
        "services": "模型 API、Embedding、密钥管理",
        "value": "以受控配置连接模型推理和语义检索。",
        "discover": "客户使用哪些模型、地域、配额和数据边界？"
      },
      {
        "stage": "会话与记忆",
        "services": "共享数据库、托管 Session、Mem0 或其他长期记忆服务",
        "value": "使状态独立于单个进程生命周期。",
        "discover": "哪些状态需要跨实例或跨会话，谁负责纠错和删除？"
      },
      {
        "stage": "应用与 Runtime",
        "services": "AgentKit App、构建与云端 Runtime",
        "value": "把同一 Agent 定义连接到可部署应用合同。",
        "discover": "入口、依赖、资源绑定和回退是否可重复？"
      },
      {
        "stage": "身份与运行证据",
        "services": "IAM、密钥、日志、Trace、指标与告警",
        "value": "保留主体、调用、错误和版本证据。",
        "discover": "一次请求能否追到模型、工具、状态和权威结果？"
      }
    ],
    "relatedSlugs": [
      "agentkit",
    ...
    "qa": [
      {
        "q": "VeADK 和 AgentKit 的职责有什么不同？",
        "a": "VeADK 负责用代码定义和执行 Agent，AgentKit 负责把应用构建、部署并接入云端运行治理。",
        "depth": "二者通过应用适配相连，但开发框架、应用合同和 Runtime 是三个独立验收面。",
        "ask": "客户当前缺的是 Agent 逻辑、应用接口，还是云端运行治理？",
        "tag": "产品定位",
        "basis": "官方平台与集成文档",
        "evidence": [
          {
      ...
      {
        "q": "为什么使用 VeADK 时会出现 Google ADK 类型或日志？",
        "a": "当前官方源码快照中的 VeADK Agent 与 Runner 复用或继承了 Google ADK 的部分类型和执行能力。",
        "depth": "这是实现关系，不要求业务代码转为直接使用底层 API；升级任一依赖时应对 Agent、Runner、Session、Tool 和 Memory 做组合回归。",
        "ask": "当前是否锁定了 VeADK 与底层依赖版本，并保留关键回归集？",
        "tag": "继承与版本",
        "basis": "当前官方源码快照",
        "evidence": [
          {
      ...
      {
        "q": "Tool 已注册，是否代表当前用户获准使用且模型一定会调用？",
        "a": "都不代表。注册只让模型看到能力，调用选择和执行授权仍是不同步骤。",
        "depth": "可选工具可由模型判断；必须发生的控制放进确定性工作流。执行前仍要校验主体、资源、动作、参数和副作用。",
        "ask": "该 Tool 是只读、可逆写入还是高影响动作，由谁授权？",
        "tag": "工具边界",
        "basis": "官方内置工具与 Agent 机制",
        "evidence": [
          {
      ...
      {
        "q": "SQLite Session 能直接支撑多实例生产吗？",
        "a": "通常不能；单机文件适合开发和恢复实验，多实例需要共享后端与一致性、备份和故障切换设计。",
        "depth": "多实例还要验证同一会话的并发写、重复请求、路由切换、恢复点和隔离，不应把进程重启可恢复外推为分布式就绪。",
        "ask": "目标实例数、并发写入、RPO/RTO 和数据地域要求是什么？",
        "tag": "多实例状态",
        "basis": "官方短期记忆后端边界",
        "evidence": [
          {
      ...
      {
        "q": "怎样区分短期记忆、长期记忆和权威业务事实？",
        "a": "短期记忆服务当前会话，长期记忆保存跨会话的受治理信息，当前事实由对应的权威来源确认。",
        "depth": "长期记忆条目需要主体、来源、用途、时间、有效期、纠错和删除；检索只提供相关候选，余额、订单、权限和政策版本仍应查询各自的权威记录。",
        "ask": "这条信息由谁产生、多久有效、谁能更正删除，错误时会影响什么决定？",
        "tag": "状态分层",
        "basis": "短期记忆机制 + 权威事实边界",
        "evidence": [
          {
      ...
      {
        "q": "`create_agentkit_app` 与本地调试入口有什么不同，调用成功是否等于已经上云？",
        "a": "前者把 root_agent 显式封装为 AgentKit App；本地调试入口主要服务开发，两者调用成功都不等于 Runtime 已部署。",
        "depth": "正式上云还要完成构建、Runtime、目标配置、Region、环境变量、资源绑定、可信身份、网络、云端回归、观测和回退。",
        "ask": "当前证据覆盖本地 App、构建、Runtime 还是业务上线中的哪一层？",
        "tag": "应用适配",
        "basis": "官方集成、CLI 与 Runtime 文档",
        "evidence": [
          {
            "sourceId": "veadk-agentkit-integration-2026-08-15",
    ...
    "evidenceCards": [
      {
        "metric": "当前源码快照",
        "title": "VeADK 扩展 Google ADK 的 Agent 抽象",
        "finding": "固定版本的 VeADK Agent 当前继承 Google ADK LlmAgent，因此上游行为属于兼容性边界。",
        "boundary": "该快照不保证未来继承关系、版本兼容或未经回归的应用行为。",
        "sourceId": "veadk-agent-source-2026-08-15",
        "accent": true
      },
      {
        "metric": "多实例边界",
        "title": "分布式持久化不同于进程本地状态",
        "finding": "短期记忆文档把 MySQL 和 PostgreSQL 列为分布式持久化后端，区别于内存与本地文件选项。",
        "boundary": "选择分布式后端也不自动证明故障切换、并发行为、租户隔离、保留和恢复。",
        "sourceId": "veadk-short-term-memory-2026-08-15"
      },
      {
        "metric": "适配 ≠ 上云",
        "title": "应用适配只是生产链的一段",
        "finding": "官方集成提供把 VeADK Agent 封装为 AgentKit App 的入口。",
        "boundary": "仍需独立完成构建、Runtime、身份、网络、资源绑定、观测和回退验收。",
        "sourceId": "veadk-agentkit-integration-2026-08-15",
        "accent": true
      }
    ]
  ...
  "agentkit": {
    ...
    "principles": [
      {
        "zh": "应用合同",
        "en": "Application Contract",
        "explanation": "入口、依赖、配置和调用接口共同定义 AgentKit 能构建和运行的应用。",
        "decision": "干净环境中的启动、错误语义和核心调用结果写入构建输入清单。"
      },
      {
        "zh": "配置与构建",
        "en": "Configuration & Build",
        "explanation": "CLI 和配置文件把代码、依赖、Region、环境变量与资源引用组合为部署输入。",
        "decision": "把本地隐式状态转成可审计、可重复的构建材料。"
      },
      {
        "zh": "Runtime 生命周期",
        "en": "Runtime Lifecycle",
        "explanation": "Runtime 承载部署后的应用版本并提供运行状态与调用入口。",
        "decision": "分别验收 Ready、可调用、业务后置条件、恢复和回退。"
      },
      {
        "zh": "部署目标",
        "en": "Deployment Target",
        "explanation": "目标标识、Region 与资源引用共同说明部署去向；实际资源访问、网络和数据边界还要在目标环境验证。",
        "decision": "把目标配置、访问权限、网络隔离和数据地域分开确认。"
      },
      {
        "zh": "外置状态",
        "en": "Externalized State",
        "explanation": "Session、Memory 和 Knowledge 不应依赖单个 Runtime 容器生命周期。",
        "decision": "记录控制面资源关联与数据面访问配置，并做跨版本回归。"
      },
      {
        "zh": "运行证据",
        "en": "Operational Evidence",
        "explanation": "日志、Trace、指标、评测和外部压力测试共同回答不同问题。",
        "decision": "用 SLO、任务成功和恢复结果决定放量，不用一个平均值代替。"
      }
    ],
    "decisions": [
      {
        "question": "什么时候需要 AgentKit？",
        "signal": "已有可运行 Agent，但缺少标准应用合同、云端 Runtime、资源管理与发布运营。",
        "recommendation": "先冻结本地候选与验收集，再引入 AgentKit 管理构建和运行生命周期。",
        "boundary": "平台不能替客户补齐错误的业务流程或授权。"
      },
      {
        "question": "怎样核对部署目标与 Region？",
        "signal": "已有获准的部署范围、地域要求、服务依赖和网络路径。",
        "recommendation": "在配置中明确目标标识与 Region，再从目标 Runtime 验证资源访问、服务可用性、网络和数据控制。",
        "boundary": "字段填写正确只证明控制面输入完整，不证明隔离、连通或数据驻留已经满足。"
      },
      {
        "question": "Memory 采用托管还是自建？",
        "signal": "团队在交付速度、数据边界、功能、可移植性和运维责任之间取舍。",
        "recommendation": "若评估 Mem0 方案，在同一用户隔离、召回质量、延迟、删除和恢复合同下比较托管 Platform 与 OSS。",
        "boundary": "Mem0 的官方对比不能证明 AgentKit Memory 使用或由 Mem0 运营，也不能证明特定地域、数据驻留或 SLA。自建 Mem0 仍可能把数据发送到外部模型、Embedding 或向量服务。"
      },
      {
        "question": "何时允许 launch？",
        "signal": "入口、依赖、配置、密钥、资源绑定和回退方案已冻结。",
        "recommendation": "先运行配置检查和本地调用，再构建、部署并在云端执行相同回归。",
        "boundary": "一条命令简化编排，不消除上线批准与生产验证。"
      },
      {
        "question": "怎样建立可观测与容量基线？",
        "signal": "需要定位慢请求、验证质量或确定并发上限。",
        "recommendation": "平台遥测用于还原调用链，评测衡量任务质量，k6、Locust 等外部工具产生固定负载。",
        "boundary": "有观测数据不等于已经定义 SLO，评测也不等于压力测试。"
      }
    ],
    ...
    "deepDives": [
      {
        ...
        "title": "App、镜像、Runtime 与回退的六项记录",
        "intro": "每一步保存对象摘要、执行结果和失败原因。成功截图可以作为附件，发布结论依赖完整记录。",
        "sourceIds": [
        ...
        "items": [
          {
            "name": "冻结应用入口",
            "en": "Freeze Entry",
            "mechanism": "确认启动对象、接口和错误合同。",
            "decision": "在干净、可复现的环境中运行。",
            "boundary": "本地运行依赖不能隐式进入构建。"
          },
          {
            "name": "冻结配置",
            "en": "Freeze Config",
          ...
          },
          {
            "name": "构建候选",
            "en": "Build Candidate",
            "mechanism": "解析依赖并生成可部署制品。",
            "decision": "绑定源码、依赖与构建结果。",
            "boundary": "构建成功不证明运行成功。"
          },
          {
            "name": "部署 Runtime",
            "en": "Deploy Runtime",
            "mechanism": "创建或更新云端运行版本。",
            "decision": "记录状态、日志、失败原因和实际版本。",
            "boundary": "Ready 覆盖平台资源状态，业务可用性由云端回归与 SLO 验收。"
          },
          {
            "name": "执行云端回归",
            "en": "Cloud Regression",
            "mechanism": "验证模型、工具、Session、Memory、身份和网络。",
            "decision": "使用与本地同源的任务集。",
            "boundary": "一次调用不能形成 SLO。"
          },
          {
            "name": "放量或回退",
            "en": "Release or Roll Back",
            "mechanism": "按质量、P95、错误、恢复和成本决定。",
            "decision": "保留上一验证版本与状态兼容方案。",
            "boundary": "回退应用不能撤销已产生的业务副作用。"
          }
        ]
      },
      {
        "kind": "diagnostic",
        "eyebrow": "RUNTIME TRIAGE",
        "title": "Runtime Ready 后的五个故障面",
        "intro": "Ready 排除一部分控制面问题；应用入口、配置、数据面连接、身份和验收合同仍要逐项检查。",
        "sourceIds": [
          "agentkit-runtime-quickstart-2026-08-15",
          "agentkit-memory-quickstart-2026-08-15",
          "agentkit-config-reference-2026-08-15"
        ...
        "items": [
          {
            "name": "应用入口错误",
            "en": "Entry Failure",
            "mechanism": "构建对象与实际启动对象不一致。",
            "decision": "在构建前执行相同入口。",
            "boundary": "修正路径不保证依赖完整。"
          },
          {
            "name": "环境与密钥缺失",
            "en": "Config Failure",
            "mechanism": "本地变量没有安全注入 Runtime。",
            "decision": "启动时只验证变量存在性与权限。",
            "boundary": "不得在日志打印秘密。"
          },
          {
            "name": "资源只绑定一层",
            "en": "Binding Failure",
            "mechanism": "控制面关联与数据面连接未同时满足。",
            "decision": "分别检查资源 ID、端点、凭据和网络。",
            "boundary": "能列出资源不等于能读写数据。"
          },
          {
            "name": "身份作用域错误",
            "en": "Identity Failure",
            "mechanism": "客户端声明被误当可信 user_id 或工作负载身份。",
            "decision": "由认证系统验证并绑定主体，应用再传播用户、租户与审计上下文。",
            "boundary": "Header 只承载身份上下文，可信度来自认证与签发链。"
          },
          {
            "name": "观测没有验收合同",
            "en": "Evidence Gap",
            "mechanism": "有日志和指标，却没有任务成功、SLO 和回退阈值。",
            "decision": "让 Trace、评测和压测结果都关联同一发布版本。",
            "boundary": "平均延迟不能代表尾部体验。"
          }
        ]
      }
    ],
    ...
    "cloudHooks": [
      {
        "stage": "应用构建与 Runtime",
        "services": "AgentKit SDK/CLI、构建、制品与 Runtime",
        "value": "把可运行 Agent 交付成版本化云端应用。",
        "discover": "入口、依赖、启动、版本和恢复目标是否明确？"
      },
      {
        "stage": "Memory 与 Knowledge",
        "services": "AgentKit Memory、知识库、共享 Session；另行评估的 Mem0 OSS 或 Platform",
        "value": "让长期状态独立于 Runtime 容器，并明确每种方案的运营责任。",
        "discover": "谁负责隔离、纠错、删除、备份和跨版本兼容？"
      },
      {
        "stage": "部署目标、网络与身份",
        "services": "目标配置、Region、网络、IAM、密钥与网关",
        "value": "把目标环境、网络路径和可信主体链纳入验收。",
        "discover": "资源访问、网络隔离和数据地域分别由什么保证？"
      },
      {
        "stage": "观测、评测与发布",
        "services": "日志、Trace、指标、评测、告警、版本与灰度",
        "value": "用证据决定放量、暂停、回退或退出。",
        "discover": "客户如何定义任务成功、P95、错误预算和恢复时间？"
      }
    ],
    "relatedSlugs": [
      "veadk",
    ...
    "qa": [
      {
        "q": "AgentKit 和 VeADK 的职责有什么差异？",
        "a": "VeADK 负责开发 Agent，AgentKit 负责把 Agent 应用配置、构建、部署并接入云端运行治理。",
        "depth": "框架定义、应用合同和 Runtime 生命周期可以组合，但必须分别验收和版本化。",
        "ask": "客户已有哪一层，真正缺的是开发、部署还是持续运营？",
        "tag": "产品定位",
        "basis": "官方平台与集成文档",
        "evidence": [
          {
      ...
      {
        "q": "AgentKit App 合同至少包含什么？",
        "a": "至少包含稳定入口、依赖、运行配置、调用接口、错误语义和需要的外部资源。",
        "depth": "本地与云端应使用同一入口和任务集；密钥通过安全配置注入，不能依赖开发机隐式环境。",
        "ask": "当前能否在全新环境中重建并调用同一 App？",
        "tag": "应用合同",
        "basis": "官方平台与配置参考",
        "evidence": [
          {
      ...
      {
        "q": "build、deploy 和 launch 有什么区别？",
        "a": "build 生成部署候选，deploy 执行部署，launch 将常用构建与部署步骤编排成快捷流程。",
        "depth": "具体命令语义以当前 CLI 为准；无论使用哪条路径，都要保留配置、制品、Runtime 版本、云端回归和回退证据。",
        "ask": "客户需要单步控制还是快捷发布，失败时怎样定位到构建或部署阶段？",
        "tag": "CLI 生命周期",
        "basis": "官方 CLI 命令参考",
        "evidence": [
          {
      ...
      {
        "q": "Runtime 显示 Ready，是否可以宣布业务上线？",
        "a": "不可以。Ready 只说明 Runtime 达到平台定义的运行状态，不证明模型、工具、Memory、身份、SLO 和业务后置条件通过。",
        "depth": "还需执行云端任务回归、故障与恢复、尾延迟、权限负例和人工接管，再由有权责任人决定放量。",
        "ask": "哪些云端证据会触发 Go、Hold 或回退？",
        "tag": "上线门禁",
        "basis": "Runtime 快速开始 + 生产验收边界",
        "evidence": [
          {
            "sourceId": "agentkit-runtime-quickstart-2026-08-15",
      ...
      {
        "q": "多实例 Runtime 怎样避免 Session 串话或丢失？",
        "a": "使用共享 Session 后端，并把用户范围绑定到认证系统已经验证的主体；客户端自报的 user_id 不能证明身份。",
        "depth": "还要验证并发写、重复请求、路由切换、重启、备份和删除传播，使应用、用户和会话作用域在所有实例保持一致。",
        "ask": "共享状态后端、身份来源、并发模型和 RPO/RTO 分别是什么？",
        "tag": "状态与身份",
        "basis": "AgentKit Runtime、共享 Session 与用户范围",
        "evidence": [
          {
            "sourceId": "agentkit-runtime-quickstart-2026-08-15",
      ...
      },
      {
        ...
        "tag": "Memory 架构",
        "basis": "AgentKit Memory + Mem0 官方对比",
        "evidence": [
          {
            "sourceId": "agentkit-memory-quickstart-2026-08-15",
    ...
    "evidenceCards": [
      {
        "metric": "build → deploy",
        "title": "launch 组合常规构建与部署流程",
        "finding": "官方 CLI 概览区分镜像构建、目标部署和组合执行的 launch。",
        "boundary": "命令选项可能改变阶段行为；命令成功也不证明制品来源、Runtime 健康或业务验收。",
        "sourceId": "agentkit-cli-overview-2026-08-15",
        "accent": true
      },
      {
        "metric": "外置状态",
        "title": "Memory 接入是资源与数据面合同",
        "finding": "官方文档把 Memory 作为独立资源，并提供应用接入路径。",
        "boundary": "资源绑定不证明连接、召回新鲜度、租户隔离、删除或权威事实状态；多实例 Session 仍是另一项设计。",
        "sourceId": "agentkit-memory-quickstart-2026-08-15"
      },
      {
        "metric": "目标 Runtime",
