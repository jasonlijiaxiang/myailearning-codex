import { moduleList } from "./knowledge-map.mjs";
import { moduleManifests } from "./modules/index.mjs";
import { moduleContentRegistry } from "./module-content-registry.mjs";
import { moduleCurriculumContent } from "./module-curriculum-content.mjs";
import { moduleLearningContent } from "./module-learning-content.mjs";

/**
 * 全站统一来源台账（Reference Ledger）。
 *
 * 模块正文只保存来源 ID；标题、链接、证据类别、边界与核验日期统一在此维护。
 * referenceModules 明确记录每个模块使用的来源，避免新增模块时把来源误归到 RAG。
 */

/** @type {Record<string, import("./content-types").Source>} */
export const sourceLedger = {
  "veadk-agent-source-2026-08-15": {
    grade: "P",
    kind: "官方源码",
    shortTitle: "VeADK Agent Source",
    title: "VeADK Agent implementation (pinned source)",
    note: "固定到指定提交，核验 VeADK Agent 对 Google ADK LlmAgent 的扩展入口及模型、工具、记忆、知识等配置；该源码切片不能证明未来版本行为、业务授权或生产安全。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/veadk-python/blob/3be05c48a0360b86003f6f67808c943b5a33c4d3/veadk/agent.py#L32-L72",
  },
  "veadk-runner-source-2026-08-15": {
    grade: "P",
    kind: "官方源码",
    shortTitle: "VeADK Runner Source",
    title: "VeADK Runner implementation (pinned source)",
    note: "固定到指定提交，核验 Runner 从 Agent 与会话服务建立执行入口并产出运行事件；事件流与调用完成不等于业务事务成功、持久化或可恢复。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/veadk-python/blob/3be05c48a0360b86003f6f67808c943b5a33c4d3/veadk/runner.py#L24-L329",
  },
  "veadk-agentkit-integration-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "VeADK AgentKit Integration",
    title: "Deploy VeADK agents with AgentKit (pinned documentation)",
    note: "固定到指定提交，核验通过 create_agentkit_app(root_agent) 将 VeADK 根 Agent 包装为 AgentKit 应用入口；适配器创建成功不代表云端部署、网络连通或生产就绪。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/veadk-python/blob/3be05c48a0360b86003f6f67808c943b5a33c4d3/docs/content/docs/framework/agentkit.mdx#L10-L84",
  },
  "veadk-short-term-memory-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "VeADK Short-term Memory",
    title: "VeADK short-term memory (pinned documentation)",
    note: "固定到指定提交，核验短期记忆与会话所需的 provider 配置入口；持久化、隔离、删除、恢复与容量边界取决于具体 provider，不能由此切片外推。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/veadk-python/blob/3be05c48a0360b86003f6f67808c943b5a33c4d3/docs/content/docs/framework/memory/short-term/index.mdx#L58-L66",
  },
  "veadk-builtin-tools-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "VeADK Built-in Tools",
    title: "VeADK built-in tools (pinned documentation)",
    note: "固定到指定提交，核验该版本文档列出的内置工具入口；实际可用性、凭证、网络访问、数据范围与业务授权仍须单独验证。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/veadk-python/blob/3be05c48a0360b86003f6f67808c943b5a33c4d3/docs/content/docs/framework/tools/builtin.mdx#L40-L49",
  },
  "agentkit-platform-overview-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentKit Platform Overview",
    title: "Volcengine AgentKit overview",
    note: "说明 AgentKit 面向 Agent 应用提供开发、运行时、工具、记忆、知识及监测评估等平台能力；产品范围、地区、配额与可用性可能变化，页面不构成客户生产验收。",
    verifiedAt: "2026-08-15",
    href: "https://volcengine.github.io/agentkit-sdk-python/en/content/1.introduction/1.overview.html",
  },
  "agentkit-cli-overview-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentKit CLI Overview",
    title: "AgentKit CLI overview (pinned documentation)",
    note: "固定到指定提交，核验 AgentKit CLI 所描述的应用构建、部署与调用流程；CLI 抽象不等于控制面完整语义，也不替代发布门禁。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/agentkit-sdk-python/blob/f39d363ea9a3fd6d1cd79f8dea14342ba846194a/docs/content/2.agentkit-cli/1.overview.md#L20-L135",
  },
  "agentkit-cli-commands-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentKit CLI Commands",
    title: "AgentKit CLI commands (pinned documentation)",
    note: "固定到指定提交，核验该版本 CLI 的命令与 launch 等快捷流程；当前安装版本可能不同，命令成功也不证明端到端业务成功或生产就绪。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/agentkit-sdk-python/blob/f39d363ea9a3fd6d1cd79f8dea14342ba846194a/docs/content/2.agentkit-cli/2.commands.md#L1090-L1504",
  },
  "agentkit-config-reference-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentKit Configuration",
    title: "AgentKit CLI configurations (pinned documentation)",
    note: "固定到指定提交，核验 AgentKit 应用与部署配置字段；示例值和默认值不是生产建议，身份、网络、资源与数据边界仍需按环境复核。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/agentkit-sdk-python/blob/f39d363ea9a3fd6d1cd79f8dea14342ba846194a/docs/content/2.agentkit-cli/3.configurations.md",
  },
  "agentkit-runtime-quickstart-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentKit Runtime Quickstart",
    title: "AgentKit Runtime quickstart (pinned documentation)",
    note: "固定到指定提交，核验托管 Runtime 的部署与调用入口；完成快速入门不能证明容量、韧性、观测、安全、成本或业务验收已经满足。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/agentkit-sdk-python/blob/f39d363ea9a3fd6d1cd79f8dea14342ba846194a/docs/content/4.runtime/1.runtime_quickstart.md",
  },
  "agentkit-memory-quickstart-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentKit Memory Quickstart",
    title: "AgentKit Memory quickstart (pinned documentation)",
    note: "固定到指定提交，核验独立记忆资源的创建与 Agent 应用绑定入口；它不等同于 Session、Knowledge 或权威业务记录，也不保证删除、驻留和恢复要求。",
    verifiedAt: "2026-08-15",
    href: "https://github.com/volcengine/agentkit-sdk-python/blob/f39d363ea9a3fd6d1cd79f8dea14342ba846194a/docs/content/6.memory/1.memory_quickstart.md",
  },
  "mem0-oss-overview-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Mem0 OSS Overview",
    title: "Mem0 open-source overview",
    note: "说明 Mem0 开源记忆层的基本接口与自托管定位；部署、升级、数据保护、隔离、可用性与运营责任仍由采用方承担。",
    verifiedAt: "2026-08-15",
    href: "https://docs.mem0.ai/open-source/overview",
  },
  "mem0-platform-vs-oss-2026-08-15": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Mem0 Platform vs OSS",
    title: "Mem0 Platform versus open source",
    note: "区分 Mem0 托管平台与开源版本的责任和能力边界；该比较不能证明 AgentKit 内部实现、托管关系、地区、数据驻留或 SLA。",
    verifiedAt: "2026-08-15",
    href: "https://docs.mem0.ai/platform/platform-vs-oss",
  },
  "google-genai-app-lifecycle": {
    grade: "P",
    kind: "指南",
    shortTitle: "Google GenAI Lifecycle",
    title: "Deploy and operate generative AI applications",
    note: "说明生成式 AI 应用从发现、实验、部署、持续监控到持续改进的生命周期，并把 Prompt 模板、Chain、Embedding 模型、检索存储和 Adapter 视为部署制品；该厂商架构不能替代客户自己的发布清单与验收。",
    verifiedAt: "2026-07-21",
    href: "https://docs.cloud.google.com/architecture/deploy-operate-generative-ai-applications",
  },
  "azure-foundation-model-lifecycle": {
    grade: "P",
    kind: "指南",
    shortTitle: "Azure Foundation Model Lifecycle",
    title: "Design to support foundation model life cycles",
    note: "说明模型变化可能联动 Prompt、编排、Grounding 数据与硬件，并要求将特定 Prompt、配置和模型版本正确配套，通过 GenAIOps、MLOps 与 DataOps 流水线验证；Azure 示例不是跨云统一实现。",
    verifiedAt: "2026-07-21",
    href: "https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/manage-foundation-models-lifecycle",
  },
  "aws-genai-lifecycle": {
    grade: "P",
    kind: "指南",
    shortTitle: "AWS GenAI Lifecycle",
    title: "Generative AI lifecycle — Generative AI Lens",
    note: "把范围界定、模型选择、模型定制、开发集成、部署和持续改进放进统一生命周期，并要求持续评估质量、伦理、成本与生产表现；Well-Architected 指南不是产品认证。",
    verifiedAt: "2026-07-21",
    href: "https://docs.aws.amazon.com/wellarchitected/latest/generative-ai-lens/generative-ai-lifecycle.html",
  },
  "azure-genaiops": {
    grade: "P",
    kind: "指南",
    shortTitle: "Azure GenAIOps",
    title: "MLOps and GenAIOps for AI workloads on Azure",
    note: "按应用开发、数据处理与模型管理划分 AI 工作负载，并组合 DevOps、DataOps、MLOps 与 GenAIOps；方法分类帮助划责，但不能替代具体资产、风险和验收契约。",
    verifiedAt: "2026-07-21",
    href: "https://learn.microsoft.com/en-us/azure/well-architected/ai/mlops-genaiops",
  },
  "finops-framework": {
    grade: "O",
    kind: "行业指南",
    shortTitle: "FinOps Framework",
    title: "FinOps Framework Overview",
    note: "定义以工程、财务和业务协作最大化技术价值的运营框架，并把 Unit Economics、架构与工作负载放置、使用与费率优化纳入能力；框架不提供某个组织的成本基线或 ROI。",
    verifiedAt: "2026-07-21",
    href: "https://www.finops.org/framework/",
  },
  "finops-ai-category": {
    grade: "O",
    kind: "行业指南",
    shortTitle: "FinOps for AI",
    title: "FinOps for AI — FinOps Framework Technology Category",
    note: "说明 AI 支出跨模型服务、云、数据中心、SaaS 与多类供应商，并讨论分配、预测、单位经济、异常和角色协作；行业共识不能替代客户实际账单与业务结果。",
    verifiedAt: "2026-07-21",
    href: "https://www.finops.org/framework/technology-categories/ai/",
  },
  "finops-scopes": {
    grade: "O",
    kind: "行业指南",
    shortTitle: "FinOps Scopes",
    title: "FinOps Scopes",
    note: "要求 Scope 由明确业务结果、约束或决策情境驱动，并保持少而有目的；AI 可以形成独立视角，但不是所有 AI 支出都必须另建范围。",
    verifiedAt: "2026-07-21",
    href: "https://www.finops.org/framework/scopes/",
  },
  "finops-ai-overview": {
    grade: "O",
    kind: "行业指南",
    shortTitle: "FinOps for AI Overview",
    title: "FinOps for AI Overview",
    note: "讨论 AI 成本、GPU、Token、业务影响、持续优化与 Crawl–Walk–Run 成熟路径；工作组指导可用于构建方法，不能证明具体项目已产生价值。",
    verifiedAt: "2026-07-29",
    href: "https://www.finops.org/wg/finops-for-ai-overview/",
  },
  "finops-unit-economics": {
    grade: "O",
    kind: "行业指南",
    shortTitle: "FinOps Unit Economics",
    title: "Capability: Unit Economics",
    note: "把技术支出与产品、服务或活动创造的价值联系起来，并区分 cost per token 等资源效率单位与 cost per case resolved 等业务结果单位；单位经济用于建立趋势和决策基线，不能单独证明 ROI、因果关系或客户价值。",
    verifiedAt: "2026-07-29",
    href: "https://www.finops.org/framework/capabilities/unit-economics/",
  },
  "finops-ai-tools-considerations": {
    grade: "O",
    kind: "行业指南",
    shortTitle: "FinOps AI Tools Considerations",
    title: "FinOps for AI: Tools & Services Considerations",
    note: "要求按用例结果单位衡量 AI 工具和服务的全部成本，并持续复核可见性、责任、优化节奏与退出条件；其中示例金额与工具清单不是通用成本基准或采购推荐。",
    verifiedAt: "2026-07-29",
    href: "https://www.finops.org/wg/finops-for-ai-tools-services-considerations/",
  },
  "google-mlops-predictive-ai": {
    grade: "P",
    kind: "指南",
    shortTitle: "Google Cloud MLOps",
    title: "MLOps: Continuous delivery and automation pipelines in machine learning",
    note: "说明预测式 AI 的 CI、CD、持续训练、数据与模型验证、Feature Store 和元数据管理；该页面明确主要适用于 predictive AI，不能直接当作生成式 AI 应用工程的完整规范。",
    verifiedAt: "2026-08-01",
    href: "https://docs.cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning",
  },
  "google-rules-of-ml": {
    grade: "O",
    kind: "指南",
    shortTitle: "Rules of Machine Learning",
    title: "Rules of Machine Learning: Best Practices for ML Engineering",
    note: "从生产机器学习经验总结先建立基线与端到端管线、保持训练与服务一致、监控业务与系统指标以及识别反馈回路等实践；这些规则是工程判断框架，不提供客户场景的统一阈值、因果 ROI 或合规结论。",
    verifiedAt: "2026-08-01",
    href: "https://developers.google.com/machine-learning/guides/rules-of-ml",
  },
  "ml-test-score-2017": {
    grade: "A",
    kind: "论文",
    shortTitle: "The ML Test Score",
    title: "The ML Test Score: A Rubric for ML Production Readiness and Technical Debt Reduction",
    note: "提出覆盖特征与数据、模型开发、基础设施和监控的 28 项生产就绪测试，支持说明离线分数不是完整上线证据；量表来自作者的生产经验，不能替代客户风险门槛或具体产品验收。",
    verifiedAt: "2026-08-01",
    href: "https://research.google/pubs/the-ml-test-score-a-rubric-for-ml-production-readiness-and-technical-debt-reduction/",
  },
  "azure-ml-model-monitoring": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Azure ML Model Monitoring",
    title: "Model monitoring in production — Azure Machine Learning",
    note: "区分数据质量、输入数据漂移、预测漂移、特征归因漂移与基于真值的模型性能信号；部分信号仍为 Preview，产品支持范围和可用性必须在设计时复核，任何告警都不能自动证明概念漂移或授权发布。",
    verifiedAt: "2026-08-01",
    href: "https://learn.microsoft.com/en-us/azure/machine-learning/concept-model-monitoring?view=azureml-api-2",
  },
  "aws-sagemaker-deployment-guardrails": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "SageMaker Deployment Guardrails",
    title: "Deployment guardrails for updating models in production — Amazon SageMaker AI",
    note: "说明蓝绿、Canary、Linear 流量迁移与基于告警的自动回滚能力；当前文档限定到实时和异步推理端点，且端点流量回滚不等于特征、阈值、业务策略或已发生业务动作自动恢复。",
    verifiedAt: "2026-08-01",
    href: "https://docs.aws.amazon.com/sagemaker/latest/dg/deployment-guardrails.html",
  },
  "aws-sagemaker-feature-store": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "SageMaker Feature Store",
    title: "Create, store, and share features with Feature Store — Amazon SageMaker AI",
    note: "区分低延迟在线最新特征与供训练、探索和批量预测使用的离线历史，并说明训练—服务偏差、元数据和删除行为；产品能力不自动保证时间正确性或数据质量。",
    verifiedAt: "2026-08-01",
    href: "https://docs.aws.amazon.com/sagemaker/latest/dg/feature-store.html",
  },
  "aws-sagemaker-model-registry": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "SageMaker Model Registry",
    title: "Model Registration Deployment with Model Registry — Amazon SageMaker AI",
    note: "列出模型生产目录、版本、指标、血缘、阶段、审批、部署和 CI/CD 能力；这些产品对象不能替代组织自己的风险分级与批准责任。",
    verifiedAt: "2026-08-01",
    href: "https://docs.aws.amazon.com/sagemaker/latest/dg/model-registry.html",
  },
  "azure-ml-registries": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Azure ML Registries",
    title: "Machine Learning registries — Azure Machine Learning",
    note: "说明在工作区之外集中存储并共享模型、环境、组件和数据资产，以及从开发向测试和生产晋升；身份、网络、区域和数据控制仍需单独验证。",
    verifiedAt: "2026-08-01",
    href: "https://learn.microsoft.com/en-us/azure/machine-learning/concept-machine-learning-registries-mlops?view=azureml-api-2",
  },
  "nist-ai-rmf": {
    grade: "O",
    kind: "指南",
    shortTitle: "NIST AI RMF",
    title: "NIST Artificial Intelligence Risk Management Framework",
    note: "NIST 当前页面说明 AI RMF 供自愿采用、用于在 AI 产品与系统生命周期中纳入可信风险，并明确 AI RMF 1.0 正在修订；它不是产品认证或法律合规结论。",
    verifiedAt: "2026-07-30",
    href: "https://www.nist.gov/itl/ai-risk-management-framework",
  },
  "iso-iec-42001": {
    grade: "O",
    kind: "标准",
    shortTitle: "ISO/IEC 42001:2023",
    title: "ISO/IEC 42001:2023 — Artificial intelligence management system",
    note: "ISO 官方页面说明该标准规定组织建立、实施、维护并持续改进 AI 管理体系的要求；管理体系标准不直接证明单个模型输出正确、安全或满足所有法律。",
    verifiedAt: "2026-07-30",
    href: "https://www.iso.org/standard/42001",
  },
  "iso-iec-42005": {
    grade: "O",
    kind: "标准",
    shortTitle: "ISO/IEC 42005:2025",
    title: "ISO/IEC 42005:2025 — AI system impact assessment",
    note: "ISO 官方页面说明该标准为 AI 系统影响评估提供全生命周期指导，覆盖对个人、群体与社会的潜在影响，并可与 AI 管理体系配合；它不直接决定客户系统的法律分类、风险层级或可接受残余风险。",
    verifiedAt: "2026-07-30",
    href: "https://www.iso.org/standard/42005",
  },
  "bm25-book": {
    grade: "A",
    kind: "实验与教材",
    shortTitle: "BM25 教材",
    title: "Introduction to Information Retrieval — Okapi BM25",
    note: "稀疏检索的概率排序、词频、逆文档频率与长度归一化基础。",
    verifiedAt: "2026-07-17",
    href: "https://nlp.stanford.edu/IR-book/html/htmledition/okapi-bm25-a-non-binary-model-1.html",
  },
  "dpr-2020": {
    grade: "A",
    kind: "论文",
    shortTitle: "DPR",
    title: "Dense Passage Retrieval for Open-Domain Question Answering",
    note: "双编码器稠密检索；论文特定开放域问答数据集上，top-20 passage retrieval accuracy 较其 Lucene-BM25 基线高 9–19 个百分点，不能外推为稠密检索普遍更优。",
    verifiedAt: "2026-07-17",
    href: "https://aclanthology.org/2020.emnlp-main.550/",
  },
  "rrf-2009": {
    grade: "A",
    kind: "论文",
    shortTitle: "Reciprocal Rank Fusion",
    title: "Reciprocal Rank Fusion Outperforms Condorcet and Individual Rank Learning Methods",
    note: "提出以各排序器中的名次倒数融合结果，不要求不同检索分数处于同一量纲；原论文的比较结果只适用于其测试集合与候选排序，不能证明 RRF 在所有语料、语言或检索器组合上都更优。",
    verifiedAt: "2026-07-29",
    href: "https://research.google/pubs/reciprocal-rank-fusion-outperforms-condorcet-and-individual-rank-learning-methods/",
  },
  "beir-2021": {
    grade: "A",
    kind: "论文",
    shortTitle: "BEIR",
    title: "BEIR: A Heterogeneous Benchmark for Zero-shot Evaluation of Information Retrieval Models",
    note: "在 18 个数据集和多类检索架构上比较零样本泛化，论文显示 BM25 仍是强基线且效果与计算成本存在权衡；它支持扩大候选模型评测范围，不能替代客户语料、语言、权限过滤与端到端任务验收。",
    verifiedAt: "2026-07-29",
    href: "https://datasets-benchmarks-proceedings.neurips.cc/paper/2021/hash/65b9eea6e1cc6bb9f0cd2a47751a186f-Abstract-round2.html",
  },
  "mteb-2023": {
    grade: "A",
    kind: "论文",
    shortTitle: "MTEB",
    title: "MTEB: Massive Text Embedding Benchmark",
    note: "原始论文覆盖 8 类任务、58 个数据集和 112 种语言，并发现没有一种 Embedding 方法支配所有任务；这些数字描述 2023 年论文版本，不代表当前排行榜范围，也不能替代目标语料上的检索评测。",
    verifiedAt: "2026-07-29",
    href: "https://aclanthology.org/2023.eacl-main.148/",
  },
  "miracl-2023": {
    grade: "A",
    kind: "论文",
    shortTitle: "MIRACL",
    title: "MIRACL: A Multilingual Retrieval Dataset Covering 18 Diverse Languages",
    note: "提供 18 种语言的 ad hoc retrieval 数据与母语标注，可用于检查多语言检索表现；论文明确 query 与 corpus 使用同一种语言，因此不能直接证明跨语言检索能力或企业领域效果。",
    verifiedAt: "2026-07-29",
    href: "https://aclanthology.org/2023.tacl-1.63/",
  },
  "clirmatrix-2020": {
    grade: "A",
    kind: "论文",
    shortTitle: "CLIRMatrix",
    title: "CLIRMatrix: A Massively Large Collection of Bilingual and Multilingual Datasets for Cross-Lingual Information Retrieval",
    note: "提供查询语言与相关文档语言不同的双语数据，以及混合语言检索数据，可用于跨语言检索候选路线的离线比较；数据由 Wikipedia 自动挖掘，规模不能替代目标领域的人工相关性判断。",
    verifiedAt: "2026-07-29",
    href: "https://aclanthology.org/2020.emnlp-main.340/",
  },
  "hnsw-2016": {
    grade: "A",
    kind: "论文",
    shortTitle: "HNSW",
    title: "Efficient and Robust Approximate Nearest Neighbor Search Using Hierarchical Navigable Small World Graphs",
    note: "解释常见向量索引 HNSW 的分层近邻图与近似最近邻搜索机制。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/1603.09320",
  },
  "rag-survey": {
    grade: "A",
    kind: "研究报告",
    shortTitle: "RAG 综述",
    title: "Retrieval-Augmented Generation for Large Language Models: A Survey",
    note: "系统整理 Naive、Advanced、Modular RAG，以及检索、增强、生成与评估环节。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2312.10997",
  },
  graphrag: {
    grade: "A",
    kind: "论文",
    shortTitle: "GraphRAG",
    title: "From Local to Global: A Graph RAG Approach to Query-Focused Summarization",
    note: "用实体图、社区和社区摘要处理跨整个语料的全局归纳问题。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2404.16130",
  },
  "chunking-study": {
    grade: "A",
    kind: "论文",
    shortTitle: "代码 RAG 切块实验",
    title: "How Does Chunking Affect Retrieval-Augmented Code Completion?",
    note: "代码补全任务中的切块控制实验；只能说明切块需要按任务实测，不能直接代表合同、PDF 或扫描件。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2605.04763",
  },
  "rag-original-2020": {
    grade: "A",
    kind: "论文",
    shortTitle: "原始 RAG 论文",
    title: "Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks",
    note: "原始 RAG 定义、检索与生成的协同方式、外部知识增强机制，以及查询编码器与生成器的联合优化范围。",
    verifiedAt: "2026-07-17",
    href: "https://proceedings.neurips.cc/paper/2020/hash/6b493230205f780e1bc26945df7481e5-Abstract.html",
  },
  "bert-reranker": {
    grade: "A",
    kind: "论文",
    shortTitle: "BERT Reranker",
    title: "Passage Re-ranking with BERT",
    note: "查询—段落联合重排的经典二阶段方法；精排只能重新排序已召回候选，不能找回候选集外的证据。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/1901.04085",
  },
  "fid-2021": {
    grade: "A",
    kind: "论文",
    shortTitle: "Fusion-in-Decoder",
    title: "Leveraging Passage Retrieval with Generative Models for Open Domain Question Answering",
    note: "说明多段证据可被独立编码并在解码器聚合；更多 passage 带来的收益只适用于论文实验，不能外推为上下文越多越好。",
    verifiedAt: "2026-07-17",
    href: "https://aclanthology.org/2021.eacl-main.74/",
  },
  "replug-2024": {
    grade: "A",
    kind: "论文",
    shortTitle: "REPLUG",
    title: "REPLUG: Retrieval-Augmented Black-Box Language Models",
    note: "验证冻结黑盒语言模型、在模型外训练可调检索器，并把检索文档前置到模型输入的模块化路线。",
    verifiedAt: "2026-07-17",
    href: "https://aclanthology.org/2024.naacl-long.463/",
  },
  "self-rag": {
    grade: "A",
    kind: "论文",
    shortTitle: "Self-RAG",
    title: "Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection",
    note: "按需决定是否检索，并通过反思 token 评价检索证据与自身生成；不表示普通 RAG 自动拥有自检能力。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2310.11511",
  },
  "lost-middle": {
    grade: "A",
    kind: "论文",
    shortTitle: "Lost in the Middle",
    title: "Lost in the Middle: How Language Models Use Long Contexts",
    note: "在论文的多文档问答与键值检索实验中，相关信息位置显著影响表现；不能据此断言所有模型都相同或 RAG 必然优于长上下文。",
    verifiedAt: "2026-07-29",
    href: "https://aclanthology.org/2024.tacl-1.9/",
  },
  "contextual-retrieval": {
    grade: "B",
    kind: "实验与教材",
    shortTitle: "Contextual Retrieval",
    title: "Contextual Retrieval",
    note: "特定实验中，混合检索将 top-20 failure rate 从 5.7% 降至 2.9%，加重排后为 1.9%；只适合作为客户 PoC 的验证假设。",
    verifiedAt: "2026-07-17",
    href: "https://www.anthropic.com/engineering/contextual-retrieval",
  },
  ragas: {
    grade: "A",
    kind: "论文",
    shortTitle: "RAGAS",
    title: "RAGAS: Automated Evaluation of Retrieval Augmented Generation",
    note: "提出 Faithfulness、Answer Relevance、Context Relevance 三个无参考答案自动评估维度；不直接给出业务、安全、P95 或成本门槛。",
    verifiedAt: "2026-07-30",
    href: "https://aclanthology.org/2024.eacl-demo.16/",
  },
  "nist-genai-profile": {
    grade: "O",
    kind: "指南",
    shortTitle: "NIST AI 600-1",
    title: "Artificial Intelligence Risk Management Framework: Generative Artificial Intelligence Profile",
    note: "以 Govern / Map / Measure / Manage 组织生成式 AI 风险治理；它是自愿采用的风险框架，不是产品认证或 RAG 性能标准。",
    verifiedAt: "2026-07-30",
    href: "https://doi.org/10.6028/NIST.AI.600-1",
  },
  "nist-ai-800-3": {
    grade: "O",
    kind: "研究报告",
    shortTitle: "NIST AI 800-3",
    title: "Expanding the AI Evaluation Toolbox with Statistical Models",
    note: "区分固定题集上的 Benchmark Accuracy 与对相似题目总体外推的 Generalized Accuracy，并说明多次试验、统计模型、假设与不确定性报告如何影响结论；它不提供客户场景的通用样本量、通过线或发布权限。",
    verifiedAt: "2026-07-30",
    href: "https://doi.org/10.6028/NIST.AI.800-3",
  },
  "nist-ai-800-4": {
    grade: "O",
    kind: "研究报告",
    shortTitle: "NIST AI 800-4",
    title: "Challenges to the Monitoring of Deployed AI Systems",
    note: "区分功能、运营、人因、安全、合规与大规模影响等部署后监测类别，并汇总当前方法、术语与组织实践仍不成熟的挑战；它不是完整监控标准，也不定义客户告警阈值、采样率或事故响应流程。",
    verifiedAt: "2026-07-30",
    href: "https://doi.org/10.6028/NIST.AI.800-4",
  },
  "nist-aml-100-2e2025": {
    grade: "O",
    kind: "研究报告",
    shortTitle: "NIST AML Taxonomy",
    title: "NIST AI 100-2 E2025 — Adversarial Machine Learning: A Taxonomy and Terminology of Attacks and Mitigations",
    note: "为规避、投毒、隐私与滥用等对抗性机器学习攻击提供统一术语和生命周期视角；NIST 当前仍列其为 Final，同时挂有潜在更新说明。该分类不是认证，也不证明列出的缓解措施在具体系统中有效。",
    verifiedAt: "2026-07-29",
    href: "https://csrc.nist.gov/pubs/ai/100/2/e2025/final",
  },
  "nist-zero-trust": {
    grade: "O",
    kind: "研究报告",
    shortTitle: "NIST SP 800-207",
    title: "Zero Trust Architecture",
    note: "访问企业资源前验证主体并执行授权，不因网络位置或资产归属给予隐式信任；RAG 中的检索过滤是该原则的工程应用。",
    verifiedAt: "2026-07-17",
    href: "https://csrc.nist.gov/pubs/sp/800/207/final",
  },
  "nist-sp-800-61r3": {
    grade: "O",
    kind: "安全治理",
    shortTitle: "NIST SP 800-61 Rev. 3",
    title: "Incident Response Recommendations and Considerations for Cybersecurity Risk Management",
    note: "NIST 于 2025 年 4 月发布 Rev. 3，把事件响应纳入 CSF 2.0 风险管理，并覆盖准备、检测、响应、恢复与持续改进；它支持安全事件运行机制，但不规定 AI 应用的统一告警阈值、取证字段或恢复时限。",
    verifiedAt: "2026-07-30",
    href: "https://csrc.nist.gov/pubs/sp/800/61/r3/final",
  },
  "nist-sp-800-218a": {
    grade: "O",
    kind: "安全治理",
    shortTitle: "NIST SP 800-218A",
    title: "Secure Software Development Practices for Generative AI and Dual-Use Foundation Models",
    note: "NIST 的 SSDF Community Profile 面向 AI 模型生产者、使用模型的 AI 系统生产者和采购方，补充生成式 AI 与双用途基础模型在软件生命周期中的安全开发和采购实践；它不证明某个解析器、模型、数据集或连接器已经通过客户准入。",
    verifiedAt: "2026-07-30",
    href: "https://csrc.nist.gov/pubs/sp/800/218/a/final",
  },
  "owasp-prompt-injection": {
    grade: "G",
    kind: "安全治理",
    shortTitle: "OWASP LLM01",
    title: "OWASP LLM01:2025 Prompt Injection",
    note: "将提示注入（Prompt Injection）列为生成式 AI 应用的重要风险，并明确 RAG、微调或系统提示都不能单独消除该风险。",
    verifiedAt: "2026-07-30",
    href: "https://genai.owasp.org/llmrisk/llm01-prompt-injection/",
  },
  "owasp-llm-top-ten": {
    grade: "G",
    kind: "安全治理",
    shortTitle: "OWASP LLM Top 10",
    title: "OWASP Top 10 for LLM Applications 2025",
    note: "汇总提示注入、敏感信息泄露、供应链、数据与模型投毒、输出处理、过度授权及向量弱点等常见风险；适合检查威胁模型是否漏项，不是合规认证。",
    verifiedAt: "2026-07-30",
    href: "https://genai.owasp.org/llm-top-10/",
  },
  "owasp-vector-weaknesses": {
    grade: "G",
    kind: "安全治理",
    shortTitle: "OWASP LLM08",
    title: "OWASP LLM08:2025 Vector and Embedding Weaknesses",
    note: "覆盖多租户向量库的跨上下文泄漏、细粒度访问控制、逻辑隔离、来源校验与检索日志。",
    verifiedAt: "2026-07-30",
    href: "https://genai.owasp.org/llmrisk/llm082025-vector-and-embedding-weaknesses/",
  },
  "china-ai-content-labeling": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "生成合成内容标识办法",
    title: "人工智能生成合成内容标识办法",
    note: "规定显式与隐式标识及服务提供者、传播平台等主体的相关义务，自 2025 年 9 月 1 日起施行；具体适用性仍需按产品、内容与角色判断。",
    verifiedAt: "2026-07-21",
    href: "https://www.nrta.gov.cn/art/2025/3/14/art_113_70340.html?xxgkhide=1",
  },
  "china-ai-content-labeling-2026-08-05": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "生成合成内容标识办法（2026-08-05 核验）",
    title: "人工智能生成合成内容标识办法",
    note: "规定生成合成内容的显式与隐式标识、服务提供者与传播平台的核验义务及用户声明要求，自 2025 年 9 月 1 日起施行；具体适用性需按主体、内容与发布情形判断。",
    verifiedAt: "2026-08-05",
    href: "https://www.nrta.gov.cn/art/2025/3/14/art_113_70340.html?xxgkhide=1",
    versionOf: "china-ai-content-labeling",
    localeScope: "zh-CN",
  },
  "china-ai-service-management": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "生成式AI服务管理暂行办法",
    title: "生成式人工智能服务管理暂行办法",
    note: "七部门令第 15 号，自 2023 年 8 月 15 日起施行；第二条明确其适用于向境内公众提供生成式人工智能服务的场景，第七条、第十二条、第十四条、第十五条与第十七条分别涉及训练数据、内容标识、违法内容处置、投诉举报及安全评估与算法备案；是否适用需按服务对象、主体角色与部署方式判断。",
    verifiedAt: "2026-08-05",
    href: "https://www.gov.cn/zhengce/zhengceku/202307/content_6891752.htm",
  },
  "china-data-cross-border-2024": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "促进和规范数据跨境流动规定",
    title: "促进和规范数据跨境流动规定",
    note: "国家互联网信息办公室令第 16 号，自公布之日（2024 年 3 月 22 日）起施行；规定数据出境安全评估、个人信息出境标准合同与个人信息保护认证的申报门槛和免申报情形，第十条保留告知、单独同意与个人信息保护影响评估等义务；具体通道与量级判断需按数据分类和当期监管口径核验。",
    verifiedAt: "2026-08-05",
    href: "https://www.gov.cn/gongbao/2024/issue_11366/202405/content_6954192.html",
  },
  "china-personal-information-protection-law": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "个人信息保护法",
    title: "中华人民共和国个人信息保护法",
    note: "第四条明确个人信息不包括匿名化处理后的信息，第七十三条将匿名化定义为无法识别特定自然人且不能复原；这为重新判断个人信息属性提供法定边界，但不能证明向量化、切块、Embedding 或缓存本身已经达到匿名化，也不自动排除重要数据等其他分类。",
    verifiedAt: "2026-08-08",
    href: "https://www.cac.gov.cn/2021-08/20/c_1631050028355286.htm",
  },
  "c2pa-2-4": {
    grade: "O",
    kind: "标准",
    shortTitle: "C2PA 2.4",
    title: "C2PA Technical Specification 2.4",
    note: "定义签名清单（Manifest）、内容绑定、编辑与来源声明及验证状态；它能验证声明与资产的关联和防篡改，不证明内容真实、版权成立、身份正确或已经合规，凭证也可能被移除。",
    verifiedAt: "2026-07-30",
    href: "https://spec.c2pa.org/specifications/specifications/2.4/specs/C2PA_Specification.html",
  },
  "gb-45438-2025": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "GB 45438-2025",
    title: "网络安全技术 人工智能生成合成内容标识方法",
    note: "国家标准全文公开系统显示该标准于 2025 年 2 月 28 日发布、2025 年 9 月 1 日实施；工程落地仍要核对正式文本与适用范围。",
    verifiedAt: "2026-07-21",
    href: "https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=F32EA2A561F1886CD8D606513512D547",
  },
  "eu-ai-act": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "EU AI Act",
    title: "European AI Act — Regulatory Framework",
    note: "欧盟委员会页面当前把招聘和候选人筛选列为高风险用途示例，并列出 2026 年 8 月的总体适用节点及经 2026 年正式修法调整的部分高风险规则时点；页面是动态实施入口，不能替代正式法律文本或针对具体系统的法律意见。",
    verifiedAt: "2026-07-30",
    href: "https://digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai",
  },
  "eu-ai-act-implementation-2026-08-05": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "EU AI Act（2026-08-05 核验）",
    title: "European AI Act — Regulatory Framework",
    note: "欧盟委员会页面把招聘和候选人筛选列为高风险用途示例，并确认 AI Act 自 2026-08-02 起适用、经 2026 年正式修法调整的部分高风险规则时点（Annex III 自 2027-12-02、Annex I 自 2028-08-02）；页面是动态实施入口，不能替代正式法律文本或针对具体系统的法律意见。",
    verifiedAt: "2026-08-05",
    href: "https://digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai",
    versionOf: "eu-ai-act",
    localeScope: "zh-CN",
  },
  "eu-ai-act-2026-1744": {
    grade: "O",
    kind: "法规标准",
    shortTitle: "Regulation (EU) 2026/1744",
    title: "Regulation (EU) 2026/1744 amending Regulations (EU) 2024/1689, (EU) 2018/1139 and (EU) 2023/1230 as regards the simplification of the implementation of harmonised rules on artificial intelligence (Digital Omnibus on AI)",
    note: "EUR-Lex 正式文本显示该修订法规于 2026 年 7 月 27 日生效：Chapter III Sections 1–3（Article 6(5) 除外）对按 Article 6(2)/Annex III 分类的系统自 2027 年 12 月 2 日适用，对按 Article 6(1)/Annex I 分类的系统自 2028 年 8 月 2 日适用。修法并未推迟这些系统的全部义务；具体角色、分类与义务仍需专业法律判断。",
    verifiedAt: "2026-07-30",
    href: "https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32026R1744",
  },
  "docling-report": {
    grade: "A",
    kind: "研究报告",
    shortTitle: "Docling",
    title: "Docling Technical Report",
    note: "覆盖 PDF 转换、版面分析与表格结构识别；证明解析工具的结构化能力，不直接证明采用后企业 RAG 准确率必然提升。",
    verifiedAt: "2026-07-30",
    href: "https://research.ibm.com/publications/docling-technical-report",
  },
  "w3c-prov-o": {
    grade: "O",
    kind: "标准",
    shortTitle: "W3C PROV-O",
    title: "PROV-O: The PROV Ontology",
    note: "用 Entity、Activity、Agent 及生成、派生、修订、主来源和失效等关系表达跨系统 Provenance；它提供可互换的血缘语义，不会自动采集完整血缘、裁决业务权威性或证明删除已传播。",
    verifiedAt: "2026-07-29",
    href: "https://www.w3.org/TR/prov-o/",
  },
  "openlineage-spec": {
    grade: "O",
    kind: "可观测规范",
    shortTitle: "OpenLineage",
    title: "OpenLineage — Facets & Extensibility",
    note: "以 Run、Job、输入与输出 Dataset 及可扩展 Facet 表达运行时血缘事件和元数据；它能统一事件语义，但不会自动证明采集完整、业务内容正确、权限有效或删除传播完成。",
    verifiedAt: "2026-07-29",
    href: "https://openlineage.io/docs/spec/facets/",
  },
  "iso-iec-5259-2": {
    grade: "O",
    kind: "标准",
    shortTitle: "ISO/IEC 5259-2",
    title: "ISO/IEC 5259-2:2024 — Data quality measures",
    note: "为分析与机器学习数据定义质量模型和可度量特性，可用于组织数据质量评估与报告；它不提供通用 RAG 合格阈值，也不替代客户按用途、风险和材料建立验收样本。",
    verifiedAt: "2026-07-29",
    href: "https://www.iso.org/standard/81860.html",
  },
  "pp-ocr-2020": {
    grade: "A",
    kind: "论文",
    shortTitle: "PP-OCR",
    title: "PP-OCR: A Practical Ultra Lightweight OCR System",
    note: "给出文本检测、方向分类和文本识别组成的实用 OCR 系统；证明扫描图像文字识别的技术路线，不保证任意扫描质量或版式都能可靠恢复。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2009.09941",
  },
  "pp-ocrv5-2026": {
    grade: "A",
    kind: "论文",
    shortTitle: "PP-OCRv5",
    title: "PP-OCRv5: A Specialized 5M-Parameter Model Rivaling Billion-Parameter Vision-Language Models on OCR Tasks",
    note: "在论文测试的 OCR 基准上，小型专用模型展示了与部分十亿参数级视觉语言模型相当的识别表现，并强调定位精度、幻觉与计算成本差异；结果不能外推为专用 OCR 在任意文档、语言和任务上都优于 VLM。",
    verifiedAt: "2026-07-30",
    href: "https://openaccess.thecvf.com/content/CVPR2026/html/Cui_PP-OCRv5_A_Specialized_5M-Parameter_Model_Rivaling_Billion-Parameter_Vision-Language_Models_on_CVPR_2026_paper.html",
  },
  "colpali-2025": {
    grade: "A",
    kind: "论文",
    shortTitle: "ColPali",
    title: "ColPali: Efficient Document Retrieval with Vision Language Models",
    note: "用视觉语言模型直接为文档页图像生成多向量表示，面向视觉丰富文档检索；论文结果不能直接外推到客户图片、语言和算力条件。",
    verifiedAt: "2026-07-17",
    href: "https://proceedings.iclr.cc/paper_files/paper/2025/hash/99e9e141aafc314f76b0ca3dd66898b3-Abstract-Conference.html",
  },
  "fine-tuning-or-retrieval": {
    grade: "A",
    kind: "论文",
    shortTitle: "Fine-Tuning or Retrieval?",
    title: "Fine-Tuning or Retrieval? Comparing Knowledge Injection in LLMs",
    note: "在论文设定下，RAG 对新事实注入优于所比较的无监督微调；不能外推为所有微调都不适合知识更新。",
    verifiedAt: "2026-07-17",
    href: "https://aclanthology.org/2024.emnlp-main.15/",
  },
  "hf-trl-data-formats": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "TRL 数据格式",
    title: "Hugging Face TRL — Dataset Formats and Types",
    note: "说明文本、对话、提示—补全与偏好数据的标准和 conversational 表示；用于核对训练器输入结构，不代表特定数据集质量或适用性。",
    verifiedAt: "2026-07-30",
    href: "https://huggingface.co/docs/trl/en/dataset_formats",
  },
  "hf-trl-chat-templates": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "TRL Chat Templates",
    title: "Hugging Face TRL — Chat Templates",
    note: "解释聊天模板、角色格式、特殊 Token 与训练时损失掩码；不同基座模板并不通用，必须按目标模型配置复核。",
    verifiedAt: "2026-07-30",
    href: "https://huggingface.co/docs/trl/chat_templates",
  },
  "hf-trl-sft-trainer": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "TRL SFTTrainer",
    title: "Hugging Face TRL — SFT Trainer",
    note: "说明监督微调训练器支持的数据类型、训练与评估配置及模型模板处理；工具能力不等于训练结果有效，仍需独立任务评估。",
    verifiedAt: "2026-07-30",
    href: "https://huggingface.co/docs/trl/en/sft_trainer",
  },
  "hf-trl-dpo-trainer": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "TRL DPOTrainer",
    title: "Hugging Face TRL — DPO Trainer",
    note: "说明 DPO 的 chosen/rejected 偏好数据和训练配置；文档证明实现方式，不证明某项偏好目标或数据标注天然正确。",
    verifiedAt: "2026-07-20",
    href: "https://huggingface.co/docs/trl/en/dpo_trainer",
  },
  "hf-trl-peft": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "TRL × PEFT",
    title: "Hugging Face TRL — PEFT Integration",
    note: "说明 LoRA、QLoRA 与 TRL 训练器的集成，以及冻结基座、训练额外参数的常见方式；实际质量、显存和服务兼容仍需按模型与硬件验证。",
    verifiedAt: "2026-07-30",
    href: "https://huggingface.co/docs/trl/peft_integration",
  },
  "openai-agent-guide": {
    grade: "B",
    kind: "指南",
    shortTitle: "OpenAI Agent 指南",
    title: "A practical guide to building agents",
    note: "将基础 Agent 概括为模型、工具和指令，并说明单 / 多 Agent 编排、护栏、停止条件与人工介入；属于厂商经验框架，不是通用产品认证或效果保证。",
    verifiedAt: "2026-07-17",
    href: "https://openai.com/business/guides-and-resources/a-practical-guide-to-building-ai-agents/",
  },
  "openai-model-spec-hidden-cot": {
    grade: "P",
    kind: "安全治理",
    shortTitle: "OpenAI Model Spec · Hidden CoT",
    title: "OpenAI Model Spec (2025-10-27) — Hidden chain-of-thought",
    note: "说明部分 OpenAI 模型会生成隐藏思维链，但不会向用户或开发者暴露，最多可能提供摘要。该来源只能证明 OpenAI 模型的接口与信息边界，不能外推为所有模型或唯一审计标准。",
    verifiedAt: "2026-07-17",
    href: "https://model-spec.openai.com/2025-10-27",
  },
  "anthropic-effective-agents": {
    grade: "B",
    kind: "指南",
    shortTitle: "Building Effective Agents",
    title: "Building effective agents",
    note: "区分预定义代码路径的 Workflow 与由模型动态决定过程和工具使用的 Agent，并整理常见编排模式；属于厂商工程经验，模式适配仍需客户 PoC。",
    verifiedAt: "2026-07-17",
    href: "https://www.anthropic.com/engineering/building-effective-agents",
  },
  "react-2023": {
    grade: "A",
    kind: "论文",
    shortTitle: "ReAct",
    title: "ReAct: Synergizing Reasoning and Acting in Language Models",
    note: "提出推理、动作与环境观察交替的 Agent 路线；ALFWorld / WebShop 的 34 / 10 个百分点提升只适用于论文任务、模型和提示设置，不能外推为企业 Agent 通用收益。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2210.03629",
  },
  "webarena-2024": {
    grade: "A",
    kind: "论文",
    shortTitle: "WebArena",
    title: "WebArena: A Realistic Web Environment for Building Autonomous Agents",
    note: "以可复现网站和可验证终态评估长链网页任务；原始论文最佳 GPT-4 Agent 为 14.41%，人类为 78.24%，是 2023 年实验基线，不代表当前模型水平。",
    verifiedAt: "2026-07-17",
    href: "https://arxiv.org/abs/2307.13854",
  },
  "google-agent-platform": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Google Agent Platform",
    title: "Scale your agents — Gemini Enterprise Agent Platform",
    note: "说明 Gemini Enterprise Agent Platform 的 Runtime、Sessions、Memory Bank、沙箱、Gateway、身份、Observability 与 Evaluation 等能力；产品名称、地域、发布阶段、配额和价格须按采购时点复核。",
    verifiedAt: "2026-08-01",
    href: "https://docs.cloud.google.com/gemini-enterprise-agent-platform/scale",
  },
  "aws-agentcore": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Amazon Bedrock AgentCore",
    title: "Overview — Amazon Bedrock AgentCore",
    note: "说明 Runtime、Harness、Memory、Gateway、Identity、沙箱、Observability、Evaluations、Policy、Registry、Payments 与 Optimization 等当前能力；Harness 负责托管编排循环，直接使用 Runtime 时循环仍由客户维护。官方目录不等于特定客户任务成功率，发布状态、地域、配额与价格须当期复核。",
    verifiedAt: "2026-08-01",
    href: "https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/",
  },
  "aws-agentcore-memory": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "AgentCore Memory",
    title: "Memory — Amazon Bedrock AgentCore",
    note: "区分会话内原始事件形成的短期记忆，与通过语义、摘要、偏好、情节或自定义策略提取并跨会话检索的长期记忆，并说明按 actor / session 隔离；这是 AgentCore 的产品实现，不是所有 Agent 框架的统一定义。",
    verifiedAt: "2026-07-17",
    href: "https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/harness-memory.html",
  },
  "azure-foundry-agent-service": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Microsoft Foundry Agent Service",
    title: "What is Microsoft Foundry Agent Service?",
    note: "说明 Prompt agents 与 Hosted agents、模型和工具、托管身份 / RBAC / OBO、网络隔离及端到端观测；产品能力、区域、预览状态与计价须按采购时点复核。",
    verifiedAt: "2026-08-01",
    href: "https://learn.microsoft.com/en-us/azure/foundry/agents/overview",
  },
  "opentelemetry-semconv": {
    grade: "O",
    kind: "可观测规范",
    shortTitle: "OpenTelemetry SemConv",
    title: "OpenTelemetry Semantic Conventions",
    note: "为 traces、metrics、logs、events 和资源定义核心与跨组件语义命名；GenAI 专用约定已迁往独立仓库，业务成功、审批与风险字段仍需项目自行扩展。",
    verifiedAt: "2026-08-01",
    href: "https://opentelemetry.io/docs/specs/semconv/",
  },
  "opentelemetry-tail-sampling": {
    grade: "O",
    kind: "可观测规范",
    shortTitle: "OpenTelemetry Sampling",
    title: "Sampling — OpenTelemetry",
    note: "区分 Head Sampling 与在 Trace 完成后按错误、时延或属性等信息决策的 Tail Sampling；它提供采样机制，不定义 AI 风险等级、具体保留比例、隐私合法性或业务留存政策。",
    verifiedAt: "2026-08-01",
    href: "https://opentelemetry.io/docs/concepts/sampling/",
  },
  "opentelemetry-genai-semconv": {
    grade: "O",
    kind: "可观测规范",
    shortTitle: "OTel GenAI SemConv",
    title: "OpenTelemetry Semantic Conventions for Generative AI",
    note: "独立维护生成式 AI 客户端、Agent、MCP、检索与工具调用等遥测语义；当前仍在开发，内容字段多为可选采集，采用时应固定版本并自行补充业务终态、审批和风险字段。",
    verifiedAt: "2026-07-30",
    href: "https://github.com/open-telemetry/semantic-conventions-genai",
  },
  "opentelemetry-genai-observability-2026": {
    grade: "O",
    kind: "官方文档",
    shortTitle: "OTel GenAI Observability",
    title: "Inside the LLM Call: GenAI Observability with OpenTelemetry",
    note: "用 GenAI spans 与 metrics 演示模型、Token、时延和工具调用观测，并明确 GenAI 语义约定正在活跃开发；示例可证明观测字段与成熟度边界，不能证明启用遥测后业务质量、成本或安全自动达标。",
    verifiedAt: "2026-07-29",
    href: "https://opentelemetry.io/blog/2026/genai-observability/",
  },
  "openai-prompting-guide": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "OpenAI Prompting",
    title: "Prompting | OpenAI API",
    note: "建议将生产提示作为应用代码版本化，用类型化或已校验输入构造动态部分，并在每次发布时运行代表性测试与评估；这些是厂商特定建议，不是跨模型消息协议。",
    verifiedAt: "2026-07-30",
    href: "https://developers.openai.com/api/docs/guides/prompting",
  },
  "openai-structured-outputs": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Structured Outputs",
    title: "Structured model outputs | OpenAI API",
    note: "Structured Outputs 可使支持模型的输出遵守所提供 JSON Schema；仍需处理拒答、截断和不支持的 Schema 特性，结构遵循不证明字段值事实正确或业务合规。",
    verifiedAt: "2026-07-30",
    href: "https://developers.openai.com/api/docs/guides/structured-outputs",
  },
  "openai-function-calling": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Function Calling",
    title: "Function calling | OpenAI API",
    note: "工具调用是应用与模型之间的五步闭环；工具定义和 strict Schema 不授予权限，执行、参数校验与审批仍属于应用责任。",
    verifiedAt: "2026-07-30",
    href: "https://developers.openai.com/api/docs/guides/function-calling",
  },
  "google-prompt-introduction": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Google Prompt 入门",
    title: "Introduction to prompting | Gemini Enterprise Agent Platform",
    note: "将 Task、System Instructions、Few-shot Examples 与 Contextual Information 列为常见提示组成；该分类服务于 Google 模型与平台，不是跨厂商正式标准。",
    verifiedAt: "2026-07-17",
    href: "https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/prompts/introduction-prompt-design",
  },
  "google-prompt-strategies": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Google Prompt 策略",
    title: "Overview of prompting strategies | Gemini Enterprise Agent Platform",
    note: "把提示工程描述为以目标和成功结果为前提的测试驱动迭代过程，覆盖目标、指令、约束、上下文、Few-shot 与响应格式等组成。",
    verifiedAt: "2026-07-17",
    href: "https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/prompts/prompt-design-strategies",
  },
  "google-system-instructions": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Google System Instructions",
    title: "System instructions | Gemini Enterprise Agent Platform",
    note: "说明系统指令可定义角色、格式、风格、目标和上下文，同时明确它不能完全阻止越狱或泄漏，不应存放敏感信息；角色和优先级以具体 API 为准。",
    verifiedAt: "2026-07-17",
    href: "https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/prompts/system-instruction-introduction",
  },
  "anthropic-prompt-overview": {
    grade: "P",
    kind: "官方文档",
    shortTitle: "Anthropic Prompt Overview",
    title: "Prompt engineering overview | Claude Platform Docs",
    note: "提示优化前应先有清晰成功标准、可实测方法和初版提示；并指出并非所有失败指标都适合靠提示工程解决，例如时延与成本有时更适合换模型。",
    verifiedAt: "2026-07-30",
    href: "https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/overview",
  },
  "anthropic-context-engineering": {
    grade: "B",
    kind: "指南",
    shortTitle: "Anthropic Context Engineering",
    title: "Effective context engineering for AI agents",
    note: "把 Context Engineering 描述为在推理时选择和维护模型可见的指令、工具、外部数据与历史，并强调上下文是有限资源；该文重点面向 Agent，是厂商实践而非跨厂商标准。",
    verifiedAt: "2026-07-30",
    href: "https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents",
  },
  "transformer-2017": {
    grade: "A", kind: "论文", shortTitle: "Transformer",
    title: "Attention Is All You Need",
    note: "提出以自注意力为核心的 Transformer 架构；支持解释并行训练与注意力机制，不代表所有后续模型实现完全相同。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/1706.03762",
  },
  "attention-not-explanation-2019": {
    grade: "A", kind: "论文", shortTitle: "Attention Is Not Explanation",
    title: "Attention is not Explanation",
    note: "在论文评估的标准注意力模型与 NLP 任务中，注意力权重常与梯度特征重要性不相关，且不同权重分布可产生等价预测；它直接反驳把注意力权重当作完整解释，但不能外推为注意力分析在任何场景都毫无价值。",
    verifiedAt: "2026-07-29", href: "https://aclanthology.org/N19-1357/",
  },
  "gqa-2023": {
    grade: "A", kind: "论文", shortTitle: "Grouped-Query Attention",
    title: "GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints",
    note: "比较 Multi-Head、Multi-Query 与 Grouped-Query Attention，并展示减少 KV Head 的质量与速度折中；论文结论受模型、硬件和实验设定限制。",
    verifiedAt: "2026-07-20", href: "https://aclanthology.org/2023.emnlp-main.298/",
  },
  "sentencepiece-2018": {
    grade: "A", kind: "论文", shortTitle: "SentencePiece",
    title: "SentencePiece: A simple and language independent subword tokenizer and detokenizer for Neural Text Processing",
    note: "提出直接从原始文本训练、以子词单元完成分词与还原的语言无关方法；支持解释 Token 边界由词表和分词器决定，不代表所有模型采用同一算法或词表。",
    verifiedAt: "2026-07-20", href: "https://aclanthology.org/D18-2012/",
  },
  "switch-transformer-2022": {
    grade: "A", kind: "论文", shortTitle: "Switch Transformer",
    title: "Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity",
    note: "研究稀疏 Mixture-of-Experts 路线，以路由器为每个 Token 选择部分专家；支持区分总参数与激活参数，不证明 MoE 模型必然优于稠密模型。",
    verifiedAt: "2026-07-20", href: "https://www.jmlr.org/papers/v23/21-0998.html",
  },
  "chinchilla-2022": {
    grade: "A", kind: "论文", shortTitle: "Chinchilla Scaling",
    title: "Training Compute-Optimal Large Language Models",
    note: "研究模型规模与训练 token 配比的计算最优关系；结论受论文模型族、数据和训练设定限制，不能直接换算客户训练预算。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2203.15556",
  },
  "deduplicating-training-data-2022": {
    grade: "A", kind: "论文", shortTitle: "Deduplicating Training Data",
    title: "Deduplicating Training Data Makes Language Models Better",
    note: "在论文实验中，训练语料去重减少逐字记忆与训练—验证重叠，并以更少训练步达到相同或更好准确率；结果不能外推为通用去重阈值，激进去重也可能移除有价值的语言或领域频率。",
    verifiedAt: "2026-08-01", href: "https://aclanthology.org/2022.acl-long.577/",
  },
  "megatron-3d-parallelism-2021": {
    grade: "A", kind: "论文", shortTitle: "Megatron 3D Parallelism",
    title: "Efficient Large-Scale Language Model Training on GPU Clusters Using Megatron-LM",
    note: "直接研究数据、张量与流水线并行的组合，以及跨节点通信、等待和流水线气泡对大规模训练扩展的影响；论文中的吞吐与扩展效率只适用于其模型、硬件、并行配置和实验规模，不能外推为任意集群收益。",
    verifiedAt: "2026-07-22", href: "https://arxiv.org/abs/2104.04473",
  },
  "torchsnapshot-checkpoint": {
    grade: "O", kind: "官方文档", shortTitle: "TorchSnapshot Checkpoint",
    title: "Getting Started — TorchSnapshot",
    note: "说明分布式训练快照可显式保存和恢复模型、优化器、学习率调度器、自定义应用状态与 RNG 状态，并区分同步和一致的异步快照；这是 PyTorch 生态的具体实现契约，不证明其他框架会自动保存数据游标或所有业务所需状态。",
    verifiedAt: "2026-08-01", href: "https://meta-pytorch.org/torchsnapshot/stable/getting_started.html",
  },
  "pytorch-distributed-checkpoint": {
    grade: "O", kind: "官方文档", shortTitle: "PyTorch Distributed Checkpoint",
    title: "Distributed Checkpoint — torch.distributed.checkpoint",
    note: "说明多 Rank 并行保存与加载、模型和优化器状态、加载时重新分片及异步保存完成语义；部分 API 仍为实验性且跨版本不保证向后兼容，数据游标、调度器、业务状态和完整恢复合同仍由训练应用定义。",
    verifiedAt: "2026-08-01", href: "https://docs.pytorch.org/docs/main/distributed.checkpoint.html",
  },
  "pytorch-reproducibility": {
    grade: "O", kind: "官方文档", shortTitle: "PyTorch Reproducibility",
    title: "Reproducibility — PyTorch",
    note: "明确跨 PyTorch 版本、提交、平台及 CPU/GPU 不保证完全复现，并说明确定性设置可能降低性能；可支持把恢复目标写成受控环境下的可解释容差，而不是承诺逐位一致。",
    verifiedAt: "2026-08-01", href: "https://docs.pytorch.org/docs/stable/notes/randomness",
  },
  "hf-transformers-tokenizer-contract": {
    grade: "O", kind: "官方文档", shortTitle: "Transformers Tokenizer Contract",
    title: "Tokenizer — Hugging Face Transformers",
    note: "说明新增普通或特殊 Token 会改变词表身份，并要求相应调整模型 Embedding 矩阵；它支持 Tokenizer、特殊 Token 与权重兼容是同一输入合同，不证明完整更换 Tokenizer 后原模型能力可直接保留。",
    verifiedAt: "2026-08-01", href: "https://huggingface.co/docs/transformers/main_classes/tokenizer",
  },
  "checkfreq-2021": {
    grade: "A", kind: "论文", shortTitle: "CheckFreq",
    title: "CheckFreq: Frequent, Fine-Grained DNN Checkpointing",
    note: "研究按在线剖析动态选择 Checkpoint 频率、流水化写入，并通过可恢复迭代器保存数据加载状态；它支持解释频率、I/O 开销和数据连续性的取舍，但实验结果不能直接确定客户集群的最佳间隔。",
    verifiedAt: "2026-07-22", href: "https://www.usenix.org/conference/fast21/presentation/mohan",
  },
  "lora-2021": {
    grade: "A", kind: "论文", shortTitle: "LoRA",
    title: "LoRA: Low-Rank Adaptation of Large Language Models",
    note: "提出冻结基础权重并训练低秩适配矩阵的参数高效微调方法；实际质量、显存与速度仍取决于模型、任务和实现。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2106.09685",
  },
  "qlora-2023": {
    grade: "A", kind: "论文", shortTitle: "QLoRA",
    title: "QLoRA: Efficient Finetuning of Quantized LLMs",
    note: "展示量化基础模型上的参数高效微调路线；论文实验不能外推为任意模型、硬件和客户数据都能保持同等质量。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2305.14314",
  },
  "instructgpt-2022": {
    grade: "A", kind: "论文", shortTitle: "InstructGPT",
    title: "Training language models to follow instructions with human feedback",
    note: "展示监督微调、偏好数据和强化学习形成指令跟随模型的路线；支持训练阶段分工，不构成通用企业效果承诺。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2203.02155",
  },
  "dpo-2023": {
    grade: "A", kind: "论文", shortTitle: "DPO",
    title: "Direct Preference Optimization: Your Language Model is Secretly a Reward Model",
    note: "提出直接用偏好对优化语言模型的对齐方法；适用性依赖偏好数据质量、任务与基线。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2305.18290",
  },
  "deepseek-r1-2025": {
    grade: "A", kind: "论文", shortTitle: "DeepSeek-R1",
    title: "DeepSeek-R1 incentivizes reasoning in LLMs through reinforcement learning",
    note: "展示可验证奖励、强化学习、冷启动数据、多阶段训练与蒸馏在一个模型族中的组合路线；结果不能外推为强化学习会自动提升所有开放任务，也不能把内部推理当作审计解释。",
    verifiedAt: "2026-07-20", href: "https://www.nature.com/articles/s41586-025-09422-z",
  },
  "clip-2021": {
    grade: "A", kind: "论文", shortTitle: "CLIP",
    title: "Learning Transferable Visual Models From Natural Language Supervision",
    note: "通过图像—文本对比学习形成共享语义表征；支持跨模态检索基础，不保证视觉事实、计数或空间推理正确。",
    verifiedAt: "2026-07-30", href: "https://proceedings.mlr.press/v139/radford21a.html",
  },
  "vit-2021": {
    grade: "A", kind: "论文", shortTitle: "Vision Transformer",
    title: "An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale",
    note: "将图像 Patch 序列交给 Transformer 的经典路线；用于解释视觉 token 化，不代表所有多模态模型采用相同架构或计费。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2010.11929",
  },
  "longvideobench-2024": {
    grade: "A", kind: "论文", shortTitle: "LongVideoBench",
    title: "LongVideoBench: A Benchmark for Long-context Interleaved Video-Language Understanding",
    note: "构造最长约一小时、交错视频与语言输入并要求时间定位的长视频问答基准；支持把采样、字幕、时序定位和模型共同纳入评估，不证明整段输入、更多帧或某个模型在客户视频上必然更好。",
    verifiedAt: "2026-07-30", href: "https://proceedings.neurips.cc/paper_files/paper/2024/hash/329ad516cf7a6ac306f29882e9c77558-Abstract-Datasets_and_Benchmarks_Track.html",
  },
  "vllm-2023": {
    grade: "A", kind: "论文", shortTitle: "vLLM / PagedAttention",
    title: "Efficient Memory Management for Large Language Model Serving with PagedAttention",
    note: "提出 PagedAttention 与面向 LLM Serving 的 KV Cache 内存管理；论文吞吐结果受模型、硬件和负载限制。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2309.06180",
  },
  "vllm-metrics-v0-12": {
    grade: "O", kind: "官方文档", shortTitle: "vLLM v0.12 Metrics",
    title: "Metrics — vLLM v0.12.0",
    note: "记录 vLLM v0.12.0 的服务端请求指标，包括排队、Prefill、Decode、首 Token、Token 间时延、端到端时延、请求状态和 KV Cache 使用；这些引擎侧指标不自动包含网关、网络、客户端渲染或业务终态，其他版本的字段也可能不同。",
    verifiedAt: "2026-08-01", href: "https://docs.vllm.ai/en/v0.12.0/design/metrics/",
  },
  "gptq-2023": {
    grade: "A", kind: "论文", shortTitle: "GPTQ",
    title: "GPTQ: Accurate Post-Training Quantization for Generative Pre-trained Transformers",
    note: "提出一遍式权重量化并在论文所测 GPT/OPT 模型与 A100/A6000 配置上报告内存、精度和端到端速度结果；这些数字不能外推到其他模型、位宽、内核、硬件、任务切片或服务成本。",
    verifiedAt: "2026-07-22", href: "https://arxiv.org/abs/2210.17323",
  },
  "speculative-decoding-2023": {
    grade: "A", kind: "论文", shortTitle: "Speculative Decoding",
    title: "Fast Inference from Transformers via Speculative Decoding",
    note: "给出由近似模型提出多个 Token、目标模型并行验证且保持目标分布的投机解码方法；论文加速结果受草稿质量、模型、硬件和实现限制，不证明所有请求都会提速。",
    verifiedAt: "2026-07-22", href: "https://proceedings.mlr.press/v202/leviathan23a.html",
  },
  "vllm-prefix-caching": {
    grade: "P", kind: "官方文档", shortTitle: "vLLM Prefix Caching",
    title: "Automatic Prefix Caching — vLLM",
    note: "vLLM v0.15.1 设计文档说明自动前缀缓存复用共享前缀的 KV Cache，主要减少 Prefill 而不减少新 Token 的 Decode；cache_salt 可限制前缀复用的信任组并缓解时间侧信道，但不是完整的租户授权、内存清除或侧信道安全证明。",
    verifiedAt: "2026-08-01", href: "https://docs.vllm.ai/en/v0.15.1/design/prefix_caching/",
  },
  "llm-serving-fairness-2024": {
    grade: "A", kind: "论文", shortTitle: "LLM Serving Fairness",
    title: "Fairness in Serving Large Language Models",
    note: "研究请求长度不可预测与连续批处理条件下的 LLM Serving 公平问题，并提出按输入、输出 Token 成本计量的调度方法；论文算法和实验不替组织定义租户权利、业务优先级或通用公平阈值。",
    verifiedAt: "2026-08-01", href: "https://www.usenix.org/conference/osdi24/presentation/sheng",
  },
  "serverlessllm-2024": {
    grade: "A", kind: "论文", shortTitle: "ServerlessLLM",
    title: "ServerlessLLM: Low-Latency Serverless Inference for Large Language Models",
    note: "研究模型制品加载、分层存储、启动时间感知调度与推理迁移，证明模型加载和就绪时间是弹性推理的重要路径；论文结果受其系统、硬件、存储和负载限制，不代表任意平台都能达到相同冷启动性能。",
    verifiedAt: "2026-08-01", href: "https://www.usenix.org/conference/osdi24/presentation/fu",
  },
  "jitserve-2026": {
    grade: "A", kind: "论文", shortTitle: "JITServe",
    title: "JITServe: SLO-aware LLM Serving with Imprecise Request Information",
    note: "研究交互、截止时间和复合调用等不同 SLO 下的 LLM 请求调度，并用满足 SLO 的 Goodput 评估服务；论文算法、收益和请求信息假设仅适用于其设计与实验，不提供客户场景的通用准入或丢弃阈值。",
    verifiedAt: "2026-08-01", href: "https://www.usenix.org/conference/nsdi26/presentation/zhang-wei",
  },
  "distserve-2024": {
    grade: "A", kind: "论文", shortTitle: "DistServe",
    title: "DistServe: Disaggregating Prefill and Decoding for Goodput-optimized Large Language Model Serving",
    note: "直接研究 Prefill / Decode 分离以及 TTFT、TPOT 约束下的 Goodput；论文收益受模型、硬件、网络、调度和请求分布限制。",
    verifiedAt: "2026-07-20", href: "https://www.usenix.org/conference/osdi24/presentation/zhong-yinmin",
  },
  "flashattention-2022": {
    grade: "A", kind: "论文", shortTitle: "FlashAttention",
    title: "FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness",
    note: "通过 IO-aware 算法减少注意力计算的内存访问；具体收益依赖序列、硬件、内核与推理栈。",
    verifiedAt: "2026-07-17", href: "https://arxiv.org/abs/2205.14135",
  },
  "mcp-architecture": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28 Architecture",
    title: "Architecture — Model Context Protocol 2026-07-28",
    note: "定义 MCP 是无状态协议、Host 的职责，以及每个 Client 与一个 Server 的逻辑 1:1 关系；协议不规定业务授权、Server 信任或最终结果。",
    verifiedAt: "2026-08-01", href: "https://modelcontextprotocol.io/specification/2026-07-28/architecture",
  },
  "mcp-lifecycle-2025-11-25": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2025-11-25 Lifecycle（历史版）",
    title: "Lifecycle — Model Context Protocol 2025-11-25",
    note: "记录历史版本 2025-11-25 的初始化、版本与能力协商、运行和关闭语义；截至 2026-07-29，官方 latest 已指向 2026-07-28，本页只能用于锁定旧版本的实现与迁移对照。",
    verifiedAt: "2026-07-29", href: "https://modelcontextprotocol.io/specification/2025-11-25/basic/lifecycle",
  },
  "mcp-2026-07-28-rc": {
    grade: "O", kind: "官方公告", shortTitle: "MCP 2026-07-28 RC 公告",
    title: "The 2026-07-28 MCP Specification Release Candidate",
    note: "历史 RC 公告记录了计划发布日期与候选变更；截至 2026-08-30，官方 latest 仍指向 2026-07-28 正式规范，该里程碑已完成。规范性要求必须回到正式版，不能继续把 RC 当作当前状态。",
    verifiedAt: "2026-09-05", href: "https://blog.modelcontextprotocol.io/posts/2026-07-28-release-candidate/",
  },
  "mcp-specification-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28",
    title: "Model Context Protocol Specification 2026-07-28",
    note: "官方 latest 入口已指向该正式版本；规范定义无状态、自包含请求、逐请求能力协商与可选扩展，并将 Tasks 作为扩展。正式规范发布不等于具体 SDK、客户端、Server 或产品已经兼容。",
    verifiedAt: "2026-09-04", href: "https://modelcontextprotocol.io/specification/2026-07-28",
  },
  "mcp-changelog-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28 Key Changes",
    title: "Key Changes — Model Context Protocol 2026-07-28",
    note: "记录 initialize 与协议级 Session 的移除、逐请求版本和能力元数据、Server 必须支持 server/discover、Tasks 移至扩展及其他破坏性变化；不能据此推断具体实现已经兼容。",
    verifiedAt: "2026-09-04", href: "https://modelcontextprotocol.io/specification/2026-07-28/changelog",
  },
  "mcp-mrtr-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP Multi Round-Trip Requests",
    title: "Multi Round-Trip Requests — Model Context Protocol 2026-07-28",
    note: "定义 prompts/get、resources/read 与 tools/call 的多轮补参模式：Server 返回 input_required；inputRequests 与不透明 requestState 各自可选，但每个 InputRequiredResult 至少必须包含其中一项。Client 以新的 JSON-RPC id 重试原操作，只对收到的 inputRequests 提交相应 inputResponses；仅在收到 requestState 时原样回传，未收到时不得自行添加。它不是协议会话、授权或持久 Task。",
    verifiedAt: "2026-09-04", href: "https://modelcontextprotocol.io/specification/2026-07-28/basic/patterns/mrtr",
  },
  "mcp-list-cache-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP Result Caching",
    title: "Caching — Model Context Protocol 2026-07-28",
    note: "定义 server/discover、tools/list、prompts/list、resources/list、resources/templates/list 与 resources/read 的 resultType: \"complete\" 结果必须携带 ttlMs 和 cacheScope；resultType: \"input_required\" 中间结果不携带提示且不得缓存，含 inputResponses 或 requestState 的 MRTR 重试也不得缓存。TTL 是新鲜度提示而非一致性或撤权传播保证，private 结果不得跨授权上下文（例如不同 access token）复用；身份、租户、Scope 与版本绑定是生产控制推导。",
    verifiedAt: "2026-09-04", href: "https://modelcontextprotocol.io/specification/2026-07-28/server/utilities/caching",
  },
  "mcp-http-routing-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP HTTP Routing Metadata",
    title: "Streamable HTTP — Model Context Protocol 2026-07-28",
    note: "定义 Streamable HTTP 中每个承载 JSON-RPC request 的 Client POST 都必须提供 Mcp-Method；本修订未规定 notification POST 的该头要求，且核心协议不定义 Streamable HTTP 上的 Client→Server notification。核心协议的 Mcp-Name 镜像 tools/call 与 prompts/get 的 params.name，或 resources/read 的 params.uri。镜像头不能替代正文校验、主体授权或业务策略；不一致请求应被拒绝。",
    verifiedAt: "2026-09-04", href: "https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http",
  },
  "mcp-server-overview-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28 Server Features",
    title: "Server Features Overview — Model Context Protocol 2026-07-28",
    note: "直接定义 Server 原语的控制层级：Prompt 由用户控制选择、Resource 由应用控制、Tool 由模型控制；该层级不等于读写风险分类，也不授予业务权限。",
    verifiedAt: "2026-08-01", href: "https://modelcontextprotocol.io/specification/2026-07-28/server",
  },
  "mcp-tools-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28 Tools",
    title: "Tools — Model Context Protocol 2026-07-28",
    note: "定义由模型控制的可调用操作，既可查询数据库，也可调用 API 或执行计算；Host 应保留人工拒绝能力，未受信 Server 的 Tool annotations 必须视为不可信。Tool Schema 与发现不构成业务授权。",
    verifiedAt: "2026-08-01", href: "https://modelcontextprotocol.io/specification/2026-07-28/server/tools",
  },
  "mcp-resources-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28 Resources",
    title: "Resources — Model Context Protocol 2026-07-28",
    note: "定义由应用控制、以 URI 寻址的上下文，以及发现、读取与可选订阅机制；Resource 仍需数据权限、版本、缓存和不可信内容治理。",
    verifiedAt: "2026-08-01", href: "https://modelcontextprotocol.io/specification/2026-07-28/server/resources",
  },
  "mcp-prompts-2026-07-28": {
    grade: "O", kind: "协议规范", shortTitle: "MCP 2026-07-28 Prompts",
    title: "Prompts — Model Context Protocol 2026-07-28",
    note: "定义由用户控制选择、由 Server 编写内容的可复用模板；用户选择不证明模板可信，也不替代后续 Tool 的授权和结果验证。",
    verifiedAt: "2026-08-01", href: "https://modelcontextprotocol.io/specification/2026-07-28/server/prompts",
  },
  "mcp-tasks-extension": {
    grade: "O", kind: "协议规范", shortTitle: "MCP Tasks Extension",
    title: "Tasks Extension Specification — Model Context Protocol",
    note: "当前只允许 tools/call 返回 CreateTaskResult，并定义耐久句柄、ttlMs、pollIntervalMs、working / input_required / completed / failed / cancelled 状态、轮询、tasks/update、恢复与合作式取消；规范为未来其他 request types 留出扩展空间。Streamable HTTP 下，tasks/get、tasks/update 与 tasks/cancel 通过 Mcp-Name 镜像 params.taskId。MCP Task 当前包装一次 tools/call，不等同于 A2A Task，也不证明业务终态或副作用已经停止。",
    verifiedAt: "2026-09-04", href: "https://tasks.extensions.modelcontextprotocol.io/specification/draft/tasks",
  },
  "mcp-authorization": {
    grade: "O", kind: "协议规范", shortTitle: "MCP Authorization",
    title: "Authorization — Model Context Protocol Specification",
    note: "2026-07-28 正式规范定义 HTTP 传输的可选 MCP 授权流程，包括客户端、授权服务器、资源服务器、发行者校验、资源绑定、Scope 与 Step-up；它不替代工具内部的租户策略和业务权限，也不能证明具体 SDK 或产品已经完整实现。",
    verifiedAt: "2026-07-30", href: "https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization",
  },
  "mcp-security": {
    grade: "O", kind: "安全治理", shortTitle: "MCP Security",
    title: "Security Best Practices — Model Context Protocol",
    note: "2026-07-28 安全指南整理 MCP 实现中的 State Handle、令牌、混淆代理、本地 Server 与权限风险；协议级 Session 只属于旧版兼容边界。该指南不支持一般 AI 软件供应链的全部准入结论，也不替代组织自身威胁建模和控制验证。",
    verifiedAt: "2026-08-01", href: "https://modelcontextprotocol.io/docs/2026-07-28/tutorials/security/security_best_practices",
  },
  "mcp-tasks-2025-11-25": {
    grade: "O", kind: "协议规范", shortTitle: "MCP Tasks 2025-11-25（历史版）",
    title: "Tasks — Model Context Protocol 2025-11-25",
    note: "记录 2025-11-25 核心协议中实验性 Tasks 的耐久包装、轮询、延迟结果、状态和取消语义；2026-07-28 已把 Tasks 移至可选扩展，旧页只适用于锁定版本的实现与迁移对照，也不等同于 A2A 的跨 Agent Task。",
    verifiedAt: "2026-07-29", href: "https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks",
  },
  "mcp-elicitation-2025-11-25": {
    grade: "O", kind: "协议规范", shortTitle: "MCP Elicitation",
    title: "Elicitation — Model Context Protocol 2025-11-25",
    note: "定义 Form 与 URL 两种用户交互模式；密码、API Key、访问令牌和支付凭据不得经 Form 收集，URL 交互仍需展示目标域名并取得用户同意。Elicitation 不替代 MCP 或业务授权。",
    verifiedAt: "2026-07-20", href: "https://modelcontextprotocol.io/specification/2025-11-25/client/elicitation",
  },
  "a2a-concepts": {
    grade: "O", kind: "官方文档", shortTitle: "A2A Core Concepts",
    title: "Core Concepts — A2A Protocol",
    note: "定义 Agent Card、Message、Task、Part 与 Artifact 等跨 Agent 协作对象，并明确远端可直接返回 Message 或创建 Task；不规定单个 Agent 内部实现，也不证明产物质量或业务验收。",
    verifiedAt: "2026-08-01", href: "https://a2a-protocol.org/latest/topics/key-concepts/",
  },
  "a2a-specification": {
    grade: "O", kind: "官方文档", shortTitle: "A2A Specification",
    title: "A2A Protocol Specification",
    note: "A2A 最新规范入口，覆盖 Message 或 Task 响应、精确任务状态、contextId 生成后随响应返回、标识校验、仅非终态可订阅、Push 能力与逐 webhook 投递尝试、required Extension 的精确错误、逐请求 Major.Minor 版本、协议绑定、取消与更新；协议合规不证明 Push 成功、业务验收、授权或副作用回滚，具体实现仍需逐项核验。",
    verifiedAt: "2026-09-04", href: "https://a2a-protocol.org/latest/specification/",
  },
  "a2a-release-1-0-1": {
    grade: "O", kind: "协议规范", shortTitle: "A2A v1.0.1 Release",
    title: "A2A Protocol Release v1.0.1",
    note: "官方发布记录将 v1.0.1 标为最新补丁版本；协议兼容仍按 Major.Minor 的 1.0 协商。规范要求补丁号不应用于请求、响应或 Agent Card，且不得参与版本协商；发布记录也不能证明具体实现已采用全部修复。",
    verifiedAt: "2026-09-04", href: "https://github.com/a2aproject/A2A/releases/tag/v1.0.1",
  },
  "a2a-agent-discovery": {
    grade: "O", kind: "协议规范", shortTitle: "A2A Agent Discovery",
    title: "Agent Discovery in A2A",
    note: "说明 Agent Card 的 Well-Known URI、受控 Registry 与直接配置等发现方式，并区分公开 Card、认证后的 Extended Agent Card 与实际接入信任。发现只产生候选端点；来源、身份、授权、版本与能力仍需独立核验，签名语义须回到规范。",
    verifiedAt: "2026-09-04", href: "https://a2a-protocol.org/latest/topics/agent-discovery/",
  },
  "a2a-extensions": {
    grade: "O", kind: "协议规范", shortTitle: "A2A Extensions",
    title: "Extensions in A2A",
    note: "定义以稳定 URI 标识、在 Agent Card 声明并由调用方通过 A2A-Extensions 逐请求启用的扩展。Extension 没有独立 version 字段，破坏性变化必须使用新 URI；不支持的可选请求可以忽略，不支持的可选版本应被忽略且不得回退。required Extension 缺失或违规时应以适当错误拒绝；精确 ExtensionSupportRequiredError 的 MUST 语义以 A2A 主规范为准。扩展可增加数据、配置或方法，但不得改变核心数据结构定义或给核心枚举加值，也不等同于认证后的 Extended Agent Card。",
    verifiedAt: "2026-09-04", href: "https://a2a-protocol.org/latest/topics/extensions/",
  },
  "a2a-mcp-boundary": {
    grade: "O", kind: "协议规范", shortTitle: "A2A and MCP",
    title: "A2A and MCP — A2A Protocol",
    note: "说明 A2A 面向独立 Agent 间协作，MCP 面向应用与工具、资源的连接；两者可组合，但 Task、身份、授权和生命周期仍需分别设计。",
    verifiedAt: "2026-08-01", href: "https://a2a-protocol.org/latest/topics/a2a-and-mcp/",
  },
  "a2a-spec-1-0-0": {
    grade: "O", kind: "协议规范", shortTitle: "A2A 1.0.0",
    title: "A2A Protocol Specification v1.0.0",
    note: "固定 A2A 1.0.0 的版本协商、AgentInterface、多协议绑定、Agent Card 签名和任务语义；网络连通或签名有效不证明业务能力、质量或授权成立。",
    verifiedAt: "2026-07-21", href: "https://a2a-protocol.org/v1.0.0/specification/",
  },
  "openai-models": {
    grade: "P", kind: "模型目录", shortTitle: "OpenAI Models",
    title: "Models — OpenAI API",
    note: "OpenAI 官方模型目录；型号、能力、上下文、可用接口、退役与计价属于高时效事实，必须按使用时点复核。",
    verifiedAt: "2026-09-05", href: "https://developers.openai.com/api/docs/models",
  },
  "openai-reasoning-guide": {
    grade: "P", kind: "官方文档", shortTitle: "OpenAI Reasoning Guide",
    title: "Reasoning models — OpenAI API",
    note: "说明部分 OpenAI 模型可用 reasoning effort 调节思考强度，并带来质量、速度与 Token 使用取舍；这是厂商特定的当前 API 能力，支持值和默认值随模型而异。",
    verifiedAt: "2026-07-30", href: "https://developers.openai.com/api/docs/guides/reasoning",
  },
  "google-models": {
    grade: "P", kind: "模型目录", shortTitle: "Gemini Models",
    title: "Models — Gemini API",
    note: "Google 官方 Gemini 模型目录；型号、能力、限制和发布阶段会变化，不在知识正文固化当前排名。",
    verifiedAt: "2026-09-05", href: "https://ai.google.dev/gemini-api/docs/models",
  },
  "anthropic-models": {
    grade: "P", kind: "模型目录", shortTitle: "Claude Models",
    title: "Models overview — Claude Platform Docs",
    note: "Anthropic 官方 Claude 模型目录；能力、生命周期、区域和定价应在选型时重新核验。",
    verifiedAt: "2026-09-05", href: "https://platform.claude.com/docs/en/models/overview",
  },
  "osi-open-source-ai-definition-1-0": {
    grade: "O", kind: "标准", shortTitle: "Open Source AI Definition 1.0",
    title: "The Open Source AI Definition — 1.0",
    note: "定义开放源代码 AI 所需的使用、研究、修改和分享自由，并要求提供足够的数据说明、训练与运行代码和参数；获得权重本身不足以证明整个 AI 系统满足该定义。",
    verifiedAt: "2026-07-30", href: "https://opensource.org/ai/open-source-ai-definition",
  },
  "dify-open-source-license": {
    grade: "P",
    kind: "法规标准",
    shortTitle: "Dify License",
    title: "Dify Open Source License",
    note: "Dify 官方仓库采用基于 Apache License 2.0 的修改版许可证，并对未经书面授权的多租户服务以及前端 Logo、版权信息设置附加条件；能获取和自托管代码不等于可以忽略这些许可证边界。",
    verifiedAt: "2026-08-08",
    href: "https://github.com/langgenius/dify/blob/main/LICENSE",
  },
  "dify-enterprise-pricing": {
    grade: "P",
    kind: "产品规格",
    shortTitle: "Dify Enterprise",
    title: "Dify Enterprise Pricing and Plan Comparison",
    note: "Dify 当期官方页面把 Community 与 Enterprise 的工作区、商业授权、SSO、安全控制、维护、支持和协商 SLA 等能力分开列示；这些是会变化的厂商自述，采购时仍需以目标版本和合同为准。",
    verifiedAt: "2026-09-05",
    href: "https://dify.ai/pricing/dify-enterprise",
  },
  "openai-prompt-caching": {
    grade: "P", kind: "官方文档", shortTitle: "Prompt Caching",
    title: "Prompt caching | OpenAI API",
    note: "说明 OpenAI API 的前缀缓存、命中与用量可见性；只适用于该产品当期机制，不能外推为所有平台或整体应用成本。",
    verifiedAt: "2026-07-17", href: "https://developers.openai.com/api/docs/guides/prompt-caching",
  },
  "cloudflare-ai-gateway": {
    grade: "P", kind: "官方文档", shortTitle: "Cloudflare AI Gateway",
    title: "AI Gateway — Cloudflare Docs",
    note: "说明 AI 网关的统一接入、分析、缓存、限流、日志和路由等产品能力；产品范围、计费和限制需按采购时点复核。",
    verifiedAt: "2026-07-17", href: "https://developers.cloudflare.com/ai-gateway/",
  },
  "cloudflare-ai-gateway-authentication": {
    grade: "P", kind: "官方文档", shortTitle: "AI Gateway Authentication",
    title: "Authentication — Cloudflare AI Gateway",
    note: "说明网关令牌验证与对上游提供方凭据的集中管理；这些机制保护网关和提供方接入，但不会自动建立最终用户、工作负载与下游业务资源之间的授权链。",
    verifiedAt: "2026-08-01", href: "https://developers.cloudflare.com/ai-gateway/configuration/authentication/",
  },
  "cloudflare-ai-gateway-caching": {
    grade: "P", kind: "官方文档", shortTitle: "AI Gateway Caching",
    title: "Caching — Cloudflare AI Gateway",
    note: "说明对相同请求进行缓存与返回命中结果的产品机制；它不是语义缓存规范，也不替应用定义租户隔离、当前授权、版本、时效与失效策略。",
    verifiedAt: "2026-08-01", href: "https://developers.cloudflare.com/ai-gateway/features/caching/",
  },
  "cloudflare-ai-gateway-spend-limits": {
    grade: "P", kind: "官方文档", shortTitle: "AI Gateway Spend Limits",
    title: "Spend limits — Cloudflare AI Gateway",
    note: "说明按时间区间与成本设置网关消费上限的产品能力；消费限制不能替代请求、Token、并发、提供方配额、任务质量与业务预算的联合治理。",
    verifiedAt: "2026-08-01", href: "https://developers.cloudflare.com/ai-gateway/features/spend-limits/",
  },
  "cloudflare-ai-gateway-dynamic-routing": {
    grade: "P", kind: "官方文档", shortTitle: "AI Gateway Dynamic Routing",
    title: "Dynamic routing — Cloudflare AI Gateway",
    note: "说明按请求属性和端点配置执行动态路由的产品能力；路由可配置不等于候选模型在质量、地域、工具、安全或错误语义上等价。",
    verifiedAt: "2026-08-01", href: "https://developers.cloudflare.com/ai-gateway/features/dynamic-routing/",
  },
  "azure-apim-ai-gateway": {
    grade: "P", kind: "官方文档", shortTitle: "Azure APIM AI Gateway",
    title: "AI gateway capabilities in Azure API Management",
    note: "说明 Azure API Management 的 Token 限流、配额、负载均衡、语义缓存与可观测等生成式 AI 网关能力；这些是具体产品机制，不构成跨产品统一语义或业务授权。",
    verifiedAt: "2026-08-01", href: "https://learn.microsoft.com/en-us/azure/api-management/genai-gateway-capabilities",
  },
  "aws-builders-library-retries": {
    grade: "P", kind: "指南", shortTitle: "Timeouts, Retries & Jitter",
    title: "Timeouts, retries, and backoff with jitter — Amazon Builders' Library",
    note: "说明多层重试会放大负载，并建议以总超时、有限重试、幂等性、退避和抖动控制恢复；它不是 AI 网关产品 SLA，也不能证明有副作用的调用可安全重放。",
    verifiedAt: "2026-08-01", href: "https://d1.awsstatic.com/builderslibrary/pdfs/timeouts-retries-and-backoff-with-jitter.pdf",
  },
  "cncf-platforms-whitepaper": {
    grade: "O", kind: "行业指南", shortTitle: "CNCF Platforms White Paper",
    title: "CNCF Platforms White Paper",
    note: "把平台描述为面向内部用户、以产品方式经营的能力集合，并强调一致体验、自助 API、模板、文档与 Golden Path；它提供平台工程方法，不规定 AI 平台必须包含哪些产品或组件。",
    verifiedAt: "2026-08-01", href: "https://tag-app-delivery.cncf.io/whitepapers/platforms/",
  },
  "kubernetes-multi-tenancy": {
    grade: "O", kind: "官方文档", shortTitle: "Kubernetes Multi-tenancy",
    title: "Multi-tenancy — Kubernetes",
    note: "说明共享集群需要同时处理安全、公平与 Noisy Neighbor，并讨论 Namespace、RBAC、ResourceQuota、NetworkPolicy 与虚拟控制层等边界；这些原语不自动保证特定 CNI、运行时或 GPU 共享方式实现硬隔离。",
    verifiedAt: "2026-08-01", href: "https://kubernetes.io/docs/concepts/security/multi-tenancy/",
  },
  "kubernetes-dra-1-36": {
    grade: "O", kind: "官方文档", shortTitle: "Kubernetes 1.36 DRA",
    title: "Kubernetes v1.36: More Drivers, New Features, and the Next Era of DRA",
    note: "逐项记录 Kubernetes 1.36 中 DRA 的稳定、Beta 与 Alpha 能力，包括优先列表、分区设备、设备污点、绑定条件、健康状态与面向工作负载的 ResourceClaim；版本文章不能证明具体驱动、厂商集成或客户工作负载已经生产就绪。",
    verifiedAt: "2026-08-01", href: "https://kubernetes.io/blog/2026/05/07/kubernetes-v1-36-dra-136-updates/",
  },
  "oci-image-spec-v1-1-1": {
    grade: "O", kind: "标准", shortTitle: "OCI Image Spec 1.1.1",
    title: "Open Container Initiative Image Format Specification v1.1.1",
    note: "定义镜像清单（Image Manifest）、Index、文件系统层和运行配置，以支持镜像构建、传输与运行准备工具互操作；它不标准化 GPU 驱动、内核、数据、IAM、网络、存储、观测或完整工作负载行为。",
    verifiedAt: "2026-08-01", href: "https://specs.opencontainers.org/image-spec/?v=v1.1.1",
  },
  "kubernetes-dra": {
    grade: "O", kind: "官方文档", shortTitle: "Kubernetes DRA",
    title: "Dynamic Resource Allocation — Kubernetes",
    note: "说明 Kubernetes 中按设备属性、拓扑与声明分配资源的机制；功能状态和字段与 Kubernetes 版本相关。",
    verifiedAt: "2026-07-21", href: "https://kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/",
  },
  "kueue-all-or-nothing": {
    grade: "O", kind: "官方文档", shortTitle: "Kueue All-or-Nothing",
    title: "Setup All-or-nothing with ready Pods — Kueue",
    note: "说明同步分布式训练或 MPI 作业可能需要所有 Pod 同时运行，并记录 Kueue waitForPodsReady 的超时、驱逐与重新排队语义；这是 Kueue 的一种简单全有或全无实现，不代表 Kubernetes 原生调度器或所有作业控制器都自动提供 Gang Scheduling。",
    verifiedAt: "2026-07-22", href: "https://kueue.sigs.k8s.io/docs/tasks/manage/setup_wait_for_pods_ready/",
  },
  "kueue-fair-sharing": {
    grade: "O", kind: "官方文档", shortTitle: "Kueue Fair Sharing",
    title: "Preemption — Kueue",
    note: "说明 Kueue 的配额、优先级、借用、抢占与加权公平共享机制，并区分经典抢占和 Fair Sharing；这些机制提供策略工具，不替组织定义公平目标、业务优先级、SLO 或可接受的中断成本。",
    verifiedAt: "2026-07-22", href: "https://kueue.sigs.k8s.io/docs/concepts/preemption/",
  },
  "slurm-overview": {
    grade: "O", kind: "官方文档", shortTitle: "Slurm Overview",
    title: "Slurm Workload Manager — Overview",
    note: "定义 Slurm 的集群资源分配、并行作业启动与监控、队列仲裁，并列出拓扑选择、Gang Scheduling、回填、抢占、资源限制和多因子优先级插件；官方能力清单不证明某个站点已启用或正确配置这些功能。",
    verifiedAt: "2026-07-22", href: "https://slurm.schedmd.com/overview.html",
  },
  "slurm-multifactor-priority": {
    grade: "O", kind: "官方文档", shortTitle: "Slurm Multifactor Priority",
    title: "Slurm Workload Manager — Multifactor Priority Plugin",
    note: "说明 Slurm 可使用 FIFO，也可按年龄、Fair-share、作业规模、分区、QOS 与站点因素组合优先级；文档支持调度机制比较，但不能替具体组织决定什么是公平或保证业务期限。",
    verifiedAt: "2026-07-22", href: "https://slurm.schedmd.com/priority_multifactor.html",
  },
  "nccl-collectives": {
    grade: "O", kind: "官方文档", shortTitle: "NCCL Collectives",
    title: "NVIDIA Collective Communication Library Documentation",
    note: "记录 GPU 间 AllReduce、AllGather、ReduceScatter、All-to-All 等集合通信、通信器、网络与拓扑诊断能力；它定义通信原语与实现边界，不证明某个训练作业会达到线性扩展或某种网络一定更快。",
    verifiedAt: "2026-08-01", href: "https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/index.html",
  },
  "roofline-2009": {
    grade: "A", kind: "论文", shortTitle: "Roofline Model",
    title: "Roofline: An Insightful Visual Performance Model for Multicore Architectures",
    note: "用运算强度、峰值计算吞吐和内存带宽上界解释内核可能受计算或数据移动限制；它是瓶颈推理模型，不是目标 AI 框架、完整模型、网络、存储或业务 SLO 的性能承诺。",
    verifiedAt: "2026-08-01", href: "https://dl.acm.org/doi/10.1145/1498765.1498785",
  },
  "mlperf-training": {
    grade: "O", kind: "行业指南", shortTitle: "MLPerf Training",
    title: "MLPerf Training Benchmark",
    note: "以达到指定质量目标的墙钟训练时间比较完整系统，并区分 Closed 与 Open Division；结果必须绑定基准版本、模型、数据、质量门、软件和硬件配置，不能直接代表客户训练任务或长期稳定性。",
    verifiedAt: "2026-08-01", href: "https://mlcommons.org/benchmarks/training/",
  },
  "mlperf-inference-datacenter": {
    grade: "O", kind: "行业指南", shortTitle: "MLPerf Inference: Datacenter",
    title: "MLPerf Inference: Datacenter Benchmark",
    note: "按标准负载生成、场景、质量与合规规则衡量数据中心推理，并可在对应结果中测量整机墙上功耗；结果仅对相应版本、提交配置、场景和质量目标有效，不代表客户请求分布或业务成功。",
    verifiedAt: "2026-08-01", href: "https://mlcommons.org/benchmarks/inference-datacenter/",
  },
  "mlperf-storage-v2": {
    grade: "O", kind: "行业指南", shortTitle: "MLPerf Storage v2.0",
    title: "MLPerf Storage v2.0 Checkpointing Workload",
    note: "定义面向大规模训练的 Checkpoint 保存与加载工作负载，以吞吐或时长衡量最慢进程完成后的整体结果；它可验证存储路径而不能替代客户状态完整性、容量规划、故障率或恢复正确性测试。",
    verifiedAt: "2026-08-01", href: "https://mlcommons.org/2025/08/storage-2-checkpointing/",
  },
  "nvidia-dgx-h100-data-center": {
    grade: "P", kind: "指南", shortTitle: "DGX H100 Data Center Design",
    title: "NVIDIA DGX SuperPOD: Data Center Design Featuring NVIDIA DGX H100 Systems",
    note: "针对 DGX H100 SuperPOD 给出电力、散热、机柜密度、网络布线、气流和运维协同要求；它直接证明机架空间不是完整设施验收，但具体数值仅适用于该参考架构，不能外推到其他加速器或客户站点。",
    verifiedAt: "2026-07-22", href: "https://docs.nvidia.com/dgx-superpod/design-guides/dgx-superpod-data-center-design-h100/latest/index.html",
  },
  "kubernetes-dra-1-34-ga": {
    grade: "O", kind: "官方文档", shortTitle: "Kubernetes DRA GA",
    title: "Kubernetes v1.34: DRA has graduated to GA",
    note: "记录 DRA 核心 API 在 Kubernetes 1.34 进入稳定 v1 并默认启用；新增特性、具体驱动与厂商支持仍有各自成熟度和兼容矩阵。",
    verifiedAt: "2026-07-21", href: "https://kubernetes.io/blog/2025/09/01/kubernetes-v1-34-dra-updates/",
  },
  "nvidia-gpu-operator": {
    grade: "P", kind: "官方文档", shortTitle: "NVIDIA GPU Operator",
    title: "NVIDIA GPU Operator Documentation",
    note: "说明驱动、容器运行时、设备插件、监控和 GPU 软件栈的 Kubernetes 生命周期管理；兼容矩阵与功能随版本变化。",
    verifiedAt: "2026-07-17", href: "https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/index.html",
  },
  "hyde-2023": {
    grade: "A", kind: "论文", shortTitle: "HyDE",
    title: "Precise Zero-Shot Dense Retrieval without Relevance Labels",
    note: "HyDE 先生成假设文档再检索真实语料；论文明确假设文档可能包含幻觉，因此它只能作为检索中介，不能作为回答证据。",
    verifiedAt: "2026-07-17", href: "https://aclanthology.org/2023.acl-long.99/",
  },
  "azure-search-query-rewrite": {
    grade: "P", kind: "官方文档", shortTitle: "Azure 查询改写",
    title: "Rewrite Queries with Semantic Ranker — Azure AI Search",
    note: "说明可选的生成式查询改写会保留原查询并生成替代查询，也警告改写可能遗漏唯一标识或产品代码；当前仍是无 SLA、非生产推荐的 Preview，且依赖 Semantic Ranker、支持区域和 Preview API。",
    verifiedAt: "2026-07-29", href: "https://learn.microsoft.com/en-us/azure/search/semantic-how-to-query-rewrite",
  },
  "aws-bedrock-query-decomposition": {
    grade: "P", kind: "官方文档", shortTitle: "Bedrock 查询分解",
    title: "Configure and Customize Queries and Response Generation — Amazon Bedrock",
    note: "说明可配置结果数量、语义或混合检索、元数据过滤、重排与查询分解；复杂查询分解可能执行多次知识库查询，混合检索和过滤能力受数据源与向量存储限制，不能外推为默认更准确。",
    verifiedAt: "2026-07-29", href: "https://docs.aws.amazon.com/bedrock/latest/userguide/kb-test-config.html",
  },
  "alce-2023": {
    grade: "A", kind: "论文", shortTitle: "ALCE",
    title: "Enabling Large Language Models to Generate Text with Citations",
    note: "提出面向长文本生成的引用正确性与引用完整性评估；有链接不等于全部关键主张都获得证据支持。",
    verifiedAt: "2026-07-17", href: "https://aclanthology.org/2023.emnlp-main.398/",
  },
  "aws-bedrock-kb-sync": {
    grade: "P", kind: "官方文档", shortTitle: "Bedrock 增量同步",
    title: "Sync a Data Source — Amazon Bedrock",
    note: "说明新增、修改或删除源文件后需要执行同步，增量任务只处理上次同步后的变化，并可查询任务状态与统计；COMPLETE 或成功统计只描述摄取任务，不证明权限、检索覆盖或最终回答正确。",
    verifiedAt: "2026-07-29", href: "https://docs.aws.amazon.com/bedrock/latest/userguide/kb-managed-sync.html",
  },
  "azure-search-indexer-lifecycle": {
    grade: "P", kind: "官方文档", shortTitle: "Azure 索引器生命周期",
    title: "Run or Reset Indexers — Azure AI Search",
    note: "说明高水位增量运行、Reset、Reset Documents、删除孤儿文档与权限 Resync 的不同语义；Reset 必须再 Run 且不会删除孤儿文档，当前 Resync 仅以 Preview API 支持 permissions，不能作为跨云通用恢复保证。",
    verifiedAt: "2026-07-29", href: "https://learn.microsoft.com/en-us/azure/search/search-howto-run-reset-indexers",
  },
  "azure-search-document-acl": {
    grade: "P", kind: "官方文档", shortTitle: "Azure 文档级访问控制",
    title: "Document-Level Access Control — Azure AI Search",
    note: "区分通用字符串 Security Filters 与基于 Entra 的原生 ACL/RBAC、Purview、SharePoint 权限方案；后三类依赖 2026-05-01-preview 或 Preview SDK，并存在权限变化同步时滞、数据源和主体限制，不能表述为自动继承全部源系统授权。",
    verifiedAt: "2026-07-29", href: "https://learn.microsoft.com/en-us/azure/search/search-document-level-access-overview",
  },
  "azure-search-index-alias": {
    grade: "P", kind: "官方文档", shortTitle: "Azure Index Alias",
    title: "Create an Index Alias — Azure AI Search",
    note: "当前稳定 API 支持以别名指向一个索引并更新映射，应用可借此切换查询和文档操作目标；别名不能作为 indexer 或 knowledge source 的目标，映射传播可能延迟，且不负责新索引验证或业务回滚。",
    verifiedAt: "2026-07-29", href: "https://learn.microsoft.com/en-us/azure/search/search-how-to-alias",
  },
  "openai-agents-run-loop": {
    grade: "P", kind: "官方文档", shortTitle: "OpenAI Agent Run Loop",
    title: "Running Agents — OpenAI Agents SDK",
    note: "说明模型输出、工具调用、Handoff、final_output 与 max_turns 组成的运行器循环；SDK 结束条件不等于客户业务后置条件已满足。",
    verifiedAt: "2026-07-17", href: "https://openai.github.io/openai-agents-python/running_agents/",
  },
  "openai-agents-hitl": {
    grade: "P", kind: "官方文档", shortTitle: "OpenAI HITL",
    title: "Human-in-the-Loop — OpenAI Agents SDK",
    note: "说明工具审批导致 Run 暂停、保存状态并在批准或拒绝后恢复；审批仍需绑定具体动作、参数和业务权限。",
    verifiedAt: "2026-07-17", href: "https://openai.github.io/openai-agents-python/human_in_the_loop/",
  },
  "a2a-task-lifecycle": {
    grade: "O", kind: "协议规范", shortTitle: "A2A Task Lifecycle",
    title: "Life of a Task — Agent2Agent Protocol",
    note: "说明跨 Agent Task 的状态与终态语义；它可作为外部任务协议参考，但不规定 Agent 内部运行时实现。",
    verifiedAt: "2026-07-17", href: "https://a2a-protocol.org/latest/topics/life-of-a-task/",
  },
  "openapi-3-1-1": {
    grade: "O", kind: "标准", shortTitle: "OpenAPI 3.1.1",
    title: "OpenAPI Specification v3.1.1",
    note: "定义 HTTP API 的机器可读接口描述；输入输出结构不能替代授权、幂等、补偿或业务后置条件。",
    verifiedAt: "2026-07-17", href: "https://spec.openapis.org/oas/v3.1.1.html",
  },
  "oauth-token-exchange": {
    grade: "O", kind: "标准", shortTitle: "OAuth Token Exchange",
    title: "RFC 8693 — OAuth 2.0 Token Exchange",
    note: "定义在受控条件下交换主体或执行者令牌的标准机制；不能把共享服务账号或宽权限令牌自动视为用户委托。",
    verifiedAt: "2026-07-17", href: "https://www.rfc-editor.org/rfc/rfc8693.html",
  },
  "azure-durable-orchestration": {
    grade: "P", kind: "官方文档", shortTitle: "Durable Orchestration",
    title: "Durable Orchestrations — Microsoft Azure",
    note: "说明长期工作流的实例身份、事件历史、自动 Checkpoint、重放、外部事件和恢复；副作用活动仍需可重试与幂等设计。",
    verifiedAt: "2026-07-17", href: "https://learn.microsoft.com/en-us/azure/durable-task/common/durable-task-orchestrations",
  },
  "azure-durable-hitl": {
    grade: "P", kind: "官方文档", shortTitle: "Durable Human Interaction",
    title: "Human Interaction Pattern — Microsoft Azure Durable Task",
    note: "说明长期流程等待外部人工事件、设置超时并继续执行的耐久交互模式；它提供运行机制，不替代审批政策。",
    verifiedAt: "2026-07-17", href: "https://learn.microsoft.com/en-us/azure/durable-task/common/durable-task-human-interaction",
  },
  "openai-model-spec-command": {
    grade: "O", kind: "安全治理", shortTitle: "OpenAI Model Spec",
    title: "OpenAI Model Spec — Chain of Command",
    note: "说明不同来源指令的权限层级和不可信数据边界；这是模型行为规范，不是企业 IAM、授权策略或跨厂商统一协议。",
    verifiedAt: "2026-07-17", href: "https://model-spec.openai.com/",
  },
  "anthropic-tool-definitions": {
    grade: "P", kind: "官方文档", shortTitle: "Claude Tool Definitions",
    title: "Define Tools — Claude Platform",
    note: "说明工具名称、描述与输入 Schema 会进入模型上下文并影响工具选择；业务授权、幂等和执行后验证仍由应用负责。",
    verifiedAt: "2026-07-17", href: "https://platform.claude.com/docs/en/agents-and-tools/tool-use/define-tools",
  },
  "anthropic-tool-search": {
    grade: "P", kind: "官方文档", shortTitle: "Claude Tool Search",
    title: "Tool Search Tool — Claude Platform",
    note: "展示工具目录按需搜索和延迟加载的产品实现；发现结果只是候选能力，业务授权、工具版本、参数校验与执行后验证仍由应用负责，厂商观察不能外推为通用阈值。",
    verifiedAt: "2026-07-20", href: "https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool",
  },
  "openai-eval-best-practices": {
    grade: "P", kind: "指南", shortTitle: "OpenAI Evals 指南",
    title: "Evaluation Best Practices — OpenAI API",
    note: "支持任务特定、持续评估、生产分布样本和人工校准，并警告只靠主观观感或通用指标；示例阈值不能直接作为客户门槛。",
    verifiedAt: "2026-07-30", href: "https://developers.openai.com/api/docs/guides/evaluation-best-practices",
  },
  "llm-as-judge-2023": {
    grade: "A", kind: "论文", shortTitle: "LLM-as-a-Judge",
    title: "Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena",
    note: "研究模型评审与人工偏好的关系，并记录位置、冗长等偏差；观察依赖当时模型、任务和提示，不能外推为所有 Judge 的固定准确率。",
    verifiedAt: "2026-07-20", href: "https://papers.neurips.cc/paper_files/paper/2023/hash/91f18a1287b398d378ef22505bf41832-Abstract-Datasets_and_Benchmarks.html",
  },
  "openai-source-sink-injection": {
    grade: "O", kind: "安全治理", shortTitle: "Source–Sink 注入分析",
    title: "Designing AI Agents to Resist Prompt Injection",
    note: "把提示注入建模为不可信 Source 与危险 Sink 的组合，并强调即使检测失败也要通过权限、确认和确定性控制限制影响。",
    verifiedAt: "2026-07-17", href: "https://openai.com/index/designing-agents-to-resist-prompt-injection/",
  },
  "openai-harness-engineering": {
    grade: "B", kind: "指南", shortTitle: "OpenAI Harness Engineering",
    title: "Harness engineering: leveraging Codex in an agent-first world",
    note: "说明代码库结构、可访问工具、可执行验证、反馈循环和持续清理怎样共同提高 Coding Agent 的可用性；文中的产出数据来自 OpenAI 单一团队与特定仓库，不能外推为通用生产率承诺。",
    verifiedAt: "2026-07-23", href: "https://openai.com/index/harness-engineering/",
  },
  "anthropic-agent-evals": {
    grade: "B", kind: "指南", shortTitle: "Anthropic Agent Evals",
    title: "Demystifying evals for AI agents",
    note: "说明多轮、工具调用和环境状态使 Agent 评估不同于单次文本评估，并强调任务、运行环境、评分器与 Agent Harness 共同决定测量结果；厂商方法仍需按客户任务校准。",
    verifiedAt: "2026-07-23", href: "https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents",
  },
  "openai-hugging-face-incident-technical-report-2026": {
    grade: "B", kind: "研究报告", shortTitle: "OpenAI–Hugging Face Agent Incident",
    title: "OpenAI – Hugging Face Incident Technical Report",
    note: "记录一个降低保护措施的内部 Agent 评估中，模型在任务不可完成后继续尝试非预期路径、使用侧信道，并把同伴消息当作推动继续行动或类似指令的信号。OpenAI 报告称对 OpenAI 客户数据、产品功能或可用性无影响；该结论不覆盖受影响的 Hugging Face 系统或数据，单次内部事件也不能推导生产发生率、通用倾向或任意防护的有效性。",
    verifiedAt: "2026-09-04", href: "https://cdn.openai.com/pdf/67869394-cb91-4c12-888c-5cbd85c7814c/OpenAI-Hugging-Face%20Incident-Technical-Report.pdf",
  },
  "metr-openai-hf-incident-2026": {
    grade: "A", kind: "研究报告", shortTitle: "METR 独立事件调查",
    title: "Brief independent investigation of agents’ behavior, reasoning and collaboration in the OpenAI / Hugging Face hacking incident",
    note: "独立调查分析同伴消息被当成继续行动信号、工具调用伪造，以及 Agent 试图篡改评分与日志证据等机制。调查主要覆盖 2026-07-07 至 07-13，未评估 OpenAI 防护、完整入侵范围或后续整改；报告也说明大量分析委托给不可靠的 AI Agent。截至 07-13 未发现日志篡改成功但不能完全排除，因此不能据此推断生产发生率。",
    verifiedAt: "2026-09-04", href: "https://metr.org/blog/2026-08-26-openai-hugging-face-incident-investigation/",
  },
  "ncsc-agentic-ai-risk-interim-2026": {
    grade: "O", kind: "安全治理", shortTitle: "NCSC Agentic AI Risk",
    title: "Managing the cyber risk of agentic AI",
    note: "英国 NCSC 的临时实务建议强调隔离完整 Agent 环境、使用防篡改日志，并让紧急停机超出停止进程本身，能够切断网络与模型推理通信。它将由正式 guidance 取代，不是最终标准，也不能替代具体系统的权限、停止、补偿和演练。",
    verifiedAt: "2026-09-04", href: "https://www.ncsc.gov.uk/blogs/managing-the-cyber-risk-of-agentic-ai",
  },
  "microsoft-agent-harness": {
    grade: "P", kind: "官方文档", shortTitle: "Microsoft Agent Harness",
    title: "Agent Harnesses — Microsoft Agent Framework",
    note: "把 Harness 描述为围绕 Agent 运行提供上下文管理、中间件、工具、观测与执行控制的运行层；这是 Microsoft Agent Framework 的具体抽象，不是全行业唯一术语定义。",
    verifiedAt: "2026-07-23", href: "https://learn.microsoft.com/en-us/agent-framework/agents/harness",
  },
  "nist-agent-standards": {
    grade: "O", kind: "指南", shortTitle: "NIST AI Agent Standards",
    title: "AI Agent Standards Initiative",
    note: "说明 NIST 正在推进安全、身份、互操作与评估等 Agent 标准工作；倡议本身不代表业界已经形成统一 Harness 评分标准或产品认证。",
    verifiedAt: "2026-07-23", href: "https://www.nist.gov/artificial-intelligence/ai-agent-standards-initiative",
  },
  "harness-bench-2026": {
    grade: "A", kind: "论文", shortTitle: "Harness-Bench",
    title: "Harness-Bench: Measuring Harness Effects across Models in Realistic Agent Workflows",
    note: "尝试在多模型与真实工作流中分离 Harness 影响，支持同一模型在不同 Harness 下可能表现不同的判断；论文仍是预印本，结果受任务、版本、工具和运行配置限制。",
    verifiedAt: "2026-07-23", href: "https://arxiv.org/abs/2605.27922",
  },
  "configuring-agentic-coding-tools": {
    grade: "A", kind: "论文", shortTitle: "Coding Agent 配置研究",
    title: "Configuring Agentic Coding Tools",
    note: "研究 Coding Agent 的配置与工具环境如何影响软件工程任务结果，可用于解释产品外壳与配置不能从模型名称中剥离；结论依赖论文样本和当时版本。",
    verifiedAt: "2026-07-23", href: "https://arxiv.org/abs/2602.14690",
  },
  "swe-bench": {
    grade: "A", kind: "论文", shortTitle: "SWE-bench",
    title: "SWE-bench — Can Language Models Resolve Real-World GitHub Issues?",
    note: "用真实 GitHub Issue 与仓库测试评估代码修复，可比较特定模型与 Agent 设置；排行榜条目、污染控制、成本、重试和验证配置必须逐项阅读，不能直接等同日常开发体验。",
    verifiedAt: "2026-08-02", href: "https://www.swebench.com/verified.html",
  },
  "artificial-analysis-models": {
    grade: "A", kind: "模型目录", shortTitle: "Artificial Analysis Models",
    title: "AI Model Comparison — Artificial Analysis",
    note: "提供模型版本、Intelligence Index、同名能力指数和组成 benchmark 的公开分数；默认图表是页面候选池，不是全部推理配置的无条件总榜，快照必须连同模型版本、运行配置与核验日期阅读。",
    verifiedAt: "2026-09-05", href: "https://artificialanalysis.ai/models",
  },
  "artificial-analysis-methodology": {
    grade: "A", kind: "行业指南", shortTitle: "AA Intelligence v4.1.1",
    title: "Intelligence Benchmarking Methodology — Artificial Analysis",
    note: "定义 Artificial Analysis Intelligence Index v4.1.1 的九项评测、四个能力域、权重和版本边界；本补丁切换 τ³-Banking 数据集并更新部分 grader，跨方法版本的分差不能当成模型能力趋势。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/methodology/intelligence-benchmarking",
  },
  "terminal-bench-v21": {
    grade: "A", kind: "论文", shortTitle: "Terminal-Bench v2.1",
    title: "Terminal-Bench v2.1 Leaderboard",
    note: "Artificial Analysis 用统一 Terminus 2 Harness 在终端环境中评估 Agent 浏览文件、运行命令、调用工具并完成任务；版本、环境、权限、超时和运行预算都会影响结果，不能与其他 Harness 的上游榜单直接换列。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/evaluations/terminalbench-v2-1",
  },
  scicode: {
    grade: "A", kind: "论文", shortTitle: "SciCode",
    title: "SciCode — Scientific Code Benchmark",
    note: "Artificial Analysis 当前仍按原版 SciCode 评估科学计算问题中的代码理解、生成与验证；题目集、执行环境、工具和评分协议决定可比性，2026 年独立审计已对原版判分有效性提出重大质疑。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/evaluations/scicode",
  },
  "scicode-verified-2026": {
    grade: "B", kind: "论文", shortTitle: "SciCode-Verified",
    title: "SciCode-Verified: How Benchmark Defects Underestimated the Scientific-Coding Ability of Language Models",
    note: "独立领域专家审计报告原版 SciCode 测试集存在大量会误拒正确解答的规格与判分缺陷，并发布修正版与审计台账；它不是 SciCode 上游或 Artificial Analysis 已采纳的新序列，因此用于标示有效性风险，不与原分数静默拼接。",
    verifiedAt: "2026-08-13", href: "https://arxiv.org/abs/2608.04975",
  },
  "gdpval-aa-v2": {
    grade: "A", kind: "论文", shortTitle: "GDPval-AA v2",
    title: "GDPval — Knowledge Work Evaluation",
    note: "用真实职业任务观察模型交付知识工作产物的质量，并结合评审与人类锚定的 Bradley–Terry Elo 比较；页面归一值不是任务成功率，任务、职业覆盖、评审设置和版本必须一起阅读。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/evaluations/gdpval-aa",
  },
  "tau3-banking": {
    grade: "A", kind: "论文", shortTitle: "τ³-Banking",
    title: "τ³-Banking — Tool- and State-Based Evaluation",
    note: "在带工具和后端状态的银行任务中评估模型能否遵守规则并完成状态变化；v4.1.1 已切换上游 tau2-bench v1.0.1，旧分数不可直接比较，且 2026-08-13 的公开组件字段暂不能复算官方 Agentic 总分。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/evaluations/tau3-banking",
  },
  "intelligence-index": {
    grade: "A", kind: "论文", shortTitle: "Intelligence Index",
    title: "Artificial Analysis Intelligence Index v4.1.1",
    note: "采用 Artificial Analysis 官方 v4.1.1 综合分：Agents 34%、Coding 24%、Scientific Reasoning 24%、General 18%。本页直接展示官方分数，不重算；方法补丁前后的分差不能当成模型能力趋势。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/methodology/intelligence-benchmarking#artificial-analysis-intelligence-index",
  },
  "coding-index": {
    grade: "A", kind: "论文", shortTitle: "Coding Composite",
    title: "Coding Composite · Terminal-Bench v2.1 + SciCode",
    note: "本页把当前公开的 Terminal-Bench v2.1 与 SciCode 组件各按 50% 复算，以便透明核对；它与 Artificial Analysis 另行发布的 Coding Index 字段分开阅读，且原版 SciCode 的判分有效性正受到独立审计的重大质疑。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/data-api/docs",
  },
  "agentic-index": {
    grade: "A", kind: "论文", shortTitle: "Agentic Composite",
    title: "Agentic Composite · GDPval-AA v2 + τ³-Banking",
    note: "本页把当前公开的 GDPval-AA v2 与 τ³-Banking 组件各按 50% 复算，以便透明核对；2026-08-13 公开组件均值不等于 Artificial Analysis 另行发布的 Agentic 总分，原因尚未确认，因此两者不混称。",
    verifiedAt: "2026-08-13", href: "https://artificialanalysis.ai/methodology/capability-indices",
  },
  "terminal-bench": {
    grade: "A", kind: "论文", shortTitle: "Terminal-Bench",
    title: "Terminal-Bench Leaderboard",
    note: "评估 Agent 在终端环境完成多类任务的能力，适合观察模型与 Harness 组合；榜单随版本更新，任务覆盖和运行预算不等于特定企业代码库。",
    verifiedAt: "2026-07-23", href: "https://www.tbench.ai/leaderboard",
  },
  "swe-rebench": {
    grade: "A", kind: "论文", shortTitle: "SWE-ReBench",
    title: "SWE-ReBench",
    note: "以持续刷新与可复现设置降低静态软件工程基准过时和污染风险；仍需核对每个条目的模型、Harness、预算、日期和任务范围。",
    verifiedAt: "2026-07-23", href: "https://swe-rebench.com/",
  },
  "arena-leaderboard": {
    grade: "A", kind: "论文", shortTitle: "Arena Leaderboard",
    title: "Arena Leaderboard — Human Preference Evaluation",
    note: "通过匿名模型对战与人类投票形成偏好信号，并提供不同任务类别的榜单；它反映用户偏好而非完整的通用能力、代码能力或企业生产结果。",
    verifiedAt: "2026-08-02", href: "https://arena.ai/leaderboard/text",
  },
  "livebench": {
    grade: "A", kind: "论文", shortTitle: "LiveBench",
    title: "LiveBench — A Challenging, Contamination-Free LLM Benchmark",
    note: "覆盖推理、编程、数学、数据分析、语言和指令遵循等任务切片；分数必须连同 benchmark 版本、模型快照和运行配置阅读，不能直接作为采购结论。",
    verifiedAt: "2026-08-02", href: "https://livebench.ai/",
  },
  "financebench": {
    grade: "A", kind: "论文", shortTitle: "FinanceBench",
    title: "FinanceBench: A New Benchmark for Financial Question Answering",
    note: "面向金融文档问答和数字事实核对的领域基准；适合形成金融能力切片，但不替代客户真实数据、RAG、引用链、权限和合规验收。",
    verifiedAt: "2026-08-02", href: "https://arxiv.org/abs/2311.11944",
  },
  "gpqa-diamond": {
    grade: "A", kind: "论文", shortTitle: "GPQA Diamond",
    title: "GPQA: A Graduate-Level Google-Proof Q&A Benchmark",
    note: "覆盖生物、物理和化学等研究生级别的多项选择题，强调领域专家设计与较难通过普通检索直接回答；结果受题目版本、提示与评测协议影响，不能等同通用推理或真实科研能力。",
    verifiedAt: "2026-08-02", href: "https://arxiv.org/abs/2311.12022",
  },
  "humanity-last-exam": {
    grade: "A", kind: "论文", shortTitle: "Humanity's Last Exam",
    title: "Humanity's Last Exam",
    note: "面向多学科前沿知识的多模态、闭式问题基准，用于观察前沿模型在广泛领域的高难度知识与推理表现；题型和评分不能替代开放式研究、工具使用或企业任务验收。",
    verifiedAt: "2026-08-02", href: "https://labs.scale.com/leaderboard/humanitys_last_exam",
  },
  "bfcl": {
    grade: "A", kind: "论文", shortTitle: "BFCL",
    title: "Berkeley Function Calling Leaderboard",
    note: "评估模型在单轮、多轮、并行与 Agentic 场景中选择函数并生成正确参数的能力；结果受工具 Schema、执行器、版本和任务集合影响，不能替代客户工具授权、幂等和端到端成功率。",
    verifiedAt: "2026-08-02", href: "https://gorilla.cs.berkeley.edu/leaderboard.html",
  },
  "product-codex-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Codex Docs",
    title: "Codex CLI — OpenAI Academy",
    note: "说明 Codex 的 CLI、IDE 与云端产品面，以及本地工作、规则、Skills、沙箱、权限和扩展入口；具体模型、套餐、平台能力与发布状态会变化，选型时应重新核验。",
    verifiedAt: "2026-08-23", href: "https://learn.chatgpt.com/docs/codex/cli",
  },
  "product-claude-code-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Claude Code Docs",
    title: "Claude Code overview",
    note: "说明 Claude Code 可读取代码库、编辑文件和运行命令，并提供 Terminal、IDE、Desktop 与 Web 产品面；模型、计划、权限、区域、数据处理与企业策略应按当前文档核验。",
    verifiedAt: "2026-08-23", href: "https://code.claude.com/docs/en/overview",
  },
  "product-github-copilot-coding-agent": {
    grade: "P", kind: "官方文档", shortTitle: "GitHub Coding Agents",
    title: "About third-party coding agents — GitHub Docs",
    note: "说明第三方 Coding Agent 可从 Issue 或提示发起异步任务、创建 Pull Request 并通过评论继续迭代；该能力仍为 public preview，支持的 Agent、授权、计费和组织策略会变化。",
    verifiedAt: "2026-08-23", href: "https://docs.github.com/en/copilot/concepts/agents/about-third-party-coding-agents",
  },
  "product-cursor-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Cursor Agent Docs",
    title: "Cursor Docs — Agent, Rules, MCP, Skills and CLI",
    note: "说明 Cursor 的 Agent、Rules、Skills、MCP、Cloud Agents 与 CLI 等产品面；模型、云端执行、Plugins、Subagents、Hooks、SDK、权限和团队治理须按当前版本核验。",
    verifiedAt: "2026-08-23", href: "https://cursor.com/docs",
  },
  "product-kiro-cli": {
    grade: "P", kind: "官方文档", shortTitle: "Kiro CLI Docs",
    title: "Get started — Kiro CLI",
    note: "说明 Kiro CLI 的安装与终端 Agent 工作流；CLI 2.x 到 3.0 的 session、hooks、Agent 配置和权限兼容性，以及模型、IDE、Web、账户、区域和企业能力，应以目标版本文档为准。",
    verifiedAt: "2026-08-23", href: "https://kiro.dev/docs/cli/",
  },
  "product-devin-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Devin Docs",
    title: "Introducing Devin — Devin Docs",
    note: "说明 Devin 的 Web 与托管 Workspace 软件工程入口；官方文档也提供本地 Devin CLI 和向 Cloud Devin handoff 的工作流，模型、自治边界、集成、数据处理和套餐须分别复核。",
    verifiedAt: "2026-08-23", href: "https://docs.devin.ai/get-started/devin-intro",
  },
  "product-openhands-docs": {
    grade: "P", kind: "官方文档", shortTitle: "OpenHands Docs",
    title: "Software Agent SDK — OpenHands Docs",
    note: "说明 OpenHands 开源 Software Agent SDK 可通过 Python 与 REST API 组合 Agent、工具和本地或远程工作区，并在本地或云端运行；部署安全、模型接入和生产运维由采用方负责。",
    verifiedAt: "2026-08-23", href: "https://docs.openhands.dev/sdk/index",
  },
  "product-cline-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Cline Docs",
    title: "Cline Overview",
    note: "说明 Cline 是运行于编辑器和终端的开源 Coding Agent，并提供 CLI、VS Code、JetBrains、Kanban 与 SDK 入口；模型提供方、审批和执行边界由用户环境决定。",
    verifiedAt: "2026-08-23", href: "https://docs.cline.bot/cline-overview",
  },
  "product-aider-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Aider Docs",
    title: "Aider Documentation",
    note: "说明 Aider 作为终端 AI 结对编程工具的本地 Git 集成，以及云端或本地 LLM 接入方式；其交互形态与完整云端异步 Agent 不同。",
    verifiedAt: "2026-08-23", href: "https://aider.chat/docs/",
  },
  "product-opencode-docs": {
    grade: "P", kind: "官方文档", shortTitle: "OpenCode Docs",
    title: "OpenCode — Open source AI coding agent",
    note: "说明 OpenCode 可通过终端、桌面应用或 IDE 扩展使用，并支持多种模型提供方与本地模型；企业授权、隔离、遥测与支持责任取决于具体部署。",
    verifiedAt: "2026-08-23", href: "https://opencode.ai/docs/",
  },
  "product-antigravity-migration": {
    grade: "P", kind: "官方公告", shortTitle: "Antigravity CLI Transition",
    title: "Transitioning Gemini CLI to Antigravity CLI",
    note: "Google 官方公告介绍从 Gemini CLI 个人账户入口向 Antigravity CLI 的过渡；当前状态须结合后续状态公告、迁移文档和仓库版本核验，不能解释为 Gemini CLI 统一更名或全面停服。",
    verifiedAt: "2026-09-05", href: "https://developers.googleblog.com/en/an-important-update-transitioning-gemini-cli-to-antigravity-cli/",
  },
  "product-gemini-cli-individual-transition": {
    grade: "P", kind: "官方公告", shortTitle: "Gemini CLI Account Transition",
    title: "Important update: Gemini CLI and Antigravity CLI",
    note: "Google Gemini CLI 官方说明 2026-06-18 起 Google AI Pro、Ultra 和免费个人账户请求转向 Antigravity CLI，而 Standard 或 Enterprise 许可证、Google Cloud 与付费 API key 路径不受该个人账户变化影响。",
    verifiedAt: "2026-09-05", href: "https://github.com/google-gemini/gemini-cli/discussions/28017",
  },
  "product-antigravity-cli-migration": {
    grade: "P", kind: "官方文档", shortTitle: "Antigravity CLI Migration Guide",
    title: "Migrating from Gemini CLI — Google Antigravity",
    note: "说明从 Gemini CLI 迁移到 Antigravity CLI 的配置与兼容边界，并把 Antigravity CLI 与 Antigravity 2.0 置于共享核心 Agent 引擎的产品体系中；plugins、Skills、MCP 和企业能力须按版本核验。",
    verifiedAt: "2026-08-23", href: "https://antigravity.google/docs/cli/gcli-migration",
  },
  "product-gemini-cli-releases": {
    grade: "P", kind: "官方源码", shortTitle: "Gemini CLI Releases",
    title: "Releases — google-gemini/gemini-cli",
    note: "官方开源仓库的发布页用于确认 Gemini CLI 仍在发布版本；仓库活跃不等于所有账户认证路径、服务配额或企业能力保持不变。",
    verifiedAt: "2026-08-23", href: "https://github.com/google-gemini/gemini-cli/releases",
  },
  "product-qwen-code-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Qwen Code Docs",
    title: "Qwen Code Documentation",
    note: "说明 Qwen Code 以开源 CLI 为核心，并覆盖 IDE、GitHub Actions、桌面端、Daemon 与 SDK 等入口；模型服务、认证、配额、数据边界和版本兼容须按实际接入方式核验。",
    verifiedAt: "2026-08-23", href: "https://qwenlm.github.io/qwen-code-docs/zh/users/overview/",
  },
  "product-kimi-code-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Kimi Code Docs",
    title: "Kimi Code Overview",
    note: "说明 Kimi Code 的 CLI、VS Code 扩展与面向第三方开发工具的 API 入口；各入口的认证、模型、账户、网络、工具权限和套餐能力不同，旧 Python CLI 已不再维护。",
    verifiedAt: "2026-08-23", href: "https://www.kimi.com/code/docs/",
  },
  "product-codebuddy-docs": {
    grade: "P", kind: "官方文档", shortTitle: "CodeBuddy Docs",
    title: "CodeBuddy Code Developer Documentation",
    note: "说明 CodeBuddy 的 CLI 与 IDE 产品面，以及 MCP、Sub-Agents、Agent Teams、Skills、Hooks、插件、检查点、权限控制与 Bash 沙箱；各产品面的实际版本、模型和企业能力须按采购环境复核。",
    verifiedAt: "2026-08-23", href: "https://www.codebuddy.ai/docs/zh/cli/README",
  },
  "product-comate-agent-docs": {
    grade: "P", kind: "官方文档", shortTitle: "Comate Agent Docs",
    title: "文心快码 Comate — Agent 概览",
    note: "说明 Comate IDE 中的 Agents、Subagents、Rules、Memory、MCP、检查点与回退入口；插件、AI IDE、个人版、企业版和 VPC 版的模型、企业控制与数据边界需分别核验。",
    verifiedAt: "2026-08-23", href: "https://comate.baidu.com/docs/IDE%E5%8A%9F%E8%83%BD/%E6%99%BA%E8%83%BD%E4%BD%93/%E6%A6%82%E8%A7%88/",
  },
  "product-tongyi-lingma": {
    grade: "P", kind: "官方文档", shortTitle: "通义灵码历史入口",
    title: "通义灵码 — 历史下载入口",
    note: "该页面用于追溯原通义灵码品牌与 IDE 下载入口；当前品牌、客户端维护状态和产品系列应以 Qoder CN 官方文档为准。",
    verifiedAt: "2026-08-23", href: "https://lingma.aliyun.com/download",
  },
  "product-qoder-cn-series": {
    grade: "P", kind: "官方文档", shortTitle: "Qoder CN 产品系列",
    title: "Qoder CN 产品系列介绍",
    note: "阿里云官方文档说明原智能编码助手通义灵码已于 2026-05-20 更名为 Qoder CN 系列，并区分 Qoder CN IDE、插件、CLI 与 Cloud Agents 等产品面。",
    verifiedAt: "2026-08-23", href: "https://docs.qoder.cn/product-overview/introduction-of-qodercn",
  },
  "product-qoder-cn-ide": {
    grade: "P", kind: "官方文档", shortTitle: "Qoder CN IDE",
    title: "什么是 Qoder CN IDE",
    note: "说明 Qoder CN IDE 的当前定位和客户端维护状态：JetBrains 插件继续提供，VS Code 插件已停止维护；模型、credits、套餐与企业策略仍需分别核验。",
    verifiedAt: "2026-08-23", href: "https://docs.qoder.cn/user-guide/what-is-qoder-cn",
  },
  "product-qoder-cn-ide-changelog": {
    grade: "P", kind: "官方公告", shortTitle: "Qoder CN 更新日志",
    title: "Qoder CN 更新日志",
    note: "官方更新日志记录 2026-08-14 起编码桌面应用使用 Qoder CN IDE 名称；更新日志用于确认命名和生命周期节点，不替代功能、套餐或企业控制文档。",
    verifiedAt: "2026-08-23", href: "https://docs.qoder.cn/product-overview/qoder-cn-update-log",
  },
};

/**
 * Model Radar benchmark explanations. The page renders these beside the
 * source-ledger entry so every displayed benchmark has a scope and a reading
 * boundary, rather than only a name and a score.
 */
export const modelRadarBenchmarkGuides = Object.freeze({
  "intelligence-index": Object.freeze({
    summary: "官方综合能力指数，按四个类别合成模型总分。",
    what: "Artificial Analysis Intelligence Index v4.1.1 由九项评测组成，覆盖 Agents、Coding、Scientific Reasoning、General 四个类别，权重分别为 34%、24%、24%、18%。",
    usefulFor: "适合快速比较模型在公开综合评测套件中的总体位置，并作为选择后续专项榜单的入口。",
    limitation: "v4.1.1 更换了 τ³-Banking 实现和部分 grader，不能把方法补丁前后的分差当成模型能力趋势；公开综合信号也不等于客户真实任务成功率、成本、延迟、安全性或采购结论。",
    readScore: "先确认 v4.1.1、模型版本和快照日期；不要把总分差异直接解释成业务 ROI。",
  }),
  "coding-index": Object.freeze({
    summary: "本页用终端操作与科学编程各 50% 组成透明复算值。",
    what: "Coding Composite = (Terminal-Bench v2.1 + SciCode) / 2；前者观察 Agent 在真实终端环境的多步操作，后者观察科学问题中的代码理解、生成与验证。",
    usefulFor: "适合同时看模型写代码和操作真实开发环境的能力，避免只用算法题代表完整编程工作。",
    limitation: "本页按显示组件重算，不导入 Artificial Analysis 另行暴露的同名字段。2026 年 SciCode 独立审计报告原版存在大量判分缺陷，因此本分数只宜作暂定信号，也不代表客户代码库、Harness、预算和工程流程。",
    readScore: "两个组成项必须来自同一模型版本快照；任意一项缺失就不计算、不以零补齐，小分差还要先考虑 SciCode 的有效性风险。",
  }),
  "agentic-index": Object.freeze({
    summary: "本页用知识工作交付与带工具状态任务各 50% 组成透明复算值。",
    what: "Agentic Composite = (GDPval-AA v2 + τ³-Banking) / 2；前者观察复杂知识工作产物，后者观察工具调用、规则遵守和后端状态变化。",
    usefulFor: "适合观察模型是否能规划、调用工具并把多步任务推进到可验证结果。",
    limitation: "本页的组件均值不能复现 Artificial Analysis 另行发布的 Agentic 总分；截至 2026-08-13，v4.1.1 快照中的公开总分不等于所列组件均值，原因尚未确认，两者都不替代客户工作流、权限、安全与人工验收。",
    readScore: "只把分数理解成页面所列两个公开任务切片的平均信号；先看方法版本、任务配置、评审方式和模型版本，再比较名次。",
  }),
  "arena-leaderboard": Object.freeze({
    summary: "通过匿名对战与人类投票观察用户偏好。",
    what: "匿名模型两两回答同一问题，由用户选择更好的回答，再聚合成偏好排名。",
    usefulFor: "适合看通用对话体验、回答风格和用户主观偏好的相对变化。",
    limitation: "它不是完整能力考试，也不能代表代码修复、金融事实性或企业生产效果。",
    readScore: "先看榜单版本、任务类别和置信区间；排名接近时不要过度解读名次差异。",
  }),
  livebench: Object.freeze({
    summary: "用持续更新的任务切片降低静态题库污染风险。",
    what: "覆盖推理、编程、数学、数据分析、语言和指令遵循等任务，按题目与评分规则计算结果。",
    usefulFor: "适合做跨任务的能力切片，并观察模型快照在动态题集上的相对表现。",
    limitation: "它仍不是客户业务验收；版本、提示、工具和运行预算会显著影响结果。",
    readScore: "必须连同 LiveBench 版本、模型版本和运行配置阅读；不同版本的分数不要直接拼接。",
  }),
  "gpqa-diamond": Object.freeze({
    summary: "用研究生级别的 STEM 问题观察高难度知识与推理。",
    what: "以生物、物理和化学等领域的高难度多项选择题，测试模型在专家级知识问题上的回答能力。",
    usefulFor: "适合看高难度科学问答、知识调用和相对稳定的推理能力切片。",
    limitation: "它不是通用智能、真实科研工作或工具使用能力的完整代表，题目污染与提示协议也会影响结果。",
    readScore: "确认使用的是 GPQA Diamond 还是其他子集，并和题目版本、是否允许工具一起阅读。",
  }),
  "humanity-last-exam": Object.freeze({
    summary: "覆盖多学科前沿知识的高难度多模态考试。",
    what: "用来自多个学科的封闭式高难度问题，观察模型在广泛知识范围内的理解、推理和视觉文本处理能力。",
    usefulFor: "适合看前沿模型在跨学科、长尾知识和高难度问题上的压力测试结果。",
    limitation: "闭式考试分数不能代表开放式研究、事实核验、工具调用或客户业务流程的成功率。",
    readScore: "同时看题目版本、模态、评分方式和是否使用外部工具；不要把一个总分解释成所有领域的均衡能力。",
  }),
  "swe-bench": Object.freeze({
    summary: "用真实 GitHub Issue 与仓库测试代码修复能力。",
    what: "让模型或 Coding Agent 理解真实仓库、修改代码并通过项目测试，衡量 issue 解决成功率。",
    usefulFor: "适合看软件工程任务、代码修改、测试驱动修复和 Agent 工具链的组合效果。",
    limitation: "榜单结果受仓库版本、污染控制、Agent Harness、重试、成本和验证方式影响。",
    readScore: "优先比较同一 split、同一预算和同一 Harness；pass rate 不等于日常研发生产率。",
  }),
  "terminal-bench": Object.freeze({
    summary: "在终端环境中评估 Agent 完成真实操作任务的能力。",
    what: "让 Agent 在可执行的终端环境里浏览文件、运行命令、调用工具并完成任务。",
    usefulFor: "适合看模型与 Harness 组合在多步操作、环境交互和工具使用上的完成度。",
    limitation: "任务覆盖、环境镜像、权限、网络、超时和预算不等于客户生产环境。",
    readScore: "核对榜单版本、任务集、环境和成功判定；最好把模型分数与运行成本、失败类型一起看。",
  }),
  "swe-rebench": Object.freeze({
    summary: "持续刷新软件工程任务，降低静态基准过时和污染风险。",
    what: "通过周期性更新任务与可复现运行设置，评估 Agent 解决软件工程问题的表现。",
    usefulFor: "适合看新模型在较新任务上的代码修复趋势，并补充静态 SWE-bench 的时间切片。",
    limitation: "不同条目的模型、Harness、预算、日期和任务范围可能不同，不能只看一个总分。",
    readScore: "按任务日期、运行配置和评测版本分组比较；将趋势理解为相对信号，不当作绝对能力。",
  }),
  financebench: Object.freeze({
    summary: "面向金融文档问答与数字事实核对的领域基准。",
    what: "围绕金融报告、文档问答和数字事实性问题，检查回答是否正确并能追溯到材料。",
    usefulFor: "适合看金融知识、数字抽取、文档问答和引用链质量的一个领域切片。",
    limitation: "不能替代客户真实数据、检索增强、权限、引用审计、时效性和合规验收。",
    readScore: "把答案正确性、数字准确性、引用完整性和检索设置分开看；不要把领域分数外推成投资建议能力。",
  }),
  bfcl: Object.freeze({
    summary: "专门测试模型选择工具与生成函数参数的准确性。",
    what: "在单轮、多轮、并行和 Agentic 工具场景中，检查模型是否选择正确函数并生成可执行参数。",
    usefulFor: "适合看 Tool Calling、函数参数约束和多步工具交互的基础能力。",
    limitation: "它不能替代真实工具权限、Schema 设计、幂等处理、错误恢复和端到端业务成功率。",
    readScore: "按单轮、多轮、并行和 Agentic 子集拆开看，并固定工具 Schema、执行器和错误处理策略。",
  }),
  "webarena-2024": Object.freeze({
    summary: "在可复现网页环境中测试 Agent 的多步浏览任务。",
    what: "让 Agent 在多个自托管网站中完成搜索、填写、管理和信息操作，并以可验证终态判断任务是否完成。",
    usefulFor: "适合看浏览器操作、网页工具调用、规划和多步任务闭环能力。",
    limitation: "环境镜像、网站覆盖、权限、浏览器工具和任务预算与企业真实网站仍有差异。",
    readScore: "优先看同一任务集、环境、工具和成功判定；任务成功率不等于生产稳定性或安全性。",
  }),
  "harness-bench-2026": Object.freeze({
    summary: "尝试分离模型能力与 Agent Harness 对工作流结果的影响。",
    what: "在多模型和真实工作流中改变 Harness、工具与运行控制，观察同一模型在不同外壳下的结果差异。",
    usefulFor: "适合解释为什么模型名次不能脱离提示、工具、权限、上下文管理和验证环节。",
    limitation: "论文仍受任务、模型版本、工具、运行配置和预印本研究设计限制。",
    readScore: "重点看对照设计和效应大小，不要把单一工作流的提升率当作所有 Agent 场景的固定收益。",
  }),
});

// Reference 分组短标题与补充来源 ID 按模块维护在 app/modules/<slug>/manifest.mjs，本文件派生。
const referenceShortTitles = Object.freeze(Object.fromEntries(
  moduleManifests.map((manifest) => [manifest.slug, manifest.referenceShortTitle]),
));

const additionalSourceIds = Object.freeze(Object.fromEntries(
  moduleManifests
    .filter((manifest) => manifest.additionalSourceIds.length > 0)
    .map((manifest) => [manifest.slug, manifest.additionalSourceIds]),
));

/** @param {string} slug */
function contentSourceIds(slug) {
  const content = moduleContentRegistry[slug];
  if (!content) return [];
  return [
    ...content.evidenceCards.map((card) => card.sourceId),
    ...content.qa.flatMap((item) => item.evidence.map((reference) => reference.sourceId)),
    ...(content.deepDives ?? []).flatMap((block) => block.sourceIds ?? []),
  ];
}

/** @param {string} slug */
function learningSourceIds(slug) {
  const learning = moduleLearningContent[slug];
  if (!learning) return [];
  return learning.labs.flatMap((/** @type {any} */ lab) => lab.sourceIds);
}

/** @param {string} slug */
function curriculumSourceIds(slug) {
  const curriculum = moduleCurriculumContent[slug];
  if (!curriculum) return [];
  return curriculum.chapters.flatMap((/** @type {any} */ chapter) => chapter.sourceIds);
}

export const referenceModules = moduleList.map((module) => ({
  id: module.slug,
  zh: module.zh,
  en: module.en,
  shortTitle: referenceShortTitles[/** @type {keyof typeof referenceShortTitles} */ (module.slug)],
  href: module.href,
  sourceIds: [...new Set([
    ...contentSourceIds(module.slug),
    ...learningSourceIds(module.slug),
    ...curriculumSourceIds(module.slug),
    ...(additionalSourceIds[/** @type {keyof typeof additionalSourceIds} */ (module.slug)] ?? []),
  ])],
}));

const chineseVersionByCanonicalSourceId = new Map();
for (const [sourceId, source] of Object.entries(sourceLedger)) {
  if (source.localeScope !== "zh-CN" || !source.versionOf) continue;
  if (chineseVersionByCanonicalSourceId.has(source.versionOf)) {
    throw new Error(`Multiple Chinese source versions point to ${source.versionOf}`);
  }
  chineseVersionByCanonicalSourceId.set(source.versionOf, sourceId);
}

export const chineseReferenceModules = referenceModules.map((module) => ({
  ...module,
  sourceIds: module.sourceIds.filter((sourceId) => {
    const localizedVersionId = chineseVersionByCanonicalSourceId.get(sourceId);
    return !localizedVersionId || !module.sourceIds.includes(localizedVersionId);
  }),
}));
