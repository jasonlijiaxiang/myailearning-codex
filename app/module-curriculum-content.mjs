...
const baseModuleCurriculumContent = Object.freeze({
  ...
  "solution-patterns": {
    ...
    chapters: [
      ...
      { title: "把非功能要求写成约束包络", en: "Constraint Envelope", explanation: "为质量、P95/P99 时延、吞吐、可用性、恢复、隐私、驻留、授权、审计、成本、维护与迁移分别写触发场景、度量、阈值优先级和 Owner。", decision: "硬约束先划定可行域；未知项进入 PoC 或 Pilot 验证，不在报价中默认为通过。", boundary: "一张没有场景、优先级和观察方法的 NFR 清单无法指导架构权衡。", sourceIds: ["nist-genai-profile", "nist-zero-trust"] },
  ...
  "ai-infra-platform": {
    ...
    chapters: [
      { title: "平台作为产品与自助契约", en: "Platform as a Product", explanation: "平台从用户与常见工作负载出发，以能力目录、API、模板、文档和受支持路径交付设备、环境、作业与服务。工作负载契约声明资源、拓扑、数据、运行时、时限、恢复和服务等级。", decision: "先选择一条高频开发到运行路径，把它做成可自助、可拒绝、可支持的产品，再扩展能力。", boundary: "部署 Kubernetes 或罗列组件不能证明用户获得了一致平台体验。", sourceIds: ["cncf-platforms-whitepaper", "kubernetes-dra"] },
