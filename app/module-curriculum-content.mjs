...
const baseModuleCurriculumContent = Object.freeze({
  ...
  mcp: {
    ...
    chapters: [
      ...
      { title: "版本、初始化与能力发现", en: "Versioned Lifecycle & Discovery", explanation: "当前正式版 2026-07-28 不再使用 initialize/initialized，而由每次请求携带协议版本、客户端信息和能力，并要求 Server 实现 server/discover；2025-11-25 等旧版仍通过初始化握手和会话协商。", decision: "把协议版本与业务能力版本分别管理，为现代与旧版兼容路径建立独立契约测试。", boundary: "正式规范生效不等于具体实现已经兼容；发现成功也不证明 Tool 的业务语义与权限正确。", sourceIds: ["mcp-lifecycle-2025-11-25", "mcp-specification-2026-07-28"] },
      ...
      { title: "长调用与 Tasks 扩展", en: "Durable MCP Requests", explanation: "当前 2026-07-28 正式规范把 Tasks 作为需双方显式支持的可选扩展，以 task handle、tasks/get、tasks/update 和 tasks/cancel 承载长调用；2025-11-25 则把实验性 Tasks 放在核心协议中。", decision: "仅在双方声明支持且业务确需耐久执行时采用；迁移时把 2026-07-28 扩展与 2025-11-25 旧版核心能力当作两套契约验证。", boundary: "MCP Task 与 A2A 的跨 Agent Task 仍只能显式映射，不能共用语义或 ID。", sourceIds: ["mcp-tasks-2025-11-25", "mcp-specification-2026-07-28", "a2a-spec-1-0-0"] },
  ...
  evaluation: {
    ...
    chapters: [
      ...
      { title: "开发、回归与盲留出集", en: "Development, Regression & Holdout", explanation: "开发集支持迭代，冻结回归集保护已知能力，盲留出集检查泛化；每条样本应保留来源、适用期、切片、期望行为和裁决。", decision: "让真实失败经过脱敏、去重和业务裁决后进入下一版回归集，同时保护留出集不被调参污染。", boundary: "更多样本不能修复错误标签和选择偏差，点赞或模型自评也不是自动真值。", sourceIds: ["nist-genai-profile"] },
  ...
  security: {
    ...
    chapters: [
      ...
      { title: "控制验证与对抗性测试", en: "Control Assurance", explanation: "测试普通与恶意简历、隐藏文本、跨候选人检索、参数篡改、审批绕过、供应链变化和控制失效后的残余影响。", decision: "验证每层能限制后果，并让已裁决事件进入回归集。", boundary: "Guardrail 命中率和一次红队通过不能证明未知攻击不存在。", sourceIds: ["nist-aml-100-2e2025", "owasp-prompt-injection", "nist-genai-profile"] },
  ...
  "fine-tuning": {
    ...
    chapters: [
      ...
      { title: "四层验收与停止规则", en: "Acceptance & Stop Rule", explanation: "数据层检查权利、覆盖与泄漏，训练层检查收敛与复现，任务层比较目标、保留能力和安全，服务层检查延迟、吞吐、显存与成本；所有结果都相对基座和更轻路线比较。", decision: "只有硬门全部通过且每个被接受初审的增益能够覆盖标注、训练、评估、服务与重训成本时，才进入有限放量。", boundary: "目标提升不能补偿安全或关键能力退化；收益不稳定、完整成本过高或轻量基线反超时应停止。", sourceIds: ["nist-genai-profile", "openai-eval-best-practices", "finops-unit-economics"] },
