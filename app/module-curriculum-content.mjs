...
const baseModuleCurriculumContent = Object.freeze({
  ...
  evaluation: {
    ...
    chapters: [
      ...
      { title: "规则、Judge 与人工", en: "Scoring Spectrum", explanation: "规则适合结构和确定约束，LLM Judge 扩展语义评分，人工负责高风险、模糊和校准样本。上线前要用人工双评样本检查一致性，并通过答案顺序交换、长度扰动和跨模型抽检暴露位置与冗长偏差。", decision: "把评分器当作需要评估、校准和版本化的模型组件。", boundary: "同一个模型生成又自证会产生相关偏差，Judge 分数不是业务真值，也不能独立授权高风险动作。", sourceIds: ["llm-as-judge-2023", "nist-genai-profile", "ragas"], updatedAt: "2026-07-20" },
  ...
  llm: {
    ...
    chapters: [
      ...
      { title: "Q/K/V 与注意力头", en: "MHA, MQA & GQA", explanation: "Query 表示当前位置要寻找什么，Key 表示可匹配特征，Value 表示被聚合的信息。MHA 为每个 Query Head 保留独立 KV Head；MQA 共享一组，GQA 在两者之间分组共享，因而把表示能力与 KV Cache、带宽和服务成本连接起来。", decision: "解释模型架构时同时记录 Query / KV Head 组织和运行时影响。", boundary: "更少 KV Head 不保证目标任务质量不变，注意力权重也不是完整因果解释或答案来源证明。", sourceIds: ["transformer-2017", "gqa-2023"], updatedAt: "2026-07-20" },
  ...
  "llm-inference": {
    ...
    chapters: [
      ...
      { title: "P/D 分离与分布式推理", en: "Disaggregated Serving", explanation: "Prefill 和 Decode 资源画像不同，分离可独立扩容并减少干扰，但会引入 KV 传输、调度和网络依赖。验收应在 TTFT 与 TPOT 双重 SLO 下比较 Goodput，而不是只比较聚合吞吐。", decision: "只有阶段资源冲突或模型规模确实需要时采用分布式复杂度。", boundary: "网络、KV 传输和调度开销可能抵消收益，论文结果不能跨硬件和负载直接照搬。", sourceIds: ["distserve-2024", "vllm-2023"], updatedAt: "2026-07-20" },
