...
const baseModuleCurriculumContent = Object.freeze({
  ...
  multimodal: {
    ...
    chapters: [
      ...
      { title: "理解侧与生成侧", en: "Understand vs Generate", explanation: "理解侧关注识别、描述、问答、检索和时间定位；生成侧关注图像、视频、语音与 any-to-any 输出。两侧的风险不同：理解错误会误导决策，生成错误还涉及品牌、版权、标识和内容安全。", decision: "把理解准确性和生成可发布性分成两套门禁。", boundary: "视觉质量好不等于事实、版权或品牌合规，生成资产仍需业务审核。", sourceIds: ["nist-genai-profile", "c2pa-2-4", "china-ai-content-labeling-2026-08-05"] },
      { title: "显式标识、水印与内容凭证", en: "Content Provenance", explanation: "显式标识负责向人告知，水印或 Soft Binding 帮助资产与凭证重关联，C2PA 签名清单（Manifest） 则记录可验证的来源与编辑声明。三者解决的问题不同，发布和接收侧都要定义缺失、失效与验证失败时如何处置。", decision: "把‘是否告知、能否重关联、能否验证来源链’拆成三项验收。", boundary: "内容凭证只证明声明、签名和资产绑定的验证结果，不证明内容为真、拥有版权或已经合规；凭证也可能被移除。", sourceIds: ["c2pa-2-4", "china-ai-content-labeling-2026-08-05"] },
  ...
  mcp: {
    ...
    chapters: [
      ...
      { title: "版本、初始化与能力发现", en: "Versioned Lifecycle & Discovery", explanation: "当前正式版 2026-07-28 不再使用 initialize/initialized；每次请求必须携带 protocolVersion 与 clientCapabilities，Client 还应通过 clientInfo 自报软件名称与版本等元数据，Server 必须实现 server/discover。Client 可用它预取能力；同时兼容新旧协议的 stdio Client 应先用它探测当前协议支持。clientInfo 不是认证身份；2025-11-25 等旧版仍通过初始化握手和会话协商。", decision: "把协议版本与业务能力版本分别管理，为现代与旧版兼容路径建立独立契约测试。", boundary: "正式规范生效不等于具体实现已经兼容；发现成功也不证明 Tool 的业务语义与权限正确。", sourceIds: ["mcp-lifecycle-2025-11-25", "mcp-specification-2026-07-28", "mcp-changelog-2026-07-28"] },
      ...
      { title: "身份、授权与同意", en: "Identity & Authorization", explanation: "经认证的 Client 主体、最终用户身份和 Server 自身身份可能同时存在；clientInfo 只是 Client 自报的软件名称与版本等元数据，不是认证身份。生产调用要明确代表谁、允许做什么、Token 面向哪个资源，以及何时需要用户确认或审批；2026-07-28 正式规范加入 issuer 校验、客户端类型和凭据绑定等授权强化。", decision: "让每次高影响调用可追到主体、授权依据和参数，并在迁移测试中验证新的授权约束。", boundary: "规范要求已经生效，但不能假设所有 SDK 与产品都已实现；OAuth 登录成功或 clientInfo 声明也不等于 Tool 内部业务授权已完成。", sourceIds: ["mcp-authorization", "mcp-specification-2026-07-28", "nist-zero-trust"] },
      { title: "Elicitation 与敏感交互", en: "Form & URL Elicitation", explanation: "2025-11-25 中，Server 可直接向 MCP Client 发送 Form 或 URL Elicitation；当前 2026-07-28 正式规范改用 Multi Round-Trip Requests。prompts/get、resources/read 与 tools/call 可返回 input_required；inputRequests 与不透明 requestState 各自可选，但每个 InputRequiredResult 至少必须包含其中一项。Client 以新的 JSON-RPC id 重试原操作，只对收到的 inputRequests 提交相应 inputResponses；仅在收到 requestState 时原样回传，未收到时不得自行添加。", decision: "按协议版本和信息敏感性选择交互模式，分别测试拒绝、取消、轮数限制、可选状态绑定和回到原操作的路径。", boundary: "requestState 不是协议会话或授权；任何补参模式都不替代 MCP 授权、第三方授权或业务后置条件。", sourceIds: ["mcp-elicitation-2025-11-25", "mcp-specification-2026-07-28", "mcp-mrtr-2026-07-28", "mcp-authorization"] },
      { title: "列表缓存与 HTTP 网关路由", en: "Cache Scope & HTTP Routing", explanation: "2026-07-28 要求 server/discover、tools/list、prompts/list、resources/list、resources/templates/list 与 resources/read 的 resultType: \"complete\" 结果携带 ttlMs 和 public / private cacheScope；resultType: \"input_required\" 中间结果以及含 inputResponses 或 requestState 的 MRTR 重试不得缓存。Streamable HTTP 中，每个承载 JSON-RPC request 的 Client POST 都必须用 Mcp-Method 镜像请求方法；本修订未规定 notification POST 的该头要求，且核心协议不定义 Streamable HTTP 上的 Client→Server notification。核心协议的 Mcp-Name 镜像 tools/call 与 prompts/get 的 params.name，或 resources/read 的 params.uri；Tasks 扩展还要求 tasks/get、tasks/update 与 tasks/cancel 镜像 params.taskId。", decision: "private 缓存键绑定授权上下文（如不同 access token）、主体、租户、Scope、Server、协议与能力版本；网关拒绝镜像头与正文不一致的请求，再执行身份、方法和能力级策略。", boundary: "TTL 不是一致性或撤权保证，public 不等于可信；HTTP 镜像头也不是身份或业务授权。", sourceIds: ["mcp-list-cache-2026-07-28", "mcp-http-routing-2026-07-28", "mcp-tasks-extension", "mcp-specification-2026-07-28", "nist-zero-trust"] },
      { title: "长调用与 Tasks 扩展", en: "Durable MCP Requests", explanation: "当前 2026-07-28 Tasks 是需双方显式支持的可选扩展，目前只有 tools/call 可返回 CreateTaskResult；客户端再通过 task handle、tasks/get、tasks/update 和 tasks/cancel 管理执行，规范为未来其他 request types 留出扩展空间。2025-11-25 则把实验性 Tasks 放在核心协议中。", decision: "仅在双方声明支持、调用属于当前受支持的 tools/call 且业务确需耐久执行时采用；迁移时把 2026-07-28 扩展与 2025-11-25 旧版核心能力当作两套契约验证。", boundary: "不能假定任意 MCP request 都可转成 Task；取消也是合作式意图，不证明副作用已经停止，MCP Task 与 A2A Task 只能显式映射。", sourceIds: ["mcp-tasks-2025-11-25", "mcp-tasks-extension", "a2a-spec-1-0-0"] },
  ...
  a2a: {
    ...
    chapters: [
      { title: "A2A 与 MCP 的分工", en: "Agent vs Tool Boundary", explanation: "MCP 让模型或应用调用工具和读取资源；A2A 让客户端与独立远端 Agent 交换 Message 或有状态 Task。A2A Task 是核心协作对象；当前 MCP Tasks 是仅覆盖 tools/call 的可选耐久执行扩展，两类 ID 只能显式映射。", decision: "按独立责任、状态所有权和组织边界决定协议，并分别治理生命周期与授权。", boundary: "多 Agent 框架内部调用不一定需要 A2A，A2A 也不能代替工具授权或业务终态。", sourceIds: ["a2a-mcp-boundary", "mcp-tasks-extension"] },
      { title: "Agent Card、Extended Card 与 Extension", en: "Discovery & Extension", explanation: "公开 Agent Card 描述候选入口；认证后的 Extended Agent Card 可以选择性披露额外能力信息，规范建议客户端在当前认证会话中用它替换此前缓存的公开 Card。Extension 以稳定 URI 标识并由调用方通过 A2A-Extensions 逐请求启用；它没有独立 version 字段，破坏性变化必须使用新 URI。不支持的可选 Extension 请求可以忽略；若请求的是不支持的可选 Extension 版本，Agent 应忽略本次激活，且不得自动回退到另一版本。只有 Server 要求使用 Card 中标为 required 的 Extension、而 Client 未在请求中声明支持时，Agent 才必须返回 ExtensionSupportRequiredError。", decision: "分别验证 Card 的 HTTPS 来源与可选 JWS、请求身份与操作授权、Extended Card 内容，以及 Extension 的 Owner、URI 演进、安全和降级；把实际启用的版本化 URI、required 与配置写入 Trace 和契约测试。", boundary: "Extended Card 不是 Extension 激活；忽略不支持的可选激活时仍要保持核心基线行为，缺少 required Extension 则是错误。Extension 不得改变核心数据结构或给核心枚举加值，两者都不证明技能质量、业务权限或授权结果。", sourceIds: ["a2a-concepts", "a2a-specification", "a2a-agent-discovery", "a2a-extensions"] },
      ...
      { title: "标识谱系与终态修订", en: "Identifier Lineage", explanation: "messageId 由消息创建方生成并标识一条 Message，服务端可以选择据此去重，但规范只给出 MAY 级幂等；taskId 标识服务端工作，contextId 归组相关交互，referenceTaskIds 表达对已有 Task 的引用。contextId 缺失时 Agent 可以生成，生成后必须在返回的 Task 或 Message 中带回；若拒绝客户端提供的值，必须报错而不能静默替换。客户端 Message 中的 taskId 只能引用既有且可访问的 Task，不能用来创建新 Task；只给 taskId 时，Agent 必须从 Task 推断 contextId；同时给出两者时必须匹配，否则拒绝 Message。业务单号、MCP Task 和本地 Run 属于其他权威域。", decision: "建立协议与业务 ID 映射；SendMessage 超时先查询 Task 与业务状态，再按双方契约决定是否复用 messageId。终态后发送不带旧 taskId 的新 Message，可保留 contextId 并通过 referenceTaskIds 引用旧 Task；响应仍可能是 Message 或新 Task。", boundary: "taskId 无效或不可访问时返回 TaskNotFoundError。客户端除非理解对端 contextId 语义，否则不应主动提供，并应将服务端生成值视为不透明标识；messageId 不是业务幂等键，协议 ID 也不自动继承权限、提供 exactly-once 或证明最终结果。", sourceIds: ["a2a-specification", "a2a-concepts"] },
      { title: "任务状态机", en: "Task Lifecycle", explanation: "规范共有九个 TaskState 枚举：TASK_STATE_UNSPECIFIED 只表示未知；八个非 UNSPECIFIED 操作状态是 TASK_STATE_SUBMITTED、TASK_STATE_WORKING、TASK_STATE_INPUT_REQUIRED、TASK_STATE_AUTH_REQUIRED、TASK_STATE_COMPLETED、TASK_STATE_FAILED、TASK_STATE_CANCELED 与 TASK_STATE_REJECTED。下文短标签会省略 TASK_STATE_ 前缀。", decision: "中断态恢复原 Task；终态不可改写或恢复，修订应发起新的 Message，并准备处理 Message 或新 Task 两种响应。", boundary: "CancelTask 不保证远端已停止或副作用已撤销；网络重试也不等于业务重试。", sourceIds: ["a2a-specification", "opentelemetry-semconv"] },
      { title: "交付、幂等与恢复", en: "Delivery Semantics", explanation: "读取类操作天然幂等；SendMessage 可由服务端选择按 messageId 去重。对于非终态 Task，SubscribeToTask 首帧给出当前 Task 快照，但请求没有恢复游标或历史重放；ListTasks 的 pageToken / cursor 只用于列表分页。对于每个已配置 webhook，Agent 必须至少尝试一次 Push；失败后可以重试，也可以在连续失败达到配置数量后停止，因此不保证至少一次成功送达，重试还可能产生重复通知。Artifact chunk 有 artifactId、append 与 lastChunk，但核心协议没有全局事件序号或重放保证。", decision: "未知写结果先查询 Task 与业务状态；接收方应以自有投递键、提供方契约或业务版本做幂等处理，这些字段不是 A2A 核心 eventId。需要历史重放时建立事件存储或双方支持的 Extension。", boundary: "Task 已处于终态时应改用 GetTask；SubscribeToTask 必须返回 UnsupportedOperationError。Agent Card 中 pushNotifications 为 false 或缺失时，Push 配置操作必须返回 PushNotificationNotSupportedError。列表分页游标不能恢复订阅，非终态重订阅也不是历史重放；messageId 和流式分块都不是业务 exactly-once，CancelTask 不证明副作用已回滚。", sourceIds: ["a2a-specification", "a2a-extensions"] },
      { title: "1.0 版本与绑定协商", en: "Version & Binding Negotiation", explanation: "截至 2026-09-04，最新标记发布仍为 v1.0.1，但线上兼容以 A2A-Version 的 Major.Minor 1.0 协商；supportedInterfaces[] 声明 URL、Binding 与协议版本。", decision: "把发布补丁、协商版本、Binding、SDK 和 Agent 能力版本分别纳入兼容矩阵。", boundary: "规范要求补丁号不应用于请求、响应或 Agent Card，且不得参与版本协商；网络连通和同为 1.0 也不证明各 Binding、SDK 或产品已采用相同修复与能力。", sourceIds: ["a2a-release-1-0-1", "a2a-specification"] },
