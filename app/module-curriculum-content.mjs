...
const baseModuleCurriculumContent = Object.freeze({
  ...
  "ai-infra-compute": {
    ...
    chapters: [
      ...
      { title: "Scale-out 网络与集合通信", en: "Scale-out", explanation: "当工作负载跨越紧耦合加速器互联域时，数据、张量或流水线并行的通信进入跨域网络。RDMA、拥塞、拓扑与集合通信的实际占比决定网络是否成为瓶颈；IB 与 RoCE 的选择还受现有运营能力和故障域影响。", decision: "用 AllReduce/All-to-All 和真实作业长跑验证跨域网络设计。", boundary: "NCCL 和 Megatron 说明通信机制与特定实验，不证明跨节点就必然跨域，也不证明任意网络升级都会提高 Goodput；短时链路测试同样不会覆盖拥塞与恢复。", sourceIds: ["nccl-collectives", "megatron-3d-parallelism-2021"] },
