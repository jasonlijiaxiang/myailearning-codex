...
const baseModuleCurriculumContent = Object.freeze({
  ...
  "solution-patterns": {
    ...
    chapters: [
      ...
      { title: "一张架构图要能解释责任", en: "Reference Architecture", explanation: "入口、身份、上下文、模型、工具、数据、护栏、观测和人工控制构成常见骨架。每条连线还要说明数据格式、权限主体、超时重试、失败补偿和责任团队，否则只能说明组件共存。", decision: "只保留能改变质量、风险、成本或维护方式的组件。", boundary: "组件齐全不等于端到端正确，跨层约定和故障恢复要单独验证。", sourceIds: ["nist-genai-profile", "opentelemetry-semconv", "opentelemetry-genai-semconv"] },
      ...
      { title: "会议助手要跟踪行动是否落实", en: "Meeting Assistant", explanation: "采集、语音识别、说话人区分、摘要、决定与行动项抽取、确认、通知和后续系统写入组成完整链。指标不仅包括转写准确率，还包括关键决定召回、责任人与截止时间正确率。", decision: "先决定会议内容可否采集、保存和进入哪些后续系统。", boundary: "摘要会省时间，但不能自动代表参会者同意或替代正式决策记录。", sourceIds: ["nist-genai-profile", "opentelemetry-semconv", "opentelemetry-genai-semconv"] },
      ...
      { title: "用每次成功结果计算全部成本", en: "TCO & Production", explanation: "模型与云资源只是成本的一部分；还要计入数据准备、集成、评估、监控、人工复核、失败重试、客户补偿和持续维护。上线门要同时覆盖质量、风险、容量、恢复和责任交接。", decision: "以每个成功业务结果的全部成本比较方案。", boundary: "缺少最终状态样本时只能报告假设与区间，不能给出虚假的精确 ROI。", sourceIds: ["nist-genai-profile", "opentelemetry-genai-semconv"] },
  ...
  mcp: {
    lead: "MCP 解决能力如何被发现和调用；生产落地还必须补上身份、授权、网络与业务事务语义，并把当前正式版与已公告但尚未生效的 RC 分开管理。",
    chapters: [
      { title: "Host、Client 与 Server", en: "Protocol Roles", explanation: "Host 承载用户体验与安全决策，Client 连接某个 Server，Server 暴露能力。当前正式版 2025-11-25 由 Client 维护协议会话；已公告的 2026-07-28 RC 计划移除协议级 session，但不会消除三类角色的责任边界。", decision: "明确每个角色由谁运行、升级、认证和审计，并把连接机制绑定到具体协议版本。", boundary: "协议角色不是部署拓扑；RC 中的无状态核心也不表示应用状态或业务状态会消失。", sourceIds: ["mcp-architecture", "mcp-lifecycle-2025-11-25", "mcp-2026-07-28-rc"] },
      ...
      { title: "版本、初始化与能力发现", en: "Versioned Lifecycle & Discovery", explanation: "当前正式版 2025-11-25 通过 initialize/initialized 协商协议版本和能力；2026-07-28 RC 计划移除该握手，把版本、客户端信息和能力放到每次请求，并增加 server/discover。两套机制不能混写成同一现行流程。", decision: "把协议版本与业务能力版本分别管理，为正式版和候选迁移路径建立独立契约测试。", boundary: "RC 已锁定和可测试不等于最终规范已经发布；握手成功或发现成功也不证明 Tool 的业务语义与权限正确。", sourceIds: ["mcp-lifecycle-2025-11-25", "mcp-2026-07-28-rc"] },
      { title: "传输与部署形态", en: "stdio & Streamable HTTP", explanation: "stdio 常用于本地子进程，边界靠操作系统用户和进程；Streamable HTTP 面向远程共享服务。2025-11-25 远程实现可能依赖协议会话，2026-07-28 RC 则计划采用无状态核心；两者都需要网络身份、重试、负载均衡和边缘治理。", decision: "先固定协议版本并画出信任边界，再选择本地或远程传输与状态管理方式。", boundary: "无协议 session 不等于无业务状态，本地开发安全假设也不能原样复制到远程服务。", sourceIds: ["mcp-lifecycle-2025-11-25", "mcp-2026-07-28-rc", "mcp-authorization"] },
      { title: "身份、授权与同意", en: "Identity & Authorization", explanation: "客户端身份、最终用户身份和 Server 自身身份可能同时存在。生产调用要明确代表谁、允许做什么、Token 面向哪个资源，以及何时需要用户确认或审批；2026-07-28 RC 还公告了 issuer 校验、客户端类型和凭据绑定等授权强化。", decision: "让每次高影响调用可追到主体、授权依据和参数，并在迁移测试中验证新的授权约束。", boundary: "RC 授权强化尚不能当作所有实现的现行能力；OAuth 登录成功也不等于 Tool 内部业务授权已完成。", sourceIds: ["mcp-authorization", "mcp-2026-07-28-rc", "nist-zero-trust"] },
      { title: "Elicitation 与敏感交互", en: "Form & URL Elicitation", explanation: "当前 2025-11-25 规范中，Server 可向 MCP Client 发送 Form 或 URL Elicitation，由 Host 负责用户界面与同意。2026-07-28 RC 计划用 Multi Round-Trip Requests 重构服务端向客户端索取输入的流程，因此实现不能只按功能名称判断兼容。", decision: "按协议版本和信息敏感性选择交互模式，分别测试拒绝、取消、重试和回到原任务的路径。", boundary: "RC 流程尚未成为最终规范；任何版本的 Elicitation 都不替代 MCP 授权、第三方授权或业务后置条件。", sourceIds: ["mcp-elicitation-2025-11-25", "mcp-2026-07-28-rc", "mcp-authorization"] },
      { title: "长调用与实验性 Tasks", en: "Durable MCP Requests", explanation: "2025-11-25 正式规范中的 Tasks 仍是实验性核心能力，把支持的请求包装为可轮询、可延迟取回结果的状态机。2026-07-28 RC 计划把 Tasks 移到独立扩展，并重设创建、查询、更新和取消生命周期。", decision: "仅在双方声明支持且业务确需耐久执行时采用；迁移时把 2025-11-25 核心能力与候选扩展当作两套契约验证。", boundary: "RC 中的 Tasks 扩展尚未成为最终规范；MCP Task 与 A2A 的跨 Agent Task 仍只能显式映射，不能共用语义或 ID。", sourceIds: ["mcp-tasks-2025-11-25", "mcp-2026-07-28-rc", "a2a-spec-1-0-0"] },
  ...
  evaluation: {
    ...
    chapters: [
      ...
      { title: "模型、组件与业务三层", en: "Three Evaluation Layers", explanation: "模型层检查通用或任务能力，组件层检查检索、路由、工具和护栏，业务层检查终态、风险和用户价值。三层共同变化时，只看最终分数无法定位责任。", decision: "每个发布门同时保留端到端结果和关键组件诊断。", boundary: "组件指标好不保证业务成功，业务结果下降也不一定由模型造成。", sourceIds: ["nist-genai-profile", "opentelemetry-genai-semconv"] },
      ...
      { title: "场景验收与生产闭环", en: "Release & Monitoring", explanation: "RAG 要分检索与生成，Agent 要检查过程、工具与终态，微调要比较未见切片和基座。上线后用抽样、业务指标和失败回流持续巡检，并把每次变更关联到版本。", decision: "离线门禁决定能否放量，在线证据决定是否继续、回滚或修复。", boundary: "线上自动评分无法覆盖所有风险，低反馈量也不表示系统成功。", sourceIds: ["opentelemetry-genai-semconv", "nist-genai-profile"] },
    ],
  },
  security: {
  ...
  "ai-ops": {
    ...
    chapters: [
      ...
      { title: "离线验收与在线巡检", en: "Offline & Online Evaluation", explanation: "离线评估使用可控样本比较版本，在线评估观察真实分布、反馈和业务结果。规则、Judge、人工抽样和用户行为共同形成漏斗，线上失败再进入治理后的回归集。", decision: "离线决定能否放量，在线决定是否继续、回滚或修复。", boundary: "自动 Judge 不能覆盖所有风险，反馈缺失也不表示成功。", sourceIds: ["nist-genai-profile", "opentelemetry-genai-semconv"] },
      ...
      { title: "版本、灰度与回滚", en: "Release Management", explanation: "模型、Prompt、数据、索引、工具 Schema、路由和护栏都要进入版本注册。离线回放、影子、金丝雀、分组放量和自动回退组成发布链，并保留回滚后的数据兼容。", decision: "任何影响输出或动作的变化都作为可审计发布。", boundary: "回滚配置不能撤销已经发生的外部副作用，需要补偿和客户处置。", sourceIds: ["opentelemetry-genai-semconv", "nist-genai-profile"] },
  ...
  "llm-training": {
    ...
    chapters: [
      ...
      { title: "并行、通信与 Checkpoint", en: "Distributed Systems", explanation: "数据、张量、流水线和专家并行在显存、通信、气泡和实现复杂度间取舍。长训练还受数据加载、网络、存储、Checkpoint 和故障恢复影响，应报告有效训练时间而非只看 GPU 小时。", decision: "先用剖析定位计算、通信和 I/O，再调整并行或集群。", boundary: "卡数翻倍不会带来线性加速，短时吞吐也不代表长作业稳定。", sourceIds: ["megatron-3d-parallelism-2021", "nccl-collectives", "torchsnapshot-checkpoint", "checkfreq-2021"] },
  ...
  "llm-inference": {
    ...
    chapters: [
      { title: "Prefill、Decode 与服务指标", en: "Request Lifecycle", explanation: "Prefill 并行处理输入，决定首 Token 前的大部分计算；Decode 逐 Token 生成，受内存带宽和串行循环影响。TTFT、TPOT/ITL、总时延和吞吐分别描述不同体验。", decision: "按交互意图选择指标，不用平均响应时间概括所有阶段。", boundary: "客户端首字还会受网关缓冲、网络和渲染影响，不能只看引擎内部。", sourceIds: ["vllm-2023", "opentelemetry-genai-semconv"] },
      ...
      { title: "容量、SLO 与故障恢复", en: "Production Serving", explanation: "短交互、长上下文、Agent 和批处理需要不同容量基线。生产还要验证冷启动、模型加载、节点故障、限流、降级、版本灰度和恢复后业务正确性。", decision: "用分层负载曲线和尾部 SLO 决定资源池与余量。", boundary: "自动重启只恢复进程或容量，不证明请求状态、缓存和业务副作用已正确恢复。", sourceIds: ["opentelemetry-genai-semconv", "nist-genai-profile"] },
    ],
  },
  "data-engineering": {
  ...
  "ai-infra-platform": {
    ...
    chapters: [
      ...
      { title: "队列、Gang 与碎片", en: "Scheduling", explanation: "分布式作业需要多设备同时可用；全有或全无准入可避免同步作业在资源不足时长期部分启动。配额、优先级、抢占、拓扑和不同请求形状也可能造成空闲但不可调度。利用率与作业完成时间必须一起看。", decision: "用排队原因和业务优先级优化，不机械追求满载。", boundary: "Kueue 的 waitForPodsReady 只是一种简单实现；抢占会浪费进度并增加恢复压力，碎片也不一定能靠时间共享解决。", sourceIds: ["kueue-all-or-nothing", "kueue-fair-sharing", "kubernetes-dra"] },
      ...
      { title: "训练容错与 Checkpoint", en: "Resilience", explanation: "长作业需要按实测故障与恢复目标设计 Checkpoint。周期在写入成本与丢失进度间取舍；正确恢复还要显式保存模型之外的优化器、调度器、随机状态和应用状态，并按训练框架补充数据游标与拓扑契约。", decision: "用失败率、保存与恢复时间和有效训练进度共同设置策略。", boundary: "TorchSnapshot 说明的是一个具体框架的状态契约；Pod 重启、节点替换或只恢复权重都不表示训练状态已经正确恢复。", sourceIds: ["torchsnapshot-checkpoint", "checkfreq-2021"] },
  ...
  "ai-infra-compute": {
    ...
    chapters: [
      ...
      { title: "Scale-up 节点内互联", en: "Scale-up", explanation: "当模型或并行状态跨设备时，节点内高带宽互联和交换承载 AllReduce、AllGather、ReduceScatter 等集合通信。拓扑、链路和通信量共同影响可扩展性，设备数增加也会增加需要协调的数据流。", decision: "根据并行策略和通信画像选择节点内规模。", boundary: "NCCL 定义通信原语和诊断能力，不保证目标框架能持续利用链路规格；单节点结果也不能外推多节点。", sourceIds: ["nccl-collectives", "megatron-3d-parallelism-2021"] },
      { title: "Scale-out 网络与集合通信", en: "Scale-out", explanation: "多节点训练需要让数据、张量或流水线并行的通信跨越网络。RDMA、拥塞、拓扑与集合通信的实际占比决定网络是否成为瓶颈；IB 与 RoCE 的选择还受现有运营能力和故障域影响。", decision: "用 AllReduce/All-to-All 和真实作业长跑验证网络设计。", boundary: "NCCL 和 Megatron 说明通信机制与特定实验，不证明任意网络升级都会提高 Goodput；短时链路测试也不会覆盖拥塞与恢复。", sourceIds: ["nccl-collectives", "megatron-3d-parallelism-2021"] },
      { title: "存储、数据管线与供电", en: "Feed the Cluster", explanation: "训练读取、Checkpoint、模型分发和推理加载需要对象、并行文件系统、本地缓存和网络协同。电力、散热、机柜密度、布线和交付条件共同限制物理可部署规模。", decision: "把数据到设备的持续吞吐、恢复路径和设施工程验收纳入容量计划。", boundary: "DGX H100 设计指南的具体功率与气流只适用于其参考架构；缓存后的稳态速度也不能掩盖首次加载、恢复或峰值供电。", sourceIds: ["nvidia-dgx-h100-data-center", "torchsnapshot-checkpoint"] },
