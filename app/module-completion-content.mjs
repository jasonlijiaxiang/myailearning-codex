...
export const completionCurriculum = Object.freeze({
  "model-landscape": freezeItems([
    { title: "评估矩阵与模型发布", en: "Evaluation & Release", explanation: "模型候选要在相同任务、输入分布和通过条件下比较，并把模型标识、Prompt、工具 Schema、数据版本和结果绑定。模型升级后先离线回归，再按任务切片灰度。", decision: "把选型结果转成可复现发布证据，不停留在一次榜单或 Demo。", boundary: "同名模型、默认别名或厂商自动升级可能改变行为；历史结果不能自动代表当前版本。", sourceIds: ["nist-genai-profile", "opentelemetry-semconv"] },
  ]),
  ...
  mcp: freezeItems([
    { title: "错误、进度与可观测", en: "Failure Semantics", explanation: "生产 MCP Server 不只返回成功结果，还要让 Host 区分参数错误、权限拒绝、暂时故障、超时和部分结果，并关联请求、工具、主体与下游业务状态。", decision: "在复用前验证错误是否可处理、重试是否安全、日志是否足以追责。", boundary: "协议层错误不能替代业务事务状态；网络超时也不能证明动作未发生。", sourceIds: ["mcp-architecture", "opentelemetry-semconv"] },
  ]),
  a2a: freezeItems([
  ...
  evaluation: freezeItems([
    ...
    { title: "线上任务结果与回流", en: "Online Evaluation", explanation: "离线评估提供可重复比较，线上灰度负责暴露真实分布、工具故障和用户行为。只有经过脱敏、复核和责任确认的新失败，才进入下一轮回归集。", decision: "把业务终态、质量、风险、时延和成本一起用于发布后判断。", boundary: "点赞、停留时间或模型自评不能单独证明任务成功，也不能未经裁决直接成为标签。", sourceIds: ["opentelemetry-semconv", "nist-genai-profile"] },
  ]),
  "ai-gateway": freezeItems([
    { title: "策略变更与证据化发布", en: "Policy Release", explanation: "路由、限流、护栏、缓存和日志策略会改变生产行为，应绑定同一策略版本，经过离线回放、影子判定、分段灰度和可验证回滚。", decision: "让一次请求可以还原当时命中的完整策略，而不是只看到当前配置。", boundary: "回滚网关配置不会撤销已经发生的外部动作，业务补偿仍由应用负责。", sourceIds: ["nist-genai-profile", "opentelemetry-semconv"] },
  ]),
  "ai-ops": freezeItems([
    { title: "遥测数据与隐私", en: "Telemetry Governance", explanation: "可观测需要记录任务、版本、阶段、错误和业务结果，但不代表默认保存所有 Prompt 与响应。采样、脱敏、访问、用途和保留期要按数据敏感度设计。", decision: "先定义需要回答的运营问题，再收集足以归因的最小数据。", boundary: "只保存全文会增加泄露风险；只保存聚合指标又可能无法复现具体故障。", sourceIds: ["opentelemetry-semconv", "nist-genai-profile"] },
  ...
  "llm-inference": freezeItems([
    { title: "版本发布与请求连续性", en: "Serving Release", explanation: "模型、引擎、量化格式、Tokenizer、聊天模板和路由策略共同决定服务行为。灰度要同时观察质量、TTFT、TPOT、拒绝率、成本和在途请求处理。", decision: "把推理栈作为一个发布单元验证，并预先定义排空、回滚和缓存处理。", boundary: "进程健康和权重加载成功不能证明协议兼容、质量不退化或请求状态连续。", sourceIds: ["vllm-2023", "opentelemetry-semconv"] },
  ]),
...
export const completionLearning = Object.freeze({
  "model-landscape": Object.freeze({
    ...
    labs: freezeLabs([
      { title: "评审一次模型自动升级", scenario: "供应商准备更新默认模型别名，客户担心行为漂移。", tasks: ["冻结当前模型、Prompt、工具和知识版本", "选择关键任务与高风险切片做新旧对照", "设计灰度、告警、回滚和无法固定版本时的补偿控制"], deliverable: "模型升级评审单", acceptance: "明确允许变化、禁止退化、负责人和回滚证据。", sourceIds: ["nist-genai-profile", "opentelemetry-semconv"] },
    ]),
  }),
  ...
  evaluation: Object.freeze({
    ...
    labs: freezeLabs([
      { title: "校准一次 LLM Judge", scenario: "开放问答没有唯一标准答案，团队准备使用模型评审。", tasks: ["把质量拆成可观察维度并写评分量表", "选择人工双评样本测一致性与偏差", "定义模型评审适用范围、抽检和争议升级"], deliverable: "评审量表与校准报告", acceptance: "确定性检查、模型评审和人工裁决各有清晰边界。", sourceIds: ["ragas", "nist-genai-profile"] },
    ]),
  }),
  "ai-gateway": Object.freeze({
    ...
    labs: freezeLabs([
      { title: "排查一次网关放大故障", scenario: "模型提供方间歇报错后，网关延迟和调用量同时上升。", tasks: ["沿 Trace 统计应用、网关、SDK 和下游的每层 attempt", "明确重试、超时、限流与幂等所有者", "设计受控降级、容量保护和恢复验证"], deliverable: "故障放大归因与策略修订", acceptance: "能解释每次额外调用来自哪一层，并证明修订后不再乘法放大。", sourceIds: ["opentelemetry-semconv", "nist-genai-profile"] },
    ]),
  }),
  "ai-ops": Object.freeze({
    ...
    labs: freezeLabs([
      { title: "设计一份 AI 事故运行手册", scenario: "Agent 出现越权工具调用，团队需要立即止损并恢复服务。", tasks: ["定义按模型、Prompt、工具、租户和动作风险分层的开关", "指定证据保留、负责人、通知和业务状态核对", "用事故样本完成回放、受限恢复与逐级放量"], deliverable: "AI 事故处置与恢复手册", acceptance: "每个阶段都有执行人、进入条件、退出条件和业务确认。", sourceIds: ["nist-genai-profile", "nist-zero-trust", "opentelemetry-semconv"] },
    ]),
  }),
  ...
  "llm-inference": Object.freeze({
    ...
    labs: freezeLabs([
      { title: "诊断一次首字延迟退化", scenario: "模型版本未变，但用户看到首字的时间突然增加。", tasks: ["分别测客户端、网关、排队和引擎 Prefill 时间", "按输入长度、并发、缓存命中和重试切片", "提出最小修复并验证 TPOT、吞吐和质量未退化"], deliverable: "TTFT 分层诊断报告", acceptance: "定位到具体阶段和条件，而不是笼统归因于模型变慢。", sourceIds: ["vllm-2023", "opentelemetry-semconv"] },
    ]),
  }),
...
export const completionQa = Object.freeze({
  "model-landscape": freezeQa([
    { q: "公开 Benchmark 第一名的模型，为什么可能不适合客户生产？", a: "Benchmark 只覆盖特定任务、数据和评分方式；生产还受语言、工具、时延、地域、版本、风险和成本限制。", depth: "先把客户任务按输入、难度和风险切片，在固定版本上比较任务成功、严重错误、P95 和单位成功成本。公开结果可用于提出候选，但不能替代客户数据、提示、工具和交付环境中的验证。", ask: "追问客户：最重要的三类任务和不可接受错误是什么？公开榜单覆盖了其中哪些？", tag: "Benchmark 边界", basis: "任务化评估 + 使用情境", evidence: [{ sourceId: "nist-genai-profile", supports: "支持按具体使用情境、受影响主体和风险容忍度测量生成式 AI，而非依赖单一通用分数。" }, { sourceId: "openai-models", supports: "支持模型家族具有不同能力与使用定位，选择仍需结合任务。" }] },
    { q: "怎样为模型停服或不可用准备替代方案？", a: "预先维护通过同一硬门槛的候选模型、适配层和回归集，并明确哪些任务可以降级、阻断或转人工。", depth: "替代不只是换一个端点。上下文窗口、工具调用、Schema、语言、安全策略和输出质量都可能不同。发布包应记录主备模型、适配模板、禁止降级任务、切换条件和回滚证据，并定期演练供应商或地域故障。", ask: "追问客户：哪些能力和数据边界切换时绝不能改变？允许牺牲时延、质量还是功能？", tag: "退出策略", basis: "供应连续性 + 版本治理", evidence: [{ sourceId: "nist-genai-profile", supports: "支持识别第三方依赖、准备响应和持续管理部署风险。" }, { sourceId: "opentelemetry-semconv", supports: "支持记录模型与提供方等调用属性，为切换前后比较提供共同遥测基础。" }] },
  ]),
  multimodal: freezeQa([
  ...
  "ai-ops": freezeQa([
    ...
    { q: "观测数据保留越多，是否越容易排查 AI 问题？", a: "不一定。需要的是可关联、可解释且合法的证据；无目的保存全文会增加泄露与治理成本。", depth: "先列运营问题，再选择任务 ID、版本、阶段、错误、时延、成本和业务结果等结构化字段。正文只在确有必要时按风险采样、脱敏和受控访问，并设置用途与保留期。没有版本和业务结果的大量日志，通常仍无法归因。", ask: "追问客户：必须回答哪些排障或审计问题？哪些字段足够，哪些原文确有合法保存理由？", tag: "遥测治理", basis: "最小必要 + 可归因性", evidence: [{ sourceId: "opentelemetry-semconv", supports: "支持用标准属性描述生成式 AI 和 Agent 遥测，不要求默认保存全部正文。" }, { sourceId: "nist-genai-profile", supports: "支持在测量需求与隐私、数据治理风险之间进行管理。" }] },
  ]),
  ...
  "ai-infra-platform": freezeQa([
    { q: "GPU 利用率很高，为什么训练和推理产出仍可能很差？", a: "利用率只说明设备忙，不说明计算对目标模型有效、作业按时完成或在线请求满足 SLO。", depth: "训练还要看 MFU、Goodput、通信、数据等待、Checkpoint、重试和合格模型产出；推理要看任务成功、尾延迟、拒绝率和单位成功成本。平台优化应从排队原因和业务结果反推资源策略。", ask: "追问客户：设备忙时完成了多少达标工作？哪些时间花在通信、等待、重试或不合格输出上？", tag: "有效产出", basis: "资源遥测 + 业务结果", evidence: [{ sourceId: "opentelemetry-semconv", supports: "支持把系统遥测与生成式 AI 调用属性关联；业务有效产出仍需应用定义。" }, { sourceId: "nvidia-gpu-operator", supports: "支持 GPU 运行栈和监控能力，但不自动定义业务 Goodput。" }] },
