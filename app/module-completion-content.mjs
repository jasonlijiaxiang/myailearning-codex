...
export const completionCurriculum = Object.freeze({
  ...
  "ai-ops": freezeItems([
    ...
    { title: "从告警到确认业务恢复", en: "Incident Recovery", explanation: "事故处理需要区分检测、止损、隔离、技术恢复、业务状态确认和客户影响评估。回滚模型或 Prompt 后，还要确认工具副作用、缓存和异步任务是否一致。", decision: "为高风险路径预先准备分层开关、责任人和恢复证据。", boundary: "错误率恢复正常或服务重新返回 200，都不能证明业务影响已经结束。", sourceIds: ["nist-genai-profile", "nist-zero-trust"] },
  ]),
  "llm-training": freezeItems([
      { title: "实验谱系与可复现制品", en: "Experiment Lineage", explanation: "训练结果需要关联基础权重、代码、数据、Tokenizer、目标函数、优化器、精度、随机状态、并行拓扑、Checkpoint、停止规则、评估和运行环境。长作业恢复后还要验证状态连续。", decision: "把训练运行配置记录（Run manifest）与可复现证据作为候选模型制品的一部分，而不是依赖运行人员记忆。", boundary: "框架可以保存与重新分片已声明状态，但不会自动覆盖数据游标或消除跨版本、硬件、内核和非确定执行带来的差异。", sourceIds: ["pytorch-distributed-checkpoint", "pytorch-reproducibility", "torchsnapshot-checkpoint", "checkfreq-2021"] },
  ]),
  "llm-inference": freezeItems([
    { title: "版本发布与请求连续性", en: "Serving Release", explanation: "模型、Tokenizer、聊天模板、引擎、量化制品、调度、缓存和路由共同决定服务行为。灰度要同时观察质量、排队、TTFT、TPOT、拒绝率、Goodput、成本和在途请求。", decision: "把推理栈作为一个发布单元验证，并预先定义排空、回滚、缓存兼容和失败处置。", boundary: "进程健康和权重加载成功不能证明协议兼容、质量不退化或请求状态连续；发布操作仍需目标平台演练。", sourceIds: ["nist-genai-profile", "vllm-metrics-v0-12"] },
  ]),
  "data-engineering": freezeItems([
    { title: "为不同用途准备数据制品", en: "Purpose-specific Products", explanation: "同一原始数据用于 RAG、评估和训练时，更新速度、许可、去重、标签和泄漏风险不同。共享来源身份与血缘，但分别发布用途明确的数据版本。", decision: "在加工前先写用途和验收，不把一个数据集无条件复制到所有下游。", boundary: "能用于检索不表示可以用于训练；能用于内部评估也不表示可以进入生产日志。", sourceIds: ["nist-genai-profile", "nist-zero-trust"] },
  ]),
...
export const completionLearning = Object.freeze({
  ...
  "llm-inference": Object.freeze({
    ...
    labs: freezeLabs([
      { title: "诊断一次首字延迟退化", scenario: "模型版本未变，但用户看到首字的时间突然增加。", tasks: ["分别测客户端、网关、排队、模型加载和 Prefill", "按输入长度、到达率、就绪容量、缓存命中和重试切片", "提出最小修复并验证 TPOT、Goodput、拒绝与质量未退化"], deliverable: "TTFT 分层诊断与过载处置报告", acceptance: "定位到具体阶段和条件，并说明何时扩容、背压或拒绝，而不是笼统归因于模型变慢。", sourceIds: ["vllm-metrics-v0-12", "serverlessllm-2024", "opentelemetry-genai-semconv"] },
    ]),
  }),
...
export const completionQa = Object.freeze({
  ...
  multimodal: freezeQa([
    { q: "文档 OCR 字符准确率很高，为什么表格问答仍可能错误？", a: "因为字符识别正确不等于版面、行列、表头、单位和阅读顺序恢复正确。", depth: "表格问题要分开验收文字识别、结构恢复、单元格关系、跨页延续、证据定位和最终计算。客户样本中常见的合并单元格、扫描歪斜、脚注和多级表头应单独切片；高风险数字还要回到原区域复核。", ask: "最复杂的表格有哪些结构？错误的行列关系会造成什么业务后果？", tag: "结构恢复", basis: "文档解析 + 分层评估", evidence: [{ sourceId: "docling-report", supports: "支持文档处理需要恢复版面、阅读顺序和表格结构，而不只是纯文本。" }, { sourceId: "pp-ocr-2020", supports: "支持 OCR 系统包含检测、方向与识别等独立阶段；其结果不自动代表表格语义正确。" }] },
  ]),
  mcp: freezeQa([
    { q: "MCP 工具返回成功，为什么业务动作仍可能失败？", a: "协议调用成功只说明交换完成；业务系统还可能拒绝授权、校验失败、部分提交或在超时后处于未知状态。", depth: "Server 应把协议错误与业务结果分开，返回可处理的错误语义和稳定关联 ID。写入工具还要使用幂等键、权威状态查询和必要的补偿流程；Host 不应把一段成功文本当作订单、付款或资源变更已经完成。", ask: "最终由哪个系统确认动作完成？网络超时后怎样查询、重试或补偿？", tag: "错误语义", basis: "协议边界 + 业务状态", evidence: [{ sourceId: "mcp-architecture", supports: "支持 MCP Host、Client 与 Server 的协议职责分离；最终业务语义由具体能力实现。" }, { sourceId: "opentelemetry-genai-semconv", supports: "支持关联 Agent、工具和模型调用，为跨层归因提供基础。" }] },
    { q: "企业应该允许客户端自动安装任意 MCP Server 吗？", a: "不应该默认允许。Server 是可执行能力和数据入口，应经过来源、代码、权限、版本和运营责任评审。", depth: "建立受管目录，登记发布者、签名或制品来源、暴露能力、所需 scope、网络出口、敏感日志、更新和下线机制。高风险 Server 在隔离环境验证，只读与写入分开批准；客户端自动发现不能绕过企业安装与授权策略。", ask: "谁批准 Server 进入目录？更新后如何重新评审，发现恶意版本怎样撤回？", tag: "Server 供应链", basis: "MCP 安全 + 零信任", evidence: [{ sourceId: "mcp-security", supports: "支持评估 MCP 生态中的授权、令牌与安全威胁。" }, { sourceId: "nist-zero-trust", supports: "支持对资源和主体持续验证并实施最小权限。" }] },
  ]),
  a2a: freezeQa([
    { q: "A2A 的取消请求，是否保证远端任务已经停止？", a: "不能默认保证。CancelTask 可能返回不可取消，取消成功也只改变协议状态，不承诺撤销已发生的业务副作用。", depth: "委托方发送取消后仍应查询 Task 终态；执行方报告 CANCELED、已完成、不可取消或需要补偿。付款、资源创建等副作用必须由业务系统提供幂等、撤销与补偿，不能只靠 Agent Message。", ask: "取消后最迟多久停止？已发生动作由谁发现和补偿？", tag: "取消语义", basis: "任务生命周期 + 业务补偿", evidence: [{ sourceId: "a2a-specification", supports: "支持 CancelTask、TaskNotCancelableError 和 CANCELED 状态，但不承诺业务副作用回滚。" }, { sourceId: "nist-genai-profile", supports: "支持为高影响 AI 行为配置人工监督、响应和恢复。" }] },
    { q: "多 Agent 架构应该由一个编排者控制，还是允许点对点协作？", a: "固定业务责任和高风险流程优先显式编排；只有协作关系确需动态发现时才增加点对点自治。", depth: "中心编排便于统一预算、身份、状态和审计，但可能成为扩展瓶颈；点对点更灵活，却增加环路、重复委托、权限传播和成本失控。可以用业务任务所有者保持最终控制，同时让局部 Agent 通过 A2A 协作。", ask: "谁对最终结果负责？协作拓扑变化时，预算、身份和停止条件由谁执行？", tag: "协作拓扑", basis: "复杂度递增 + 风险管理", evidence: [{ sourceId: "anthropic-effective-agents", supports: "支持从简单可组合模式开始，仅在任务需要时增加多 Agent 复杂度。" }, { sourceId: "a2a-concepts", supports: "支持独立 Agent 通过任务与产物语义进行协作。" }] },
  ]),
  evaluation: freezeQa([
    { q: "评估集版本升级时，怎样接纳线上失败又保护盲留出集？", a: "让 AI Ops 只负责采集候选失败；Evaluation 再做脱敏、去重、裁决、切片和版本化，把确认案例加入下一版回归集而不是当前盲留出集。", depth: "每个候选样本记录来源、版本、任务切片、失败类型、期望行为、裁决人和适用时间。高频相似案例合并，高风险或少数群体案例独立保留；开发集、冻结回归集和盲留出集用途分开。升级后，新旧候选要在相同评估版本上比较，并说明新考卷改变了什么目标人群或失败覆盖。", ask: "谁有权裁决线上样本？哪些案例进入开发、回归或留出，考卷换版后历史结果怎样比较？", tag: "黄金集治理", basis: "评估数据治理 + 污染控制", evidence: [{ sourceId: "nist-genai-profile", supports: "支持记录测量方法、数据限制和持续出现的新风险。" }, { sourceId: "nist-ai-800-4", supports: "支持部署后监测会产生需要治理的新证据，但不把生产信号自动定义为真值。" }] },
  ]),
  "ai-gateway": freezeQa([
    { q: "AI 网关的策略应该怎样安全上线？", a: "把路由、限流、重试、护栏、缓存和日志绑定为版本化策略包，先回放和影子判定，再按业务风险分段灰度。", depth: "离线回放比较新旧决策，不实际执行高风险动作；影子模式只记录拟议变化；灰度按租户、任务和风险分组，而不是只按流量百分比。超过质量、时延、风险或成本门槛时原子回滚，并保留受影响请求。", ask: "哪些策略变化会改变业务结果？谁批准，怎样还原一次请求使用的策略版本？", tag: "策略发布", basis: "发布治理 + 可观测", evidence: [{ sourceId: "nist-genai-profile", supports: "支持在系统变更前后进行风险测量、监控和管理。" }, { sourceId: "opentelemetry-genai-semconv", supports: "支持用共同遥测字段关联生成式 AI 请求与版本属性。" }] },
    { q: "模型提供方故障时，AI 网关应该自动切到任意可用模型吗？", a: "不应该。只能切到预先验证过能力、数据边界和安全策略的候选；不满足硬门槛的任务应阻断或转人工。", depth: "备用模型即使返回成功，也可能缺少工具、Schema、上下文、语言或地域能力。为每条回退路径维护评估结果、允许降级项和禁止任务，并在故障演练中验证路由原因、质量、尾延迟和恢复。", ask: "故障时允许牺牲质量、功能、地域还是时延？哪些任务不能自动降级？", tag: "故障降级", basis: "模型等价性 + 风险门槛", evidence: [{ sourceId: "nist-genai-profile", supports: "支持识别依赖失效并准备与风险相称的响应和恢复。" }, { sourceId: "cloudflare-ai-gateway-dynamic-routing", supports: "支持配置多端点动态路由；具体功能、地域与质量等价性仍需应用验证。" }] },
  ]),
  "ai-ops": freezeQa([
    { q: "AI 事故恢复后，为什么还要核对业务系统状态？", a: "因为模型或服务恢复只证明技术链可用，事故期间已经发生的订单、通知、权限或数据写入不会自动撤销。", depth: "事件处理要从受影响任务和 Trace 找到外部动作，查询权威系统的最终状态，按幂等或补偿规则修复，再确认客户影响和通知。只有技术指标、业务状态和风险复测共同通过，才能逐级恢复流量。", ask: "AI 系统可以改变哪些外部状态？事故后由谁逐项核对和补偿？", tag: "业务恢复", basis: "事件响应 + 权威状态", evidence: [{ sourceId: "nist-genai-profile", supports: "支持为生成式 AI 风险建立响应、恢复和受影响主体管理。" }, { sourceId: "nist-zero-trust", supports: "支持对资源访问持续执行身份与策略控制，恢复时不能沿用隐式信任。" }] },
    { q: "观测数据保留越多，是否越容易排查 AI 问题？", a: "不一定。需要的是可关联、可解释且合法的证据；无目的保存全文会增加泄露与治理成本。", depth: "先列运营问题，再选择任务 ID、版本、阶段、错误、时延、成本和业务结果等结构化字段。正文只在确有必要时按风险采样、脱敏和受控访问，并设置用途与保留期。没有版本和业务结果的大量日志，通常仍无法归因。", ask: "必须回答哪些排障或审计问题？哪些字段足够，哪些原文确有合法保存理由？", tag: "遥测治理", basis: "最小必要 + 可归因性", evidence: [{ sourceId: "opentelemetry-genai-semconv", supports: "支持用标准属性描述生成式 AI 和 Agent 遥测，不要求默认保存全部正文。" }, { sourceId: "nist-genai-profile", supports: "支持在测量需求与隐私、数据治理风险之间进行管理。" }] },
  ]),
  "llm-training": freezeQa([
    { q: "训练恢复后 Loss 连续，为什么还不能证明状态正确？", a: "因为 Loss 是聚合结果，短期连续可能掩盖样本重复或跳过、优化器重置、异步快照未完成和环境不兼容，必须直接验证状态与下一批数据。", depth: "把恢复验证写成独立门：核对 运行配置记录摘要与异步完成状态，比较下一批样本身份和数据游标，校验优化器、调度器与 RNG 摘要，确认拓扑和软件兼容，再比较吞吐与未见任务。任何关键项无法解释时，隔离这条恢复 Run，并回退到更早的可信 Checkpoint。", ask: "恢复门逐项验证什么？哪一项失败会阻断继续训练并触发回退？", tag: "训练恢复", basis: "状态核验程序 + 回退门", evidence: [{ sourceId: "pytorch-distributed-checkpoint", supports: "支持多 Rank 保存与加载模型和优化器状态、重新分片及异步保存完成语义；数据游标与完整应用状态仍需另行定义。" }, { sourceId: "pytorch-reproducibility", supports: "支持以下复现边界：跨版本与平台不保证完全复现，因此恢复验收要声明受控环境和容差。" }, { sourceId: "checkfreq-2021", supports: "支持把数据加载状态和 Checkpoint 频率纳入恢复连续性设计；论文结果不能替代目标训练栈的恢复演练。" }] },
  ]),
  "llm-inference": freezeQa([
    { q: "模型权重能装进显存，为什么并发一上来仍会 OOM？", a: "因为权重只是固定内存；KV Cache、激活、工作区、碎片和并发序列还会持续占用显存。", depth: "容量模型要按层数、KV 头、头维度、精度、输入与输出长度和并发计算缓存，再加运行时工作区与安全余量。最大上下文和最大并发通常不能同时兑现；批处理和缓存策略也会改变峰值。", ask: "真实输入、输出和并发分布是什么？最长请求占比与可排队时间是多少？", tag: "显存容量", basis: "KV Cache + 运行时内存", evidence: [{ sourceId: "vllm-2023", supports: "支持 KV Cache 块管理、连续批处理和显存碎片会影响推理容量。" }] },
    { q: "量化后吞吐提高，为什么仍可能不值得上线？", a: "因为吞吐只是中间指标；还要验证目标任务质量、长上下文、尾延迟、硬件内核、稳定性和每个达标任务成本。", depth: "量化格式减少内存或带宽，但不同硬件与算子收益不同，质量损失可能集中在少数高价值切片。发布前用相同负载比较 TTFT、TPOT、Goodput、拒绝率、能耗和关键任务，并把模型、Tokenizer、模板、量化制品与引擎组合成可灰度、排空和回滚的版本。", ask: "量化要解决的是容量、延迟还是单位达标结果成本？哪些任务退化不能接受？", tag: "量化发布", basis: "性能、质量与单位经济共同验收", evidence: [{ sourceId: "gptq-2023", supports: "支持量化可减少权重位宽并在论文特定模型与硬件上带来内存和速度收益；不能外推到其他任务、内核或硬件。" }, { sourceId: "finops-unit-economics", supports: "支持区分资源效率单位与每个合格业务结果的单位经济；不单独证明 ROI。" }] },
  ]),
  "data-engineering": freezeQa([
    { q: "同一份数据能否同时用于 RAG、评估和训练？", a: "可以共享来源，但不能默认共享用途；三者的许可、更新、标签、泄漏和删除要求不同。", depth: "RAG 需要当前权威版本和查询时权限，评估需要稳定且未被调参污染的样本，训练还涉及复制、长期保留和权重记忆。应从同一来源身份派生用途明确的数据版本，分别登记使用权、截止条件和删除传播。", ask: "数据所有者允许哪些用途？评估样本如何避免进入训练，撤回后各下游多久生效？", tag: "用途治理", basis: "数据血缘 + 使用目的", evidence: [{ sourceId: "nist-genai-profile", supports: "支持管理生成式 AI 数据来源、用途、隐私和生命周期风险。" }, { sourceId: "nist-zero-trust", supports: "支持资源访问按当前主体和具体资源授权。" }] },
  ]),
  "ai-infra-platform": freezeQa([
    { q: "GPU 利用率很高，为什么训练和推理产出仍可能很差？", a: "利用率只说明设备忙，不说明计算对目标模型有效、作业按时完成或在线请求满足 SLO。", depth: "训练还要看 MFU、Goodput、通信、数据等待、Checkpoint、重试和合格模型产出；推理要看任务成功、尾延迟、拒绝率和单位成功成本。平台优化应从排队原因和业务结果反推资源策略。", ask: "设备忙时完成了多少达标工作？哪些时间花在通信、等待、重试或不合格输出上？", tag: "有效产出", basis: "资源遥测 + 业务结果", evidence: [{ sourceId: "opentelemetry-semconv", supports: "支持用核心资源、运行时与跨组件语义关联系统遥测。" }, { sourceId: "opentelemetry-genai-semconv", supports: "支持关联生成式 AI 模型调用属性；业务有效产出仍需应用定义。" }, { sourceId: "nvidia-gpu-operator", supports: "支持 GPU 运行栈和监控能力，但不自动定义业务 Goodput。" }] },
    { q: "训练和在线推理可以长期混在同一个 GPU 资源池吗？", a: "可以评估共享，但必须证明抢占、干扰、碎片和故障不会破坏在线尾延迟或训练恢复。", depth: "训练偏长作业吞吐和 Checkpoint，在线推理偏优先级、弹性和尾延迟。可采用独立池、保留容量、配额、低优先级训练或设备切分；用峰值、故障和恢复压测决定边界，而不是只看平均利用率。", ask: "在线请求的硬 SLO 是什么？训练被抢占后能丢多少进度，谁承担额外成本？", tag: "混部边界", basis: "调度契约 + 隔离验证", evidence: [{ sourceId: "kubernetes-dra", supports: "支持以设备声明和属性参与调度，不同工作负载仍需明确资源契约。" }, { sourceId: "nvidia-gpu-operator", supports: "支持不同 GPU 共享与运行管理方式；实际隔离和性能需目标环境验证。" }] },
  ]),
  "ai-infra-compute": freezeQa([
    { q: "为什么不能直接用峰值 FLOPS 比较 AI 加速器？", a: "峰值只适用于特定精度和理想算子；真实性能还受运算强度、内存带宽、互联、软件内核、数据供给和工作负载形状限制。", depth: "先用 Roofline 判断目标内核更可能受计算还是数据移动限制，再用目标模型、精度、序列、批量和框架做端到端长跑，观察 HBM、通信、数据等待和恢复。训练与推理的瓶颈不同，单一峰值无法解释首字、同质量训练时间或扩展效率。", ask: "目标负载偏计算、内存、通信还是 I/O 受限？候选硬件在哪一层形成瓶颈？", tag: "规格边界", basis: "Roofline + 端到端验证", evidence: [{ sourceId: "roofline-2009", supports: "支持用运算强度、峰值计算与内存带宽上界区分计算受限和内存受限。" }, { sourceId: "flashattention-2022", supports: "支持减少 HBM 与片上存储的数据移动可改变注意力性能；结果限于目标算子和配置。" }] },
    { q: "多加一倍 GPU，为什么训练速度没有接近翻倍？", a: "设备增加会同时放大通信、同步、数据供给、负载不均和故障开销，扩展效率通常不是线性的。", depth: "比较单卡、单紧耦合互联域和跨域剖析，定位 AllReduce 或 All-to-All、拓扑、拥塞、数据加载、Checkpoint 和 Straggler。MoE 还可能出现专家负载不均。只有在最窄层改善后，增加设备才可能转成有效吞吐。", ask: "扩容后哪项等待时间增长最快？单个紧耦合域内和跨域的有效训练进度分别是多少？", tag: "扩展效率", basis: "通信与系统长跑", evidence: [{ sourceId: "megatron-3d-parallelism-2021", supports: "支持数据、张量与流水线并行在大规模训练中受到跨节点通信和等待限制；论文扩展效率仅适用于其测试配置。" }, { sourceId: "nccl-collectives", supports: "支持 AllReduce、All-to-All 等集合通信及诊断能力，但不保证具体作业或网络获得线性扩展。" }] },
  ]),
});
