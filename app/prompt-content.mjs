...
export const promptDeepDives = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "按策略排序", en: "Order by Policy", mechanism: "明确且稳定的指令与复用内容保持一致顺序；动态证据按主张和重要性组织，避免关键限制长期落在中间。", decision: "排序是否同时考虑缓存、位置偏差和引用完整性？", boundary: "为了缓存而把过期或越权内容放入稳定前缀是错误优化。" },
      ...
      { name: "保存 上下文装配记录（Context Manifest）", en: "Persist Manifest", mechanism: "记录块顺序、淘汰原因、token、模型、Prompt 与工具版本，供评估和事故复盘。", decision: "线上失败能否还原模型当时实际看到了什么？", boundary: "Manifest 可能含敏感元数据，日志同样需要最小化和保留策略。" },
    ],
...
export const promptQa = [
  {
    ...
    ask: "当前错误主要是缺知识、没听懂指令、格式不稳定，还是业务动作执行错误？",
    tag: "能力边界",
    basis: "厂商官方工作流 + 工程分层",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪些约束必须绝对执行？其中哪些已经在模型外通过 IAM、策略或代码落实？",
    tag: "安全边界",
  ...
  },
  {
    ...
    ask: "哪些输入最容易被模型误判？有没有反例、边界例和拒答例可作为评估样本？",
    tag: "核心模式",
    basis: "云厂商官方提示指南",
    evidence: [
  ...
  },
  {
    ...
    ask: "下游系统真正要求哪些字段、枚举和跨字段约束？错误值会造成什么业务后果？",
    tag: "结构化输出",
  ...
  },
  {
    ...
    ask: "工具中哪些只读、哪些会改变状态？谁有权调用、批准和撤销？",
    tag: "工具安全",
    basis: "官方工具调用流程 + 安全指南",
    evidence: [
  ...
  },
  {
    ...
    ask: "这次调用中的每一段上下文来自哪里、由谁维护、何时更新、是否经过权限过滤？",
    tag: "架构边界",
  ...
  },
  {
    ...
    ask: "目前调用的是浮动别名还是固定快照？升级由谁批准，能否快速回滚？",
    tag: "版本治理",
  ...
  },
  {
    ...
    a: "不建议。模板（Prompt Template）应保持明确且稳定的指令与动态变量分离；变量使用类型、长度、来源和权限校验后再注入，并用清晰边界标出不可信数据。",
    ...
    ask: "变量来自用户、数据库还是外部网页？其中哪些是不可信输入或敏感数据？",
    tag: "模板工程",
    basis: "官方工程建议 + 安全边界",
    evidence: [
  ...
  },
  {
    ...
    ask: "什么算成功？不可接受错误有哪些？谁提供权威答案并负责签署上线门槛？",
    tag: "PoC 评估",
  ...
  },
  {
    ...
    ask: "真正需要的是更高任务成功率、可调推理预算，还是必须满足某种审计记录？",
    tag: "推理模型",
    basis: "官方模型行为边界 + 发布评估",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪些步骤可以预先画出，哪些例外必须在运行时根据工具结果判断？",
    tag: "编排边界",
    basis: "厂商工程指南 + 责任分层",
    evidence: [
  ...
  },
  {
    ...
    depth: "设计上可把明确且稳定的指令和复用示例放前部，把用户与动态证据放后部；但缓存键、TTL、模型版本、租户隔离、计费和命中条件都由具体平台决定。应同时测缓存命中率、输入成本、P95、首 Token 延迟和错误隔离，不能把厂商标价折扣直接外推为整体应用降本。",
    ask: "请求中有多少前缀真正相同？按租户还是全局复用？目标模型当前缓存规则和计价是什么？",
    tag: "成本优化",
    basis: "官方产品文档 + 客户流量实测",
    evidence: [
  ...
  },
  {
    ...
    ask: "模型会读取哪些外部内容，读取后能调用哪些工具，哪些动作会产生不可逆影响？",
    tag: "安全",
    basis: "行业安全指南 + 零信任原则",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪些规则是模型行为指导，哪些必须由网关、IAM、代码或规则引擎硬执行？",
    tag: "指令治理",
  ...
  },
  {
    ...
    ask: "下游真正依赖哪些业务不变量？字段值错误或事务部分成功时如何停止与恢复？",
    tag: "输出契约",
  ...
  },
  {
    ...
    ask: "哪些工具职责重叠？模型一次可见多少定义？动态加载由任务、身份还是租户策略决定？",
    tag: "工具上下文",
    basis: "按需工具发现 + 工程评估",
    evidence: [
  ...
  },
  {
    ...
    ask: "当前能否还原每次回答的完整调用配置？谁维护人工金标和回滚触发器？",
    tag: "发布治理",
    basis: "官方持续评估 + 版本工程",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪些规则允许模型按语境判断，哪些规则一旦违反就会造成越权、资金或合规风险？",
    tag: "指令层级",
    basis: "官方指令层级 + 应用控制边界",
    evidence: [
  ...
  },
  {
    ...
    depth: "生产调用应先保护任务、硬约束和输出 Schema，再按相关性、权限、时效和证据等级选择资料，为历史、示例、工具定义和检索证据分配独立预算。对被删除、压缩和摘要的内容记录原因，并保留 上下文装配记录（Context Manifest），才能复盘模型实际看到了什么。长上下文可以减少部分切分工作，但不能替代检索、去重、排序和权限过滤。",
    ask: "一次调用中哪些信息是必须保留、可摘要、可按需检索或绝不能跨租户进入上下文的？",
    tag: "上下文装配",
    basis: "长上下文研究 + 工程装配原则",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪些历史内容决定后续动作，哪些只是改善体验？用户要求删除后，摘要、日志和向量索引是否同步处理？",
    tag: "上下文生命周期",
    basis: "上下文位置研究 + 风险治理",
    evidence: [
  ...
  },
  {
    ...
    ask: "示例来自哪里、是否包含个人或商业敏感信息、谁负责标签正确性和过期清理？",
    tag: "Few-shot 治理",
  ...
  },
  {
    ...
    ask: "目前失败日志能否区分解析、Schema、截断、拒答、业务校验和事务失败？",
    tag: "结构化输出",
  ...
  },
  {
    ...
    ask: "哪些工具结果来自权威数据库，哪些来自开放互联网？动作完成后能否回读真实状态？",
    tag: "工具结果",
    basis: "官方工具闭环 + 提示注入边界",
    evidence: [
  ...
  },
  {
    ...
    ask: "任务最看重成功率、可解释检查点、延迟还是成本？哪些步骤必须由系统或人工批准？",
    tag: "推理模型",
  ...
  },
  {
    ...
    ask: "问题源于缺最新知识、任务说明不清，还是需要大规模稳定复制某种行为？是否要求引用和按用户权限过滤？",
    tag: "方案选型",
    basis: "提示工程工作流 + 架构边界",
    evidence: [
  ...
  },
  {
    ...
    ask: "系统会读取哪些用户无法直接看见的外部内容？这些内容最终能影响哪些写操作、外发或敏感数据访问？",
    tag: "提示注入",
  ...
  },
  {
    ...
    ask: "如果检测器漏掉一次攻击，模型当前能够访问什么数据、调用什么工具、造成多大不可逆影响？",
    tag: "安全控制",
  ...
  },
  {
    ...
    ask: "Prompt 提升在人工盲评和留出集上是否仍成立？换一个 Judge 后结论会不会反转？",
    tag: "评估方法",
    basis: "官方评估实践 + 人工校准",
    evidence: [
  ...
  },
  {
    ...
    ask: "样本是否覆盖真实长尾和高风险任务？谁拥有金标，如何处理业务规则变化导致的答案失效？",
    tag: "评估数据",
    basis: "持续评估 + 风险分层",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪些前缀可跨用户或跨租户复用？目标云的缓存隔离、保留、删除和计费规则是否已经由安全团队确认？",
    tag: "缓存安全",
    basis: "官方缓存机制 + 零信任边界",
    evidence: [
  ...
  },
  {
    ...
    ask: "成本主要来自长输入、长输出、模型档位、低缓存命中、工具调用还是失败重试？",
    tag: "成本优化",
    basis: "缓存机制 + 评估驱动优化",
    evidence: [
  ...
  },
  {
    ...
    ask: "目前只保存 Prompt ID，还是能够还原一次调用的模型、上下文、工具和策略版本？",
    tag: "版本治理",
    basis: "提示版本化 + 完整配置评估",
    evidence: [
  ...
  },
  {
    ...
    ask: "当前发布系统能否一键恢复完整配置？哪些指标触发自动停止，哪些异常必须人工决定？",
    tag: "灰度回滚",
    basis: "持续评估 + 发布工程",
    evidence: [
  ...
  },
  {
    ...
    depth: "售前架构可分为控制面与数据面：控制面管理模板、模型、上下文装配记录（Context Manifest）、评估集、发布和权限；数据面完成请求路由、上下文装配、模型调用、结构与业务校验以及受控工具执行。由此可映射模型服务、搜索与数据、网关、工作流、IAM/KMS、可观测、CI/CD 和 FinOps 等云机会，但必须按目标云核验地域、配额、私网、保留、缓存、密钥、日志和计价，并明确客户、云平台与模型提供方的共享责任。",
    ask: "谁维护模板和上下文策略？要求单云还是多云？是否需要私网、数据驻留、客户管理密钥、跨区域容灾和部门级成本分摊？",
    tag: "云服务承载",
    basis: "工程能力分层 + 持续治理",
    evidence: [
  ...
  },
  {
    ...
    ask: "多云是灾备、议价、数据驻留还是按任务择优？愿意承担多套适配和回归测试成本吗？",
    tag: "多云与可移植性",
    basis: "跨平台指令差异 + 版本化评估",
    evidence: [
  ...
  },
  {
    ...
    ask: "谁定义成功和不可接受错误？PoC 数据是否代表真实流量，是否包含安全与运维验收？",
    tag: "PoC 验收",
    basis: "官方评估前提 + 风险管理",
    evidence: [
  ...
  },
  {
    ...
    ask: "哪类错误可以人工修复，哪类错误一旦发生就必须停止上线？现有人工流程的质量、时延和成本基线是多少？",
    tag: "PoC 门槛",
    basis: "任务特定评估 + 风险分层",
    evidence: [
