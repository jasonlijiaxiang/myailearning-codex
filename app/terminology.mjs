...
export const terminology = Object.freeze({
  ...
  "ai-application-engineering": term("AI 应用工程", "AI Application Engineering", "把模型、Prompt、检索、工具、工作流、策略与评估作为一个可测试、可发布、可回滚的软件系统。", ["ai-application-engineering"]),
  genaiops: term("生成式 AI 运营工程", "Generative AI Operations", "把生成式应用的实验、制品、评估、发布、观测和持续改进接入工程流水线。", ["ai-application-engineering", "ai-ops"], "GenAIOps"),
  "ai-release-manifest": term("AI 发布清单", "AI Release Manifest", "绑定应用代码、模型、Prompt、数据或索引、工具、编排、策略、评估与运行配置的一次发布记录。", ["ai-application-engineering", "ai-governance"]),
  "configuration-bundle": term("成套配置", "Configuration Bundle", "保留模型、Prompt、工具、数据和策略之间的兼容关系，避免单组件升级造成静默退化。", ["ai-application-engineering", "prompt-engineering"]),
  "release-evaluation": term("发布评估", "Release Evaluation", "在固定任务、风险与性能门槛上比较候选和已发布版本，为灰度与回滚提供证据。", ["ai-application-engineering", "evaluation"]),
  "shadow-traffic": term("影子流量", "Shadow Traffic", "让候选版本读取受控真实请求并记录拟议结果，但不对用户响应或业务系统产生未经批准的副作用。", ["ai-application-engineering", "ai-gateway"]),
  ...
  "ai-ops": term("AI 可观测与运营", "AI Operations", "把请求、模型、检索、工具、成本和业务结果关联起来，支持诊断、治理和持续改进。", ["ai-ops", "evaluation", "ai-application-engineering", "ai-finops"]),
  ...
  tco: term("总拥有成本", "Total Cost of Ownership", "把采购、运行、集成、人工维护和失败处理等完整成本一起计算。", ["solution-patterns", "model-landscape", "ai-finops", "ai-infra-compute"], "TCO"),
  "ai-finops": term("AI FinOps 与单位经济", "AI FinOps & Unit Economics", "把跨模型、算力、数据、工具和人工的 AI 成本归因到任务、产品与成功业务结果。", ["ai-finops"]),
  finops: term("技术财务运营", "FinOps", "让工程、财务和业务共同理解并优化技术使用、单位成本、预算和价值。", ["ai-finops", "solution-patterns", "ai-ops", "ai-infra-platform", "ai-gateway"]),
  "unit-economics": term("单位经济", "Unit Economics", "把总成本连接到一个可验证业务单位，用于比较规模扩大后每个成功结果的经济性。", ["ai-finops", "solution-patterns"]),
  "cost-allocation": term("成本分配", "Cost Allocation", "按产品、租户、任务、模型、环境或责任中心把直接与共享费用归到可行动的消费者。", ["ai-finops", "ai-ops"]),
  "cost-to-serve": term("服务全成本", "Cost to Serve", "完成一次服务所需的模型、算力、数据、工具、网络、保证、人工和失败处理成本。", ["ai-finops", "solution-patterns"]),
  "cost-anomaly": term("成本异常", "Cost Anomaly", "相对任务、版本、预算或历史模式显著偏离的支出或用量变化，需要结合业务情境归因和处置。", ["ai-finops", "ai-ops"]),
});
