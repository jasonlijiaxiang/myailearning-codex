...
export const terminology = Object.freeze({
  ...
  embedding: term("向量表示", "Embedding", "学习得到的数值表示；它可以对应 Token、段落、图片或其他对象，其粒度、训练目标与距离含义由具体模型决定。", ["llm", "rag", "data-engineering"]),
  ...
  "kv-cache": term("键值缓存", "Key-Value Cache", "复用已经处理过的上下文 Token 在各层注意力中的键和值，减少后续解码的重复计算。", ["llm-inference", "llm", "ai-infra-compute"], "KV Cache"),
  moe: term("混合专家模型", "Mixture of Experts", "每个 Token 只激活部分专家网络，以较低单次计算换取更大的总参数容量。", ["llm", "model-landscape"], "MoE"),
  multimodal: term("多模态", "Multimodal AI", "围绕同一任务处理或生成文本、图片、文档、音频与视频，并保留模态之间及其与原始证据坐标的关系。", ["multimodal", "model-landscape"]),
  ...
  "data-engineering": term("AI 数据工程", "Data Engineering for AI", "把接入、解析、清洗、版本、质量、权限引用、血缘和生命周期组织成可运营的数据产品。", ["data-engineering", "rag"]),
  "document-intelligence": term("文档智能", "Document Intelligence", "从复杂文档中恢复文字、版面、表格和证据坐标，供搜索、抽取和复核使用。", ["multimodal", "data-engineering"]),
  "data-contract": term("数据契约", "Data Contract", "明确数据负责人、权威来源、语义、用途、格式、更新、权限引用、保留、版本与质量要求。", ["data-engineering", "rag"]),
  ...
  a2a: term("智能体间协议", "Agent2Agent Protocol", "让客户端与独立远端智能体发现能力并交互的协议；远端可以直接返回 Message，也可以创建有状态 Task。", ["a2a", "ai-agent"], "A2A"),
  "agent-card": term("Agent Card 能力名片", "Agent Card", "声明智能体服务元数据、端点、技能、协议和认证要求，供协作方发现候选服务；它不证明真实身份或能力质量。", ["a2a"]),
  "a2a-message": term("A2A 消息", "A2A Message", "客户端与远端智能体的一次通信，可直接形成即时响应，也可关联 Context 或 Task；消息不保证全部持久保存。", ["a2a"]),
  "a2a-task": term("A2A 任务", "A2A Task", "由服务端生成的状态化工作对象，承载跨智能体工作的明确生命周期；Artifact 是可选任务输出。", ["a2a"]),
  artifact: term("任务产物", "Artifact", "与 A2A Task 关联的可交付输出；跨任务版本谱系、访问边界和业务接受由客户端与应用治理。", ["a2a"]),
  ...
  veadk: term("VeADK", "VeADK", "用 Python 定义 Agent、Runner、工具与记忆适配，并可本地运行或对接 AgentKit 的智能体开发框架；部署与业务授权仍由外部系统负责。", ["veadk"]),
  "google-adk": term("Google ADK", "Google Agent Development Kit", "VeADK Agent 所继承或扩展的基础接口；VeADK 的附加能力不等于 Google ADK 原生保证。", ["veadk"], "Google ADK"),
  "agent-runner": term("Agent Runner", "Agent Runner", "接收根 Agent、会话与记忆服务并驱动一次运行事件流的执行对象；它不替代业务事务状态。", ["veadk"]),
  "short-term-memory": term("短期记忆", "Short-term Memory", "围绕会话保存当前交互状态的短期记忆接口；后端、持久化、隔离与恢复能力取决于具体 provider。", ["veadk"]),
  "root-agent": term("根 Agent", "Root Agent", "应用暴露给运行入口的顶层 Agent 对象，作为当前任务路由和执行树起点；它不是全局权限主体。", ["veadk"]),
  "agentkit-app-adapter": term("AgentKit 应用适配器", "AgentKit App Adapter", "把 VeADK root_agent 包装成 AgentKit 可接收的应用入口；包装成功不代表部署、网络或生产验收完成。", ["veadk"]),
  "tool-loop": term("工具循环", "Tool Loop", "Agent 在模型请求工具、受控执行、读取结果并决定下一步之间形成的迭代循环；工具可调用不等于获得业务授权。", ["veadk"]),

  agentkit: term("AgentKit", "Volcengine AgentKit", "用于构建、部署和运行 Agent 应用的火山引擎平台；平台能力不接管客户业务规则和最终结果责任。", ["agentkit"]),
  "agentkit-runtime": term("AgentKit Runtime", "AgentKit Runtime", "托管运行 Agent 应用并提供调用入口的 Runtime 服务；运行成功不等于端到端业务成功。", ["agentkit"]),
  "agent-application": term("Agent 应用", "Agent Application", "包含 Agent 入口、依赖与运行配置的可构建交付单元；它和构建制品、Runtime 实例不是同一对象。", ["agentkit"]),
  "runtime-binding": term("Runtime 资源绑定", "Runtime Binding", "把 Runtime 与模型、工具、记忆、知识、网络或身份等外部资源显式关联的配置；绑定不自动证明权限与数据边界正确。", ["agentkit"]),
  "deployment-mode": term("部署模式", "Deployment Mode", "描述 Agent 应用以何种构建与运行形态交付到 Runtime 的配置选择；可选项与限制需按当前版本核验。", ["agentkit"]),
  "build-deploy-lifecycle": term("构建—部署生命周期", "Build–Deploy Lifecycle", "从本地应用生成制品、创建或更新 Runtime、发布并验证调用的阶段链；launch 等快捷命令不替代生产门禁。", ["agentkit"]),
  "managed-agent-memory": term("托管 Agent 记忆", "Managed Agent Memory", "由平台作为独立资源提供并由 Agent 绑定使用的长期记忆能力；它不等同于 Session、Knowledge 或权威业务记录。", ["agentkit"]),

  evaluation: term("评估", "Evaluation", "用版本化任务、样本、评分器、重复试验和失败归因，为选型、发布与持续运营提供可解释的决策证据。", ["evaluation"]),
  "evaluation-contract": term("评估契约", "Evaluation Contract", "把被测版本、任务与人群、样本切片、评分器、基线、阈值、责任人与后续动作绑定为可重放的测量约定。", ["evaluation", "ai-ops"]),
  "golden-set": term("黄金评估集", "Golden Evaluation Set", "经过来源确认和业务裁决、用于稳定回归比较的代表性与关键风险样本集合。", ["evaluation", "ai-ops"]),
  "evaluation-layers": term("评估二维地图", "Evaluation Matrix", "以被测对象（模型、组件、应用/工作流、业务结果）和证据生命周期（Benchmark 初筛、离线验收、部署后监测）两条轴定位评估，避免用局部指标或单一时点替代端到端成效。", ["evaluation", "ai-ops"]),
  "llm-as-judge": term("模型裁判", "LLM-as-a-Judge", "用模型扩展语义评分，但必须通过人工校准、偏差测试和明确量表控制误判。", ["evaluation"]),
  "ai-application-engineering": term("AI 应用工程", "AI Application Engineering", "把模型、Prompt、检索、工具、工作流、策略与评估作为一个软件系统来测试、发布、停止和恢复。", ["ai-ops"]),
  genaiops: term("生成式 AI 运营工程", "Generative AI Operations", "把生成式应用的实验、制品、评估、发布、观测和持续改进接入工程流水线。", ["ai-ops"], "GenAIOps"),
  "ai-release-manifest": term("AI 发布清单", "AI Release Manifest", "绑定应用代码、模型、Prompt、数据或索引、工具、编排、策略、评估与运行配置的一次发布记录。", ["ai-ops", "ai-governance"]),
  "configuration-bundle": term("成套配置", "Configuration Bundle", "保留模型、Prompt、工具、数据和策略之间的兼容关系，避免单组件升级造成静默退化。", ["ai-ops", "prompt-engineering"]),
  "release-evaluation": term("发布评估", "Release Evaluation", "在固定任务、风险与性能门槛上比较候选和已发布版本，为灰度与回滚提供证据。", ["ai-ops", "evaluation"]),
  "shadow-traffic": term("影子流量", "Shadow Traffic", "让候选版本读取受控真实请求并记录拟议结果，但不对用户响应或业务系统产生未经批准的副作用。", ["ai-ops", "ai-gateway"]),
  ...
  "human-oversight": term("人工监督", "Human Oversight", "为高影响 AI 用途设计持续的人类理解、干预、申诉与停止能力，而不是只在单次工具执行前点击批准。", ["ai-governance", "ai-agent", "evaluation"]),
  ...
  "ai-ops": term("AI 应用工程与运营", "AI Application Engineering & GenAIOps", "把跨组件制品、评估、发布、观测、成本和业务结果连起来，使过程可回放、问题可定位、运行可恢复。", ["ai-ops", "evaluation"]),
  ...
  goodput: term("有效吞吐", "Goodput", "衡量满足质量和服务目标的已完成工作量，而不是只看设备忙碌率或原始吞吐。", ["ai-infra-platform", "llm-inference", "ai-ops"]),
  ...
  "scale-up": term("紧耦合扩展", "Scale-up", "在一个高带宽、低时延的加速器互联域内扩大协同计算能力；该互联域不必严格等同于单台主机。", ["ai-infra-compute", "llm-training"]),
  "scale-out": term("跨域扩展", "Scale-out", "通过紧耦合加速器互联域之间的网络、RDMA、拥塞控制和集合通信扩大工作负载。", ["ai-infra-compute", "llm-training"]),
  ...
  "ai-finops": term("AI FinOps 与单位经济", "AI FinOps & Unit Economics", "把跨模型、算力、数据、工具和人工的 AI 成本归因到任务、产品与成功业务结果。", ["solution-patterns", "ai-ops"]),
  finops: term("技术财务运营", "FinOps", "让工程、财务和业务共同理解并优化技术使用、单位成本、预算和价值。", ["solution-patterns", "ai-ops", "ai-infra-platform", "ai-gateway"]),
  "unit-economics": term("单位经济", "Unit Economics", "把总成本连接到一个可验证业务单位，用于比较规模扩大后每个成功结果的经济性。", ["solution-patterns"]),
  "cost-allocation": term("成本分配", "Cost Allocation", "按产品、租户、任务、模型、环境或责任中心把直接与共享费用归到可行动的消费者。", ["solution-patterns", "ai-ops"]),
  "cost-to-serve": term("服务全成本", "Cost to Serve", "完成一次服务所需的模型、算力、数据、工具、网络、保证、人工和失败处理成本。", ["solution-patterns"]),
  "cost-anomaly": term("成本异常", "Cost Anomaly", "相对任务、版本、预算或历史模式显著偏离的支出或用量变化，需要结合业务情境归因和处置。", ["solution-patterns", "ai-ops"]),
});
...
export const glossaryGroups = Object.freeze([
  ...
  Object.freeze({ id: "agents-protocols", zh: "智能体与协议", en: "Agents and Protocols", termIds: Object.freeze(["ai-agent", "harness", "harness-engineering", "agent-engineering", "perceive", "reason", "act", "observe", "planning", "memory", "tools", "tool-calling", "api", "mcp", "mcp-protocol-roles", "mcp-primitives", "a2a", "agent-card", "a2a-message", "a2a-task", "artifact", "tool-discovery", "agent-collaboration", "veadk", "google-adk", "agent-runner", "short-term-memory", "root-agent", "agentkit-app-adapter", "tool-loop"]) }),
  Object.freeze({ id: "evaluation-security", zh: "应用交付、评估、安全与治理", en: "Application Delivery, Evaluation, Security and Governance", termIds: Object.freeze(["agentkit", "agentkit-runtime", "agent-application", "runtime-binding", "deployment-mode", "build-deploy-lifecycle", "managed-agent-memory", "evaluation", "evaluation-contract", "golden-set", "evaluation-layers", "llm-as-judge", "ai-application-engineering", "genaiops", "ai-release-manifest", "configuration-bundle", "release-evaluation", "shadow-traffic", "ai-governance", "ai-inventory", "ai-risk-tiering", "impact-assessment", "human-oversight", "governance-evidence", "continuous-assurance", "security", "ai-ops", "observability", "guardrails", "identity-authorization", "iam", "acl", "dlp", "hitl"]) }),
  Object.freeze({ id: "training-inference", zh: "训练、MLOps 与推理", en: "Training, MLOps and Inference", termIds: Object.freeze(["predictive-ai-mlops", "feature-store", "model-registry", "point-in-time-correctness", "training-serving-skew", "model-drift", "llm-training", "pretraining", "distributed-training", "fine-tuning", "sft", "peft", "rlhf", "lora", "qlora", "dpo", "llm-inference", "batching", "quantization", "ttft", "tpot"]) }),
  ...
  Object.freeze({ id: "solution-delivery", zh: "方案、成本与交付", en: "Solution, Economics and Delivery", termIds: Object.freeze(["solution-patterns", "ai-gateway", "model-routing", "rate-limiting", "semantic-cache", "poc", "sla", "slo", "tco", "ai-finops", "finops", "unit-economics", "cost-allocation", "cost-to-serve", "cost-anomaly"]) }),
]);
