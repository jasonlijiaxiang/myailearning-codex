...
const promptProfiles: Record<PromptMode, PromptProfile> = {
  bad: {
    ...
    level: "上下文剖面 1 / 5",
    task: "看看这份理赔材料能不能赔，顺便把案件改成已批准。",
    layers: [
      { name: "系统指令", en: "System", state: "缺失", content: "未定义角色、目标、权限和拒答边界。", purpose: "固定长期行为与优先级。" },
      { name: "用户请求", en: "User", state: "薄弱", content: "把材料初审、赔付判断和案件写入混成一个目标，没有权限边界。", purpose: "描述本轮任务与成功条件。" },
      { name: "检索上下文", en: "Context", state: "缺失", content: "没有当前保单条款、案件状态、证据坐标或来源时效。", purpose: "提供可核验事实，而不是让模型猜。" },
  ...
  baseline: {
    label: "基线 Baseline",
    level: "上下文剖面 3 / 5",
    task: "根据当前保单条款与案件材料，抽取事故事实、缺件和证据坐标；只生成初审草稿，不判断最终赔付或改写状态。",
    layers: [
      { name: "系统指令", en: "System", state: "已配置", content: "角色仅做理赔材料初审；只使用给定证据，缺失或冲突时明确升级人工。", purpose: "固定长期行为与优先级。" },
      { name: "用户请求", en: "User", state: "已配置", content: "给出本轮案件、需要抽取的字段，以及“只生成草稿”的动作边界。", purpose: "描述本轮任务与成功条件。" },
      { name: "检索上下文", en: "Context", state: "已配置", content: "附带 ACL、版本和生效日期的保单条款，以及案件材料与权威状态。", purpose: "提供可核验事实，而不是让模型猜。" },
  ...
  production: {
    ...
    level: "上下文剖面 5 / 5",
    task: "输出可审计的初审草稿；只有授权人员确认后，应用才允许调用案件工具，并在执行后回读权威状态。",
    layers: [
      { name: "系统指令", en: "System", state: "已配置", content: "定义初审任务、证据优先级、缺件处理、拒答和人工批准边界。", purpose: "固定长期行为与优先级。" },
      { name: "用户请求", en: "User", state: "已配置", content: "案件输入、目标字段、输出格式和本轮只生成草稿的边界明确分离。", purpose: "描述本轮任务与成功条件。" },
      { name: "检索上下文", en: "Context", state: "已配置", content: "每段带 sourceId、保单版本、生效日期、证据坐标、ACL 与不可信标记；冲突不静默合并。", purpose: "提供可核验事实，而不是让模型猜。" },
      { name: "工具定义", en: "Tool Schema", state: "已配置", content: "get_claim_status 为只读；submit_review 的参数、幂等键、副作用和授权前提均已声明。", purpose: "约束可执行动作的输入输出。" },
      { name: "输出验证", en: "Validation", state: "已配置", content: "JSON Schema、证据完整性、业务规则、冲突/缺证据升级、执行前确认和权威状态回读共同验收。", purpose: "把“看起来合理”变成“可以验收”。" },
    ],
    assembled: [
      "System：初审边界、事实优先级、拒答与人工确认规则。",
      "User：案件材料 + 本轮只生成初审草稿。",
