...
export const dataEngineeringBrief = {
  ...
  presentation: "pipeline",
  principleTitle: "把源数据整理为可用的 AI 数据",
  principles: [
    {
      zh: "来源与契约",
    ...
    },
    {
      ...
      explanation:
        "方案启动前先按五问分诊数据就绪度：数据能否合法稳定访问；对象是否有稳定身份、版本与删除语义；权限能否传播到索引、检索、生成与日志；质量是否足以支撑目标任务和评估；变更、删除与纠错能否进入运行流程。",
      decision:
    ...
    },
    {
      ...
      decision:
        "先画数据从采集、处理、检索、生成到删除的完整路径，再判断是否需要安全评估、标准合同或认证。",
    },
  ],
  decisions: [
    {
    ...
    },
    {
      ...
      recommendation:
        "缩到单一权威来源、有限对象集合、明确 ACL 与可回放评估集；先证明端到端判断成立，再逐项补齐数据契约。",
      boundary:
    ...
    },
    {
      ...
      recommendation:
        "先完成数据分类与位置映射，识别个人信息、敏感个人信息与重要数据，记录数量与时间窗口，画出跨境路径并核对出境通道（安全评估、标准合同或认证）与告知、单独同意、影响评估义务；同时验证模型服务的数据保留、训练使用和删除能力，形成可审计证据包。",
      boundary:
...
export const aiInfraComputeBrief = {
  ...
  principles: [
    ...
    },
    {
      ...
      explanation:
        "采购前按顺序核验五类约束：工作负载合同（训练/推理、模型、精度、序列、并发、质量、SLO 与增长）；设施条件（电力、散热、机架、布线与故障容量）；内存与数据路径（HBM、KV Cache、存储、检查点与加载恢复）；通信（Scale-up、Scale-out、网络与拓扑）；软件与运维（驱动、运行时、框架、镜像、监控与升级）。",
      decision:
  ...
  decisions: [
    ...
    },
    {
      ...
      recommendation:
        "先通过设施与供给就绪门：电力与散热余量、机架与布线、内存与数据路径、网络拓扑、软件与运维、供应连续性（设备交付、替代、云上补充与退出）逐项确认后再下单。",
      boundary:
        "机架空间足够不等于具备高密度 AI 集群条件；能源、散热与故障容量必须实测。",
    },
    {
      ...
      recommendation:
        "不做固定排位。按四个面分别核验：工作负载可运行性（模型、算子、编译器、量化、分布式通信、镜像与监控）、生态成熟度（驱动、运行时、框架与升级路径）、供应连续性（交付周期、替代货源、云上补充与退出）和单位有效作业成本；并保留可迁移层。",
      boundary:
...
export const aiInfraPlatformBrief = {
  ...
  definition:
    "AI 基础设施平台（AI Infrastructure Platform）是面向开发者与运维者的自助平台，用于申请和使用异构算力、网络、存储、运行时和策略。它负责准入、调度、隔离、恢复和观测，并让训练与推理的成本去向可解释。",
  position:
