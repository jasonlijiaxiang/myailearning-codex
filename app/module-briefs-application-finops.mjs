...
export const aiApplicationEngineeringContribution = {
  definition: "AI 应用工程（AI Application Engineering）把模型、Prompt、检索、工具、工作流、策略、评估和运行配置作为软件系统来管理：记录版本、测试、发布、停止和恢复；GenAIOps 则把这些非确定性组件的变更纳入持续交付和日常运行改进。",
  position: "位于应用模式与工程保障之间，负责把 RAG、Agent、多模态和模型能力组合成可交付应用；它复用评估、安全、网关和可观测能力，但重点回答一次跨组件变更如何形成同一个发布单元。",
  presentation: "lifecycle",
  principleTitle: "让生成式 AI 应用从实验进入受控生产",
  principles: [
  ...
  qa: [
    { q: "GenAIOps 与传统 DevOps 有什么不同？", a: "DevOps 管理软件代码和基础设施交付；GenAIOps 还要管理模型、Prompt、检索数据、工具、评估和非确定输出的组合变化。", depth: "两者应叠加而不是替代。单元测试仍验证确定性代码，数据集与人工裁决验证语义质量，端到端测试验证业务终态，发布清单把它们绑定到同一版本。", ask: "当前流水线能证明哪些组件没有退化，哪些只证明构建和部署成功？", tag: "职责边界", basis: "生成式应用生命周期 + DevOps 边界", evidence: [evidence("google-genai-app-lifecycle", "生成式应用需要适配 DevOps 与 MLOps，并管理 Prompt、检索库和 Adapter 等多类制品。"), evidence("azure-genaiops", "AI 工作负载同时需要 DevOps、DataOps、MLOps 与 GenAIOps。")], addedAt: "2026-07-21" },
    { q: "为什么只版本化 Prompt 还不够？", a: "因为同一 Prompt 在不同模型、检索数据、工具 Schema、策略和运行参数下可能产生不同结果。", depth: "发布清单至少绑定应用代码、模型与参数、Prompt、数据或索引、工具契约、编排、策略和评估报告。外部服务还要记录可获得的稳定版本或变更时间。", ask: "今天能否从一次生产输出还原当时完整配置和外部依赖？", tag: "发布清单", basis: "成套配置 + 可重放性", evidence: [evidence("azure-foundation-model-lifecycle", "Prompt、配置、编排和模型版本需要正确配套。"), evidence("google-genai-app-lifecycle", "部署需要管理 Prompt、Chain、Embedding 模型、检索存储和 Adapter 等制品。")], addedAt: "2026-07-21" },
    { q: "模型供应商宣布兼容升级，是否可以跳过回归？", a: "不可以。接口兼容不保证行为、Token、工具调用、拒答、延迟、成本或安全表现一致。", depth: "先在固定任务与保护组上比较新旧版本，再验证 Prompt、工具和数据兼容；必要时调整应用并作为一组发布。对停服升级还要预先演练替代、回退和数据迁移。", ask: "升级最可能改变哪些关键任务和工具副作用？旧版本能保留多久？", tag: "模型升级", basis: "模型生命周期设计", evidence: [evidence("azure-foundation-model-lifecycle", "模型变化可能要求同步修改 Prompt、编排、Grounding 数据和硬件。")], addedAt: "2026-07-21" },
    { q: "怎样测试一个输出不确定的 AI 应用？", a: "把确定性契约与语义质量分开：前者使用断言，后者使用代表性数据、量表、模型裁判和人工复核。", depth: "端到端还要验证工具后置条件、权限、失败恢复和业务终态。总体平均分不能掩盖高风险错误，Judge 需要与人工校准并保存版本。", ask: "哪些结果可以确定性验证，哪些需要人工裁决，哪些错误零容忍？", tag: "测试方法", basis: "分层评估 + 业务终态", evidence: [evidence("aws-genai-lifecycle", "生成式应用生命周期需要持续评估集成组件和生产表现。"), evidence("openai-eval-best-practices", "任务特定、持续评估和人工校准是可靠评估基础。")], addedAt: "2026-07-21" },
    { q: "影子发布会不会造成重复业务动作？", a: "如果影子路径可以调用真实写工具，就会。影子只应产生可审计的拟议结果，副作用必须被隔离、替换或阻断。", depth: "读取也要遵守身份和数据最小化。对写入工具使用模拟端点、事务回滚或只比较动作计划；任何真实灰度都需要幂等键、预算、审批与补偿。", ask: "影子流量会访问哪些真实数据和工具？怎样证明没有产生副作用？", tag: "影子发布", basis: "副作用隔离 + 受控流量", evidence: [evidence("azure-foundation-model-lifecycle", "路由、流量镜像和蓝绿发布可用于验证变更。"), evidence("nist-zero-trust", "每次资源访问仍需按真实主体和策略验证。")], addedAt: "2026-07-21" },
    { q: "线上失败是否应该自动加入评估集？", a: "不应该直接加入。需要先脱敏、去重、确认来源并裁决真正期望结果。", depth: "未经确认的用户反馈、模型输出或业务结果可能包含噪声、攻击、隐私和错误标签。只有能代表新分布或系统缺陷的案例才进入冻结回归集，并记录采集条件和责任人。", ask: "谁判断一条线上失败属于数据、模型、工具还是业务规则问题？", tag: "反馈治理", basis: "持续评估 + 数据治理", evidence: [evidence("google-genai-app-lifecycle", "开发、评估、监控和持续改进形成反馈循环。"), evidence("openai-eval-best-practices", "生产分布样本应经过任务化评估设计。")], addedAt: "2026-07-21" },
    { q: "RAG、Agent 和多模态应用能共用一套发布门吗？", a: "可以共用版本、审批、遥测和风险底线，但必须保留专用质量与失败测试。", depth: "RAG 要测检索与引用，Agent 要测工具、状态和副作用，多模态要测采集、解析和证据坐标。平台提供共同执行框架，各应用对自己的业务终态负责。", ask: "希望统一的是控制面、工具链，还是把所有应用强行使用同一指标？", tag: "平台边界", basis: "共同控制 + 专用证据", evidence: [evidence("aws-genai-lifecycle", "生成式生命周期覆盖 Prompt、RAG、Agent 和模型定制等不同组件。"), evidence("azure-genaiops", "不同运营方法按应用、数据和模型职责组合使用。")], addedAt: "2026-07-21" },
    { q: "出现质量事故时应该先调 Prompt 还是先换模型？", a: "都不是默认答案。先恢复业务并沿版本、输入、检索、工具、策略和模型逐层归因。", depth: "可先回滚、降级或转人工，再用同一失败样本验证假设。未经归因同时调整多个组件，会让修复不可证实并可能引入新退化。", ask: "事故发生前哪些组件发生了变化？有没有可用的已验证发布清单？", tag: "事故归因", basis: "可重放发布 + 单变量验证", evidence: [evidence("google-genai-app-lifecycle", "持续监控与改进需要管理应用各类制品和变化。"), evidence("azure-foundation-model-lifecycle", "模型、Prompt、编排与数据变化具有联动影响。")], addedAt: "2026-07-21" },
    { q: "什么时候不值得建设完整 GenAIOps 平台？", a: "只有少量低风险试验、变化很少且人工发布可可靠复核时，不必先建大平台。", depth: "仍要保留最小版本、数据集、发布记录和停止条件。随着应用、团队、供应商或风险增加，再把重复控制平台化；平台投资应由减少交付风险和重复劳动的证据支持。", ask: "当前最频繁、最昂贵、最难解释的发布失败是什么？", tag: "建设边界", basis: "渐进成熟度 + 平台投资", evidence: [evidence("azure-genaiops", "运营方法按工作负载任务和成熟度组合采用。"), evidence("aws-genai-lifecycle", "生命周期从范围界定到持续改进逐步建立。")], addedAt: "2026-07-21" },
  ],
  evidenceCards: [
    ...
    { metric: "生命周期", title: "生成式 AI 应用的完整生命周期", finding: "AWS Generative AI Lens 把范围、选型、定制、集成、部署和持续改进置于同一生命周期。", boundary: "Well-Architected 指南不是某个应用已经达到生产要求的认证。", sourceId: "aws-genai-lifecycle" },
...
export const aiFinopsContribution = {
  ...
  qa: [
    { q: "AI FinOps 是否就是统计 Token 成本？", a: "不是。Token 只是部分模型服务的技术用量，AI 全成本还包括算力、数据、检索、工具、网络、评估、安全、人工和失败。", depth: "Token 数据适合解释输入输出结构和供应商账单，但不能覆盖自建 GPU、固定承诺、SaaS 席位或人工复核。最终应回到产品、任务和成功业务结果。", ask: "除模型账单外，哪些团队和系统正在为同一 AI 任务付费？", tag: "成本边界", basis: "AI 技术类别 + TCO", evidence: [evidence("finops-ai-category", "AI 支出跨越模型服务、云、数据中心、SaaS 和多类供应商。"), evidence("finops-ai-overview", "AI 成本治理需要连接业务价值和完整技术使用。")], addedAt: "2026-07-21" },
    { q: "什么时候应该建立独立 AI FinOps 范围？", a: "只有当 AI 的业务目标、波动、归因或治理要求需要不同决策时；不是所有 AI 费用都要另立范围。", depth: "可以先在现有云或产品范围增加 AI 标签与指标。如果领导层需要独立投资节奏、跨技术类别归因或不同创新容忍度，再建立有明确退出条件的范围。", ask: "独立范围将改善哪一项预算、架构或投资决定？", tag: "范围设计", basis: "FinOps Scopes", evidence: [evidence("finops-scopes", "新范围只应在不同业务结果、约束或决策情境需要时建立。"), evidence("finops-ai-category", "AI 费用可按组织目标形成差异化关注范围，但不要求全部纳入。")], addedAt: "2026-07-21" },
    { q: "每百万 Token 更便宜的模型，为什么总成本可能更高？", a: "因为它可能需要更长 Prompt、更多输出、更多重试、检索或人工修正，最终成功率也可能更低。", depth: "应在同一任务和质量门槛下比较输入、输出、缓存、工具、重试、P95、失败和人工成本。更高单价模型若减少轮次和返工，单位成功成本反而可能更低。", ask: "当前成本按调用、Token，还是按合格业务结果比较？", tag: "单位经济", basis: "技术用量 + 成功结果", evidence: [evidence("finops-framework", "Unit Economics 属于量化业务价值的核心能力。"), evidence("finops-ai-category", "AI 需要把成本与模型输出消费者和业务产出连接。")], addedAt: "2026-07-21" },
    { q: "GPU 利用率低是否表示应该减少容量？", a: "不一定。在线服务可能为尾部时延、故障和突发流量保留余量，训练也可能受数据、网络或成组调度限制。", depth: "同时看 Goodput、排队、SLO、拓扑碎片、故障、峰谷和单位达标结果成本。先定位闲置原因，再决定共享、调度、预留或缩容。", ask: "低利用期间是否仍满足必要余量？等待的是计算、数据、网络还是成组资源？", tag: "容量经济", basis: "有效产出 + 服务余量", evidence: [evidence("finops-ai-overview", "AI 成本治理需要优化 GPU 分配并持续关联使用和结果。"), evidence("finops-ai-category", "架构与工作负载放置应服务成本和业务价值。")], addedAt: "2026-07-21" },
    { q: "怎样给多 Agent 任务分摊成本？", a: "先用统一父任务 ID 关联子 Agent、模型、检索、工具与人工，再按可解释的消费者或业务结果归集。", depth: "共享组件可按请求、资源时间或约定比例分摊，但要公开规则和未分配金额。不要把子调用成本丢给平台公共池，否则无法比较编排收益和循环浪费。", ask: "一次最终任务能否追到所有子任务、重试、工具和人工升级？", tag: "Agent 归因", basis: "端到端任务 + 分摊规则", evidence: [evidence("finops-ai-category", "多层 AI 架构和多 Agent 工作负载会增加成本分配复杂度。"), evidence("opentelemetry-genai-semconv", "生成式 AI Trace 可关联模型调用和使用量属性。")], addedAt: "2026-07-21" },
    { q: "成本异常应该自动停掉 AI 服务吗？", a: "通常不应仅凭费用阈值自动全停。应结合任务、风险、业务影响和异常来源采取限流、降级、审批或隔离。", depth: "突增可能来自攻击、循环、供应商计费变化，也可能是正常业务高峰。高风险写操作可快速阻断，核心服务则优先使用预算上限、保护组和已验证降级。", ask: "哪些支出异常意味着攻击或故障，哪些代表正常业务增长？", tag: "异常管理", basis: "成本异常 + 业务连续性", evidence: [evidence("finops-ai-category", "AI 支出波动和异常管理需要更高频、情境化处理。"), evidence("finops-framework", "异常管理属于理解使用与成本的能力。")], addedAt: "2026-07-21" },
    { q: "缓存命中率提高是否一定降本？", a: "不一定。误命中、过期、跨租户泄漏和失效维护可能增加业务风险与人工成本。", depth: "只对可复用、权限一致、时效明确的请求测净收益，同时记录命中后的任务成功、错误复用、存储、检索和失效成本。高命中率不能替代正确性。", ask: "哪些响应可被谁复用、多久有效，错误复用的代价是什么？", tag: "优化边界", basis: "缓存收益 + 风险门槛", evidence: [evidence("finops-ai-category", "使用优化需与业务价值和风险共同判断。"), evidence("nist-zero-trust", "资源访问不能因技术复用绕过身份与策略验证。")], addedAt: "2026-07-21" },
    { q: "API 与自建模型应该怎样做成本比较？", a: "在同一质量、容量和责任边界下比较全成本，而不是拿 API 标价对比 GPU 采购价。", depth: "API 要计调用、数据、配额、地域和供应连续性；自建要计设备、闲置、能耗、网络、软件、团队、升级、故障和退出。结果以稳定工作负载下每个达标任务成本呈现。", ask: "负载、质量和增长是否已稳定到足以支持长期容量承诺？", tag: "采购选型", basis: "TCO + 工作负载放置", evidence: [evidence("finops-framework", "架构与工作负载放置、费率优化和单位经济共同支持技术价值。"), evidence("finops-ai-category", "AI 支出横跨托管服务、基础设施和多种采购渠道。")], addedAt: "2026-07-21" },
    { q: "AI 项目没有直接收入，怎样衡量价值？", a: "使用与业务目标相连的可验证结果，如处理时间、解决率、产能、风险避免、体验或韧性，并同时报告质量和归因边界。", depth: "建立人工或现有系统基线，记录采用率、节省时间是否转化为实际容量，以及副作用与维护成本。不能把生成次数、活跃用户或理论节省自动等同于价值。", ask: "如果项目成功，哪个业务流程或风险状态会真实改变，如何观测？", tag: "价值衡量", basis: "业务价值 + 基线", evidence: [evidence("finops-ai-overview", "AI 投资需要与成本效率、韧性、体验、生产率、可持续性和增长等业务价值连接。"), evidence("finops-framework", "FinOps 以最大化技术业务价值为目标。")], addedAt: "2026-07-21" },
  ],
  evidenceCards: [
...
export const applicationFinopsCurriculum = {
  ...
  "ai-finops": {
    ...
    chapters: [
      ...
      { title: "异常处理与价值复盘", en: "Operate & Review", explanation: "成本异常与质量、风险、业务需求和版本事件联合判断；产品、工程和财务共同复盘单位经济与投资状态。", decision: "将成本信号用于调整产品和架构路线。", boundary: "自动停服可能扩大业务损失。", sourceIds: ["finops-ai-category", "finops-ai-overview"] },
    ],
  },
};
...
export const applicationFinopsLearning = {
  "ai-application-engineering": {
    ...
    route: [
      ...
      { title: "训练评估与发布门槛", learn: "按组件、风险和业务终态设计测试与保护组。", checkpoint: "每次放量都有证据和停止条件。" },
