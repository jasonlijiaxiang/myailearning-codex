...
test("localization registry passes its recursive schema and covers every module", () => {
  ...
  for (const maintenanceId of [
    ...
    "erm-batch03-unified-readers-2026-08-30",
  ...
  const batch03Readers = maintenances.get("erm-batch03-unified-readers-2026-08-30");
  assert.equal(batch03Readers?.kind, "document-shell");
  assert.equal(batch03Readers?.receiptId, "receipt-batch03-unified-readers-runtime-2026-08-30");
  assert.deepEqual(batch03Readers?.changedRendererFiles, [
    "app/a2a-module-experience.tsx",
    "app/fieldbook-interactions.tsx",
    "app/i18n/english-pilot-module-page.tsx",
    "app/unified-brief-module-config.mjs",
  ]);
  assert.deepEqual(batch03Readers?.affectedModuleSlugs, [...publishedModuleSlugs].sort());
  assert.deepEqual(batch03Readers?.contentProjectionChangeSlugs, ["a2a", "multimodal", "veadk"]);
  assert.equal(batch03Readers?.metadataScope, "none");
  const batch04Readers = maintenances.get("erm-batch04-unified-readers-2026-08-30");
  assert.equal(batch04Readers?.kind, "document-shell");
  assert.equal(batch04Readers?.receiptId, "receipt-batch04-unified-readers-runtime-2026-08-30");
  assert.deepEqual(batch04Readers?.changedRendererFiles, [
    "app/dense-module-reading-modes.tsx",
    "app/i18n/english-pilot-module-page.tsx",
    "app/unified-brief-module-config.mjs",
  ]);
  assert.deepEqual(batch04Readers?.affectedModuleSlugs, [...publishedModuleSlugs].sort());
  assert.deepEqual(batch04Readers?.contentProjectionChangeSlugs, ["agentkit", "ai-governance", "evaluation"]);
  assert.equal(batch04Readers?.metadataScope, "none");
  const batch05Readers = maintenances.get("erm-batch05-unified-readers-2026-08-30");
  assert.equal(batch05Readers?.kind, "document-shell");
  assert.equal(batch05Readers?.receiptId, "receipt-batch05-unified-readers-runtime-2026-08-30");
  assert.deepEqual(batch05Readers?.changedRendererFiles, [
    "app/(zh)/modules/[slug]/page.tsx",
    "app/i18n/english-pilot-module-page.tsx",
    "app/module-pilot-views.tsx",
    "app/unified-brief-module-config.mjs",
  ]);
  assert.deepEqual(batch05Readers?.affectedModuleSlugs, [...publishedModuleSlugs].sort());
  assert.deepEqual(batch05Readers?.contentProjectionChangeSlugs, ["ai-gateway", "security", "solution-patterns"]);
  assert.equal(batch05Readers?.metadataScope, "none");
  const batch06Readers = maintenances.get("erm-batch06-unified-readers-2026-08-31");
  assert.equal(batch06Readers?.kind, "document-shell");
  assert.equal(batch06Readers?.receiptId, "receipt-batch06-unified-readers-runtime-2026-08-31");
  assert.deepEqual(batch06Readers?.changedRendererFiles, [
    "app/(zh)/modules/prompt-engineering/page.tsx",
    "app/i18n/english-pilot-module-page.tsx",
    "app/unified-brief-module-config.mjs",
  ]);
