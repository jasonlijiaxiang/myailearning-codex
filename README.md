...
`npm run check` 是日常门禁，覆盖代码检查、内容、发布分支回归和网站构建；日常开发用 `npm run check:fast`（约 30 秒），提交前用 `npm run check`。`main` 是唯一生产分支；只有推送到 `origin/main` 的正式变更才用同一精确提交保存和部署 GPT Sites 版本。任务分支可以推送做跨设备同步或可选审查，但不发布公开站点。离线 ZIP、跨机器迁移和私有聊天整理不属于日常发布步骤。
